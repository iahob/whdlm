...
{
    public partial class RecordGroupScoreChangeList : AdminPage
    {
        /// <summary>
        /// 页面加载
        /// </summary>
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ddlChangeType.DataSource = GetSerialTypeList(typeof(GroupScoreType));
        ...
        private void RecordInsureDataBind()
        {
            ...
            if (!string.IsNullOrEmpty(queryContent))
            {
                ...
                condition.AppendFormat(" AND {0}={1}", searchType == 1 ? "GroupID" : "",
                    queryContent);
            }
            ...
            PagerSet pagerSet = FacadeManage.aideGroupFacade.GetList(RecordGroupScoreChange.Tablename,
                condition.ToString(), Orderby, anpNews.CurrentPageIndex, anpNews.PageSize);
            anpNews.RecordCount = pagerSet.RecordCount;
            if (pagerSet.RecordCount > 0)
            {
                var outTotal = FacadeManage.aideGroupFacade.GetTotal(RecordGroupScoreChange.Tablename,"ChangeScore", condition.ToString() + " AND ChangeType=0");
                var inTotal = FacadeManage.aideGroupFacade.GetTotal(RecordGroupScoreChange.Tablename,"ChangeScore", condition.ToString() + " AND ChangeType=1");
                rptInsure.DataSource = pagerSet.PageSet.Tables[0];
                rptInsure.DataBind();
                ltTotal.Text = $"发放总数：{-outTotal} 扣除总数：{inTotal} (当前条件统计)";
            }
