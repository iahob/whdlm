 ...
 */
function gettokenc(params, callback) {
  $.ajax(generateUrl("gettoken", params)).done(function(data) {
    callback(data);
  });
}
    ...
    if (sourceData.valid === true) {
      sourceData.list = sourceData.list.map(info => {
        return Object.assign({}, info, {
          NickName: info.NickName + " / " + info.GameID,
          TypeDesc: info.IsAgent ? "代理" : "玩家"
        });
      });
      callback(sourceData);
    }
  ...
  switch (type) {
    ...
    case "revenue":
      return record.map(value => {
        value.IsAgent = value.IsAgent ? "代理" : "玩家";
        return value;
      });
    default:
      return record;
...
function present(params, callback) {
  $.ajax(generateUrl("PresentScore", params)).done(function(data) {
    toLogin(data) || callback(data);
  });
}
 ...
 * @param {Function} callback
 */
function presentdiamondorscore(params, callback) {
  $.ajax(generateUrl("presentdiamondorscore", params)).done(function(data) {
    toLogin(data) || callback(data);
  });
}
/**
