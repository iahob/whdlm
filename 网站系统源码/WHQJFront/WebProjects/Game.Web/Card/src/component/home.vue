<template>
  <div class="ui-main">
      ...
      <div class="ui-info-mydetail">
        <ul>
          <li class="ui-mydetail-1 ui-left">
            ...
            <p>身份证号码：<span>{{info.PassPortID}}</span><a href="javascript:;" :disabled="disabled" @click="presentdiamond">赠送钻石</a></p>
          </li>
        </ul>  
      </div>
    ...
    <div class="ui-main-diamond">
      <h2>
      <img src="../assets/images/diamond.png" />钻石提成
      <router-link to="/payInfo">查看详情</router-link>
      <router-link class="ui-main-diamond-grant" to="/grantDiamondInfo">赠送记录</router-link>
      </h2>
    ...
    <div class="ui-main-gmoney">
      <h2><img src="../assets/images/gold.png" />游戏币提成<router-link to="/expendInfo">查看详情</router-link>
      <router-link class="ui-main-gmoney-grant" to="/grantGoldInfo">赠送记录</router-link>
      </h2>
    ...
    <ui-dialog :show="msg.show">
      <extract :info="msg" :limit="save[msg.content]" v-on:close="closeDialog"></extract>
    </ui-dialog>
     <ui-dialog :show="presentmsg.show">
      <presentdiamond :info="presentmsg" :limit="save[presentmsg.content]" v-on:presentclose="closePresentDialog"></presentdiamond>
    </ui-dialog>
  </div>
</template>
...
import presentdiamond from "./presentdiamond";
import { mapMutations } from "vuex";
import { getInfo, userFace } from "../fetch/fetch";
export default {
  name: "home",
  props: {
    nav: String
  },
  components: { top, loading, UiDialog, message, extract,presentdiamond },
  data: function() {
    return {
      loading: false,
      disabled: false,
  ...
  methods: {
    ...
    closePresentDialog: function(){
      console.log(1);
      this.presentmsg.show = false;
    },
    takeDiamond: function() {
      this.msg = {
        content: "钻石",
        award: this.info.DiamondAward,
        presentType: 1,
        show: true,
        cleanInput: true
      };
      this.msg.limit = this.save[this.msg.content];
    },
    takeGold: function() {
      this.msg = {
        content: "游戏币",
        award: this.info.GoldAward,
        presentType: 2,
        show: true,
        cleanInput: true
      };
     ...
     presentdiamond:function(){
      ...
      this.presentmsg.limit=this.save[this.presentmsg.content];
    }
  }
};
</script>
