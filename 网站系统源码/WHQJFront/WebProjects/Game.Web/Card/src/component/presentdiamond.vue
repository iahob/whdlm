<template>
  <div class="ui-dialog-message ui-message">
    <div class="ui-message-close">
       赠送{{info.content}}
      <a @click="$emit('presentclose')"><img :src="closeImg"/></a>
    </div>
    <div class="ui-message-content">
      <div class="ui-panel">
        <div class="ui-take">
          <p>可送{{info.content}} ：<span>{{info.diamond}}</span></p>
        </div>
        <div class="ui-form-item">
          <label>游戏&nbsp;I&nbsp;D&nbsp; ：</label>
          <input type="text" class="ui-value" v-model="gameId" @change="gameIdChange" placeholder="不填赠送给自己">
        </div>
        <div class="ui-form-item">
          <label>游戏昵称 ：</label>
...
<script>
import { presentdiamondorscore,getInfo, getNickNameByGameID } from "../fetch/fetch";
  ...
  methods: {
    ...
    getAgentInfo: function() {
      getInfo({ token: localStorage.getItem('token') }, data => {
        if (data.data.valid) {
          console.info(data);
          this.info.diamond = data.data.info.Diamond
        }
      })
    },
    giveAward: function(e) {
      ...
      presentdiamondorscore(params, data => {
