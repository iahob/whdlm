...
function ClubRoomCreateLayer:ctor( scene,param,level )
    ...
    if self.param.tabConfigInfo then
        ...
        for i=1,7 do
            self.lastData[i] = cc.UserDefault:getInstance():getIntegerForKey(GlobalUserItem.tabAccountInfo.dwUserID.."blackjackData"..i, -1)
        end
    end
    ...
    --上庄配置
    for i = 1, 2 do
        local checkbx = self.m_nodeRule:getChildByName("node_config_3"):getChildByName("zhuang" .. i)
        if nil ~= checkbx then
            checkbx:setVisible(true)
            checkbx:setTouchEnabled(true)
            checkbx:setTag(CBT_BANKER + i)
            checkbx:addTouchEventListener(cbtlistener)
            checkbx:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
            self.m_tabRuleCheckBox[CBT_BANKER + i] = checkbx
        end
    end    
    self.selectBanker = CBT_BANKER + 2
    self.m_tabRuleCheckBox[self.selectBanker]:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")
...
function ClubRoomCreateLayer:updateConfigSetting()
    ...
    self.m_tabRuleCheckBox[self.selectBanker]:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
    self.selectBanker = self.param.tabConfigInfo.cbGameRule[1][4] + CBT_BANKER + 1
    self.m_tabRuleCheckBox[self.selectBanker]:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")
...
function ClubRoomCreateLayer:updateLastData()
    ...
    self.m_tabRuleCheckBox[self.selectBanker]:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
    self.selectBanker = self.lastData[6] + CBT_BANKER + 1
    self.m_tabRuleCheckBox[self.selectBanker]:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")
...
function ClubRoomCreateLayer:sendGroupConfig()
            ...
            buffer:pushword(self.gx_flag and 6 or self.selectPlayer -CBT_PLAYER)    --游戏人数
            ...
            buffer:pushbyte(self.gx_flag and 6 or self.selectPlayer -CBT_PLAYER)

            buffer:pushbyte(6)    --游戏人数

            buffer:pushbyte(self.selectBanker - CBT_BANKER - 1)
            ...
            table.insert(lastData,self.selectBanker - CBT_BANKER - 1)
            ...
            if self.m_isCreate then --保存当前配置
                for i=1, 7 do
                    cc.UserDefault:getInstance():setIntegerForKey(GlobalUserItem.tabAccountInfo.dwUserID.."blackjackData"..i, lastData[i])
                end
            end
...
function ClubRoomCreateLayer:getInviteShareMsg( roomDetailInfo )
    ...
    local shareTxt = "局数:" .. turnCount .. "局。21点游戏精彩刺激, 一起来玩吧!"
...
function ClubRoomCreateLayer:getCopyShareMsg(roomDetailInfo)
    ...
    return {content = string.format("21点, 房号[%06d],您的好友喊你打牌了!", dwRoomID)}
end
...
function ClubRoomCreateLayer:updateXunZhang()
    ...
    self.m_nodeRule:setPositionY(self.gx_flag and 376 or self.m_nodeScore:getPositionY())

    self.m_nodeRule:getChildByName("node_config_2"):setVisible(not self.gx_flag and true or false)

    self.m_nodeRule:getChildByName("node_config_3"):setPositionY(not self.gx_flag and -205 or -85)

    self.m_spBg:getChildByName("ScrollView"):setTouchEnabled(true)
...
function ClubRoomCreateLayer:onRuleSelectedEvent(tag, sender)
    ...
    if tag > CBT_PLAYER  and tag < CBT_BANKER then 
        for k,v in pairs(self.m_tabRuleCheckBox) do
                if k > CBT_PLAYER and k < CBT_BANKER then 
    ...
    if tag > CBT_BANKER   then 
        for k,v in pairs(self.m_tabRuleCheckBox) do
                if k > CBT_BANKER  then
                    v:getChildByName("Sprite_11"):setTexture(k == tag and "room/dt_dian2.png" or "room/dt_dian1.png")
                end
        end
        self.selectBanker = tag
    end
