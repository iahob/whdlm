...
function Bullet:ctor(angle,chairId,score,mutipleIndex,CannonType,cannon)
   ...
   self.m_fishIndex = cmd.INT_MAX --鱼索引
...
function Bullet:followFish(dt)
	...
	if cc.pGetDistance(fishPos,cc.p(self:getPositionX(),self:getPositionY())) <= movedis and not tolua.isnull(self) and self then
...
function Bullet:fallingNet()
	...
	for i=1,bulletNum+1 do
		...
		net:setScale(100/net:getContentSize().width)
	...
	if self.m_isSelf then
		...
		-- 子弹减少
		self._dataModule.curBulletNum = self._dataModule.curBulletNum - 1;
...
function Bullet:sendCathcFish( rect )
	...
	for i=1,#tmp do
		local fish = tmp[i]
		if fish.fishCreateData.nFishState ~= cmd.FishState.FishState_Normal then
	...
	for i=1,#tmp do
		...
		--网中鱼变红
		fish:setColor(cc.c3b(247,172,172));
		-- fish:setOpacity(255*0.44);
		local fishChild = fish:getChildByTag(100)

		if fishChild then
			fishChild:setColor(cc.c3b(247,172,172))
			-- fishChild:setOpacity(255*0.44);
		end

		--恢复颜色
		local delayTime = cc.DelayTime:create(0.5);
		local callFun = cc.CallFunc:create(function()		
			fish:setColor(cc.c3b(255,255,255));
			-- fish:setOpacity(255*1.0);
			
			if fishChild then
				fishChild:setColor(cc.c3b(255,255,255))
				-- fishChild:setOpacity(255*1.0);
			end
   		end);
		local seq = cc.Sequence:create(delayTime,callFun);
		fish:runAction(seq);
	end
	-----------------------------
	--取出前5条鱼
...
--发送消息包
	...
	for i=1,#catchList do
		...
		-- --网中鱼变红
		-- fish:setColor(cc.c3b(255,0,0));
		-- local fishChild = fish:getChildByTag(100)

		-- if fishChild then
		-- 	fishChild:setColor(cc.c3b(255,0,0))
		-- end
  ...
  --  		end);
	...
	local cmddata = CCmd_Data:create(24)
   	cmddata:setcmdinfo(game_cmd.MDM_GF_GAME, cmd.SUB_C_CATCH_FISH);
    ...
    ------------------------------------
    if  yl.INVALID_CHAIR==self:getParent()._scene.m_nChairID then --nil==self:getParent()._scene or
    	return
    end
    -------------------------------------
    --发送失败,发送捕鱼消息
