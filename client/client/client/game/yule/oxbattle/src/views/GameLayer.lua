...
function GameLayer:onEventGameMessage(sub, dataBuffer)
	...
	elseif sub == Game_CMD.SUB_S_CONTINUE_JETTON_FAI then
...
function GameLayer:onReJetton(dataBuffer)
    local cmd_table = ExternalFun.read_netdata(Game_CMD.CMD_S_ContinueJetton, dataBuffer)
    self._gameView:onReJetton(cmd_table)
end

--重复下注失败
function GameLayer:onReJettonFail(dataBuffer)
    local cmd_table = ExternalFun.read_netdata(Game_CMD.CMD_S_ContinuePlaceBetFail, dataBuffer)
...
function GameLayer:onGameMessageCancelBanker(dataBuffer)
    ...
    print(" 从申请列表移除 onGameMessageCancelBanker",cmd_table.wCancelUser)
...
function GameLayer:onAddGameTrumpet(item)
    -- dump(item, "#GameLayer:onAddGameTrumpet")
    self._gameView:onAddGameTrumpet(item)
end
------
