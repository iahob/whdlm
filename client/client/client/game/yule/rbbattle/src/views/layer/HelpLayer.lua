...
function HelpLayer:ctor()
    ...
    --加载csb资源
    local csbNode = ExternalFun.loadCSB("cardtype/CardTypeLayer.csb", self);
...
function HelpLayer:tableCellAtIndex( view, idx )
    ...
    if nil == cell then
        ...
        item = display.newSprite("cardtype/card_type.png");
        item:setPosition(view:getViewSize().width * 0.5, 600)
        item:setName("user_item_view")
