...
function GameViewLayer:initUserInfo( csbNode )
	...
	local head = g_var(PopupInfoHead):createClipHead(self:getMeUserItem(), tmp:getContentSize().width-20, "game/sp_head_bg.png")
	head:setPosition(tmp:getContentSize().width/2,tmp:getContentSize().height/2)--(tmp:getPositionX(),tmp:getPositionY()-2)
	tmp:setGlobalZOrder(100)
	tmp:addChild(head)
...
function GameViewLayer:addLabelAtlasWithParent(parent, str)
	local lab = cc.LabelAtlas:create("","game/num_big.png",18, 27, string.byte("."))
	parent:addChild(lab)
...
function GameViewLayer:initJettonBtnInfo( csbNode )
	local clip_layout = self._csbNode:getChildByName("Node_ChipBtn")
	...
	if self.m_pJettonNumber == nil then
...
dump(self.m_pJettonNumber,"self.m_pJettonNumber")
	...
	for i=1,#self.m_pJettonNumber do
		...
		-- local lab = cc.LabelAtlas:create(tostring(self.m_pJettonNumber[i].k),"game/num_big.png",14, 18, string.byte("."))
		-- btn:addChild(lab)
		-- lab:setString(self.m_pJettonNumber[i].k)
		-- lab:setAnchorPoint(0.5,0.5)
		-- lab:setPosition(btn:getContentSize().width/2,btn:getContentSize().height/2 + 5)
		self:addLabelAtlasWithParent(btn, self.m_pJettonNumber[i].k)
...
function GameViewLayer:adjustJettonBtn(  )
	...
	print("***********可以下注的数额***********",lCanJetton,lCondition)
...
function GameViewLayer:onButtonClickedEvent(tag,ref)
	...
	if tag == TAG_ENUM.BT_EXIT then
		if self.m_lHaveJetton > 0 then
			self._scene.m_cbGameStatus = 100
		else
			self._scene.m_cbGameStatus = 0
		end
...
function GameViewLayer:onGetUserBet( )
	...
	local sp = cc.Sprite:create("game/2_"..nIdx..".png")
	sp:setScale(0.3)
	-- local lab = cc.LabelAtlas:create("","game/num_min.png",8, 10, string.byte("0"))
	-- lab:setAnchorPoint(0.5,0.5)
	-- lab:setString(tostring(self.m_pJettonNumber[nIdx].k))
	-- lab:setPosition(19,17)
	-- lab:addTo(sp)
	self:addLabelAtlasWithParent(sp, self.m_pJettonNumber[nIdx].k)
...
function GameViewLayer:getCancelable( )
	return self._scene.m_cbGameStatus == g_var(cmd).GS_GAME_FREE
end
...
function GameViewLayer:createJettonNode(area)
	...
	--所有下注
	local allStr = string.format("Text_%d",area)
	local m_textAllJetton = Node_myjetton:getChildByName(allStr)
	--m_textAllJetton:setVisible(false)

	--自己下注
	local myStr = string.format("myJetton_%d",area)
	local m_textMyNode = Node_myjetton:getChildByName(myStr)
	-- local m_textMyJetton = m_textMyNode:getChildByName("Text_num")


	jettonNode.m_textAllJetton = m_textAllJetton
...
function GameViewLayer:refreshJettonNode( node, my, total, bMyJetton )	
	...
	str = string.gsub(str, "万", "/")
	-- node.m_textMyJetton:setString(str)
	-- node.m_textMyJetton:getParent():setVisible(node.m_llMyTotal > 0)
	--node.m_textMyJetton:setVisible(node.m_llMyTotal > 0)

	--总数
	str = ExternalFun.formatScoreText(node.m_llAreaTotal)
	node.m_textAllJetton:setString(str);
	node.m_textAllJetton:setVisible(isShow)
...
function GameViewLayer:reSetJettonNode(node)
	...
	-- node.m_textMyJetton:getParent():setVisible(false)
	if node.m_llAreaTotal then
		node.m_textAllJetton:setString("0")
		node.m_textAllJetton:setVisible(false)
	end
...
function GameViewLayer:refreshScore( userinfo )
	local userinfo = userinfo and userinfo or self:getMeUserItem()
	dump(userinfo,"userinfo")
	--携带游戏币
	local str = ExternalFun.numberThousands(userinfo.lScore)
