...
function GameLayer:onEventGameSceneFree( dataBuffer )
    ...
    dump(cmd_table,"onEventGameSceneFree")

    --庄家信息
    yl.m_bDynamicJoin = false

    self.BetScore = cmd_table.dBetScore[1] -- 筹码
    self._gameView.m_pJettonNumber = 
    {
        {k = self.BetScore[1], i = 1},
        {k = self.BetScore[2], i = 2}, 
        {k = self.BetScore[3], i = 3}, 
        {k = self.BetScore[4], i = 4},  
        {k = self.BetScore[5], i = 5},
        {k = self.BetScore[6], i = 6} 
    }
...
function GameLayer:onEventGameSceneEnd( dataBuffer )
    ...
    local cmd_table = ExternalFun.read_netdata(g_var(cmd).CMD_S_StatusPlay, dataBuffer)
    dump(cmd_table,"onEventGameSceneEnd")
    --保存游戏结果
    ...
    local winMultiple = {}
    if cmd_table.cbDiceValue[1][1] == 0 then
        return
    end
