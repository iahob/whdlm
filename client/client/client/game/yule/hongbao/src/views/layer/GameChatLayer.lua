...
function GameChatLayer:ctor(netframe, tabconfig)
    ...
    self.m_textSelect = chat_bg:getChildByName("text_select"):setVisible(true)
    ...
    self.m_emojSelect = chat_bg:getChildByName("emoj_select"):setVisible(false)

    --选中提示
    self.m_spSelect = chat_bg:getChildByName("sp_select"):setVisible(false)
...
function GameChatLayer:onButtonClickedEvent( tag, sender )
    ...
    elseif CBT_TEXT == tag then
        ...
        self.m_spSelect:setPositionX(self.m_checkText:getPositionX())
        self.m_spSelect:setScaleX(1)
        self.m_textSelect:setVisible(true)
        self.m_emojSelect:setVisible(false)
    ...
    elseif CBT_BROW == tag then
        ...
        self.m_emojSelect:setVisible(true)
...
function ChatItem:ctor( nType, tabconfig, callFun )
    if 1 == nType then --聊天
        ...
        --选中效果 
        local framename = tabconfig.szItemSelectFrameName or "chat/public_chat_txtSelect.png"
        if cc.FileUtils:getInstance():isFileExist(framename) then
