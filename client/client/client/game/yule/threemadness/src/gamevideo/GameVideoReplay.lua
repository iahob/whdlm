...
local module_pre = "game.yule.threemadness.src"
local cmd = appdf.req(module_pre .. ".models.CMD_Game")
local GameLogic = appdf.req(module_pre .. ".models.GameLogic")
--local GameResultLayer = appdf.req(module_pre .. ".views.layer.ResultLayer")
local ResultLayer = appdf.req(appdf.GAME_SRC.."yule.threemadness.src.views.layer.GameEndView")
...
function GameVideoReplay:onReadMessage( pBuffer )
    ...
    if curlen < datalen then
        ...
        print("读取消息=========================》", sub,datalen,curlen)
        if 100 == sub then
            -- 发牌消息
            local t = --发送扑克/游戏开始
            {
                {k = "PlayerCount", t = "word"},                            --玩家人数
                {k = "wGamePlayerCountRule", t = "word"},                   --玩家人数配置
                {k = "cbHandCardData", t = "byte", l = {3,3,3,3,3}},     --用户扑克
                {k = "lMaxScore", t = "score"},                             --最大下注
                {k = "lCellScore", t = "score"},                            --底分
                {k = "lCurrentTimes", t = "score"},                         --当前倍数
                {k = "lUserMaxScore", t = "score"},                         --分数上限(游戏币)
            ...
            local playerCount = tmp1.PlayerCount
        ...
        elseif 102 == sub then
            -- 用户放弃
            print("消息包 放弃")
            local cmdtable = ExternalFun.read_netdata(cmd.CMD_S_GiveUp, pBuffer)
            cmdtable.msgsub = 102
            return cmdtable
        elseif 104 == sub then
            -- 游戏结果
        ...
        elseif 106 == sub then
            -- 看牌
            print("消息包 看牌")
            local cmdtable = ExternalFun.read_netdata(cmd.CMD_S_LookCard, pBuffer)
            cmdtable.msgsub = 106
            return cmdtable
        end
    end
...
function GameVideoReplay:onGetVideoStart(gameLayer, msgTable, bForward, bBackward)
    ...
    for k,v in pairs(msgTable.startinfo) do
        ...
        gameLayer._gameView:SetCellText(gameLayer.m_lCellScore)
...
function GameVideoReplay:onGetVideoGiveUp(gameLayer, msgTable, bForward, bBackward)
    ...
    gameLayer._gameView:SetUserOperate(viewid,2)--
...
function GameVideoReplay:onGetVideoGameConclude(gameLayer, msgTable, bForward, bBackward)
    ...
    gameLayer._gameView.m_ChipNode:setVisible(false)
...
function GameVideoReplay:onGetVideoLookCard(gameLayer, msgTable, bForward, bBackward)
    ...
    gameLayer._gameView:SetUserOperate(viewid,1)
...
function GameVideoReplay:cacheUserCards(msg, gameLayer, isForward)
    ...
    for i=1, cmd.GAME_PLAYER do
        ...
        gameLayer._gameView:SetUserOperate(i)
    end
