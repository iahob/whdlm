...
function TrendInfoLayer:freshExchangeRecordShow()
    ...
    local nGameCount = self.m_parent.nGameCount%100+1;
    local nOpenColorCount = self.m_parent.nOpenColorCount;
    local wheelNum = math.floor(self.m_parent.nGameCount/100) + 1;
    ...
    for i = 1, 5 do
        ...
        local openColorCount = nOpenColorCount[index];
        
        local per = 0;
        if 0 ~= self.m_parent.nGameCount then
            per = (openColorCount / self.m_parent.nGameCount) * 100;
        end
        ...
        -- else
            game_num:setString(openColorCount);
