...
--纹理宽高
local CARD_WIDTH = 130--118
local CARD_HEIGHT = 180--164.4
...
function CardSprite:createCard( cbCardData, tagParam )
  ...
  if nil ~= sp and nil ~= tex and sp:initWithTexture(tex, cc.rect(2*sp.m_nCardWidth,4*sp.m_nCardHeight,sp.m_nCardWidth,sp.m_nCardHeight)) then
    ...
    sp.m_fiveTh = cc.Sprite:create("game/card5th.png"):move(100,150):addTo(sp)
    sp.m_fiveTh:setVisible(false)
...
function CardSprite:setFiveCard(visible)

  if self.m_fiveTh then
    self.m_fiveTh:setVisible(visible)
  end
