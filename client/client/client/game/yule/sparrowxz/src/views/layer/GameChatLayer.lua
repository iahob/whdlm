...
function GameChatLayer:ctor(netframe, tabconfig)
    ...
    csbfile = "chat/GameChatLayer.csb"
    ...
    self.m_common_region_szie = self.m_chat_common_region:getContentSize()

    --表情聊天
    self.m_emoj_region = chat_bg:getChildByName("emoj_region")
    self.m_emoj_region:setVisible(false)
    self.m_emoj_region_szie = self.m_emoj_region:getContentSize()
...
function GameChatLayer:loadEmojChat(idx)  
    ...
    --缓存表情
    local dict = cc.FileUtils:getInstance():getValueMapFromFile(string.format("public/emoj/%d.plist", idx))
    local framesDict = dict["frames"]
    if nil ~= framesDict and type(framesDict) == "table" then
...
function ChatItem:ctor( nType, tabconfig, callFun )
    if 1 == nType then --聊天
        local framename = "chat/public_chatLine.png"
        ...
        -- --选中效果 
        -- local framename = "chat/public_chat_txtSelect.png"
        -- if cc.FileUtils:getInstance():isFileExist(framename) then
        --     -- local sp_select = cc.Sprite:create(framename)
        --     -- local frame = sp_select:getSpriteFrame()
        --     if nil == self.m_TextSelect then
        --         self.m_TextSelect = cc.Sprite:create(framename)
        --        -- self.m_TextSelect:setAnchorPoint(cc.p(0.5, 0.5))
        --         print("位置----------》", LEN_TABLEVIEW_WIDTH *0.5 - self.m_TextSelect:getContentSize().width * 0.5)
        --         self.m_TextSelect:setPosition(0, self.m_TextSelect:getContentSize().height * 0.5)
        --         self:addChild(self.m_TextSelect)
        --         self.m_TextSelect:setVisible(false)
        --     end
        -- end
...
function ChatItem:refreshTextItem(str,sendusernick)
    ...
    if nil ~= self.m_TextSelect then
        -- self.m_TextSelect:setPosition(LEN_TABLEVIEW_WIDTH *0.5 - self.m_TextSelect:getContentSize().width * 0.5, self:getContentSize().height * 0.5)
    end
end
...
function ChatItem:refreshEmojItem(idx)
    ...
    for i=1,3 do
        ...
        local filename = string.format("public/emoj/emoj_%d.png", index)
