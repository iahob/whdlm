...
function GameLayer:ctor(frameEngine, scene)
    ...
    -- self.file = io.open(string.format("c:\\sparrowxz.log"), "a")
...
function GameLayer:OnResetGameEngine()
	...
	self.cbPriorChangeCardData = {}   ---服务端默认换的三张牌
...
function GameLayer:onEventUserEnter(tableid,chairid,useritem)
    ...
    if tableid ~= self:GetMeUserItem().wTableID and not GlobalUserItem.bVideo then
	   	for i=1, cmd.GAME_PLAYER do
			...
			if self._gameView.m_UserItem[wViewChairId] ~= nil and self._gameView.m_UserItem[wViewChairId].dwUserID == useritem.dwUserID then
				self._gameView:OnUpdateUserExit(wViewChairId)
				self._gameView:showUserState(wViewChairId, false)
			end
		end
...
-- function GameLayer:getGameRule(dataBuffer)

--    local tabStr={}
--    local str
--    if dataBuffer[4]==1 and dataBuffer[5]==1 then
--    	   str="玩法：换三张、呼叫转移"
--    elseif dataBuffer[4]==1 or dataBuffer[5]==1 then
--    	   str="玩法："..(dataBuffer[4]==1 and "换三张" or "呼叫转移")
--    end
--    if str then table.insert(tabStr,str) end

--    local fanStrs={"自摸加倍","天地胡","海底捞月","门清*2"}
--    str=nil
--    local n=0
--    for i=6,9 do
--    		if dataBuffer[i]==1 then
--    			n=n+1
--    			if n==3 then table.insert(tabStr,str) str=nil end
-- 	   		if nil==str then
-- 	   			str=(n==3 and "          " or "番型：")..fanStrs[i-5]
-- 	   		else
-- 	   			str=str.."、"..fanStrs[i-5]
-- 	   		end
-- 	   	end
--    end
--    if str then table.insert(tabStr,str) end
--    dump(tabStr,"tabStr",3)
--    return tabStr
-- end

function GameLayer:getGameRule(dataBuffer)
	 local strs={"换三张","呼叫转移","自摸加倍","天地胡","海底捞月","门清*2"}
	 local tabStr={}
	 for i=4,9 do
	 	if dataBuffer[i]==1 then
	 		table.insert(tabStr,strs[i-3])
	 	end
	 end
	 return tabStr
end
...
function GameLayer:OnEventGameClockInfo(viewId,time,clockId)
	...
	if (GlobalUserItem.bPrivateRoom and not GlobalUserItem.bPrivateGoldRoom) or self._gameView.bIsVideo then
    	return
    end
    -- body
    ...
    if clockId == cmd.IDI_START_GAME then
		...
		if time <= 3 then
			...
			local count = PriRoom:getInstance().m_tabPriData.dwPlayCount
			local totallCount = PriRoom:getInstance().m_tabPriData.dwDrawCountLimit
			-- print(count,totallCount,GlobalUserItem.dwCurRoomGroupID,"-------------超时开始--------111111111----------")
			if GlobalUserItem.dwCurRoomGroupID ~= 0 and count >0 and count < totallCount then
				-- self._gameView:onButtonClickedEvent(self._gameView.BT_START)
				return
			else
				print("准备超时退出")
    ...
    elseif clockId == cmd.IDI_OPERATE_CARD then
    		if self.bMeOperater==true then
	    		...
	    		if time <= 1 then
	    			...
	    			self.m_bTrustee = true --self:sendUserTrustee()
...
function GameLayer:onEventGameScene(cbGameStatus,dataBuffer)
	print("游戏场景GameLayer:onEventGameScene cbGameStatus",cbGameStatus)
	...
	self._gameView:updateSelfScore()
	self.m_cbGameStatus=cbGameStatus
	local function assignRule(data)
		self.bHuanSanZhang=data.bHuanSanZhang --true为允许换三张
		self.bHuJiaoZhuanYi=data.bHuJiaoZhuanYi --true允许呼叫转移
		self.bZiMoAddTimes=data.bZiMoAddTimes --true允许自摸加倍
		self.bTianDiHu=data.bTianDiHu		--true为允许天地胡加倍
		self.bHaiDiLaoYue=data.bHaiDiLaoYue --true允许海底捞月加倍
		self.bMenQing=data.bMenQing							--true允许门清加倍
	...
	local data,pStatusFree,pStatusChange,pStatusCall,pStatusPlay
	if cbGameStatus==cmd.GS_MJ_FREE then
		 data = ExternalFun.read_netdata(cmd.CMD_S_StatusFree, dataBuffer)
	elseif cbGameStatus==cmd.GS_MJ_CHANGECARD then
		 data = ExternalFun.read_netdata(cmd.CMD_S_StatusChangeCard, dataBuffer)
	elseif cbGameStatus==cmd.GS_MJ_CALL then 
		data = ExternalFun.read_netdata(cmd.CMD_S_StatusCall, dataBuffer)
	elseif cbGameStatus==cmd.GS_MJ_PLAY then
		data = ExternalFun.read_netdata(cmd.CMD_S_StatusPlay, dataBuffer)
	end
	pStatusFree,pStatusChange,pStatusCall,pStatusPlay=data,data,data,data

	GlobalUserItem.bPrivateGoldRoom=data.cbPlayMode==1

	self.lCellScore = pStatusCall.lCellScore
    local lvStr = {"新手场", "初级场", "进阶场", "中级场", "高级场", "精英场", "特级场", "顶级场", "大师场", "牌王场",}
	self._gameView:initGameInfo(data.lCellScore, lvStr[data.szServerName + 1])
	
	local myTableID=self._gameFrame:GetTableID() or "nil"
	 local tabUser={"gamesceneUsers","myTableID:"..myTableID}
	 for i = 1, cmd.GAME_PLAYER do
        ...
        if nil ~= userItem then
            ...
            self._gameView:OnUpdateUser(wViewChairId, userItem)
            table.insert(tabUser,userItem.szNickName)
            if userItem.cbUserStatus == yl.US_READY then
        		self._gameView:showUserState(wViewChairId, true)
        	else
        		self._gameView:showUserState(wViewChairId, false)
	...
	if cbGameStatus==cmd.GS_MJ_FREE then
		...
		self.szServerName = pStatusFree.szServerName
		self.cbPlayMode = pStatusFree.cbPlayMode
		print("pStatusFree.cbPlayMode",pStatusFree.cbPlayMode, self.szServerName)
		 GlobalUserItem.bPrivateGoldRoom = self.cbPlayMode == 1 
	...
	elseif cbGameStatus==cmd.GS_MJ_CHANGECARD then --换三张状态
		...
		self.szServerName = pStatusFree.szServerName
	     self.cbPlayMode = pStatusChange.cbPlayMode
		 GlobalUserItem.bPrivateGoldRoom = self.cbPlayMode == 1 
		...
		--手牌
		dump(pStatusChange.cbCardData,"pStatusChange.cbCardData")
		for i=1,cmd.GAME_PLAYER do
	    ...
	    --self._gameView:updateLeftCard()
	    	print("pStatusChange.cbTimeRemain === ",pStatusChange.cbTimeRemain, pStatusChange.bChangeCard[1][self:GetMeChairID()+1])
	    if not pStatusChange.bChangeCard[1][self:GetMeChairID()+1] then
	    	self._gameView:setShowLabelClock(true)
	    	self._gameView._cardLayer:setPopChangeCard(pStatusChange.cbPriorChangeCardData)
			self.cbPriorChangeCardData = pStatusChange.cbPriorChangeCardData
	    	self._gameView:showChangCardHint(true)
	    	self:SetGameClock(cmd.MY_VIEWID,cmd.IDI_CHANGE_CARD,pStatusChange.cbTimeRemain) --temp 5
	    end
	...
	elseif cbGameStatus==cmd.GS_MJ_CALL then  --选缺状态
		   --showToast(self,"选缺场景",1)
		    ...
		    self.szServerName = pStatusFree.szServerName
			 self.cbPlayMode = pStatusCall.cbPlayMode
		 	GlobalUserItem.bPrivateGoldRoom = self.cbPlayMode == 1 
			...
			print(",pStatusCall.cbTimeRemain=",pStatusCall.cbTimeRemain)
			for i=1,cmd.GAME_PLAYER do
				...
				if self.m_bCallCard[i]==true then
					...
					if viewId and viewId~=cmd.MY_VIEWID then self._gameView:setShowCallingCard(viewId,false) end
				else
					if viewId==cmd.MY_VIEWID and not self.m_bLookOn then
						self._gameView:showChangCardHint(false)
						self._gameView:setShowLabelClock(true)
						self._gameView:showCallCardHint()
						self:SetGameClock(cmd.MY_VIEWID,cmd.IDI_CALL_CARD,pStatusCall.cbTimeRemain) 
					else
	...
	elseif cbGameStatus==cmd.GS_MJ_PLAY then	--游戏状态
		...
		-- end
			...
			print("=======self.outcardUser,self.m_wCurrentUser,self.cbTimeOutCard, self.cbTimeOperateCard",pStatusPlay.cbTimeRemain,self.outcardUser,self.m_wCurrentUser, self.cbTimeOutCard, self.cbTimeOperateCard)
			
			if self.m_wCurrentUser~=yl.INVALID_CHAIR then	
			  --	showToast(self._gameView,"show out clock",1)
			    self._gameView:setShowLabelClock(true)	
				self:SetGameClock(self:SwitchViewChairID(self.m_wCurrentUser), cmd.IDI_OUT_CARD, pStatusPlay.cbTimeRemain)
			elseif pStatusPlay.wOutCardUser~=yl.INVALID_CHAIR then
				--showToast(self._gameView,"show operate clock",1)
				self._gameView:setShowLabelClock(true)	
				self:SetGameClock(self:SwitchViewChairID(pStatusPlay.wOutCardUser), cmd.IDI_OPERATE_CARD, pStatusPlay.cbTimeRemain)
			end
			...
			for i=1,cmd.GAME_PLAYER do
				if huCards[i]~=0 then
					local viewid=self:SwitchViewChairID(i-1)
					self._gameView:setShowHuSign(viewid,true)
...
function GameLayer:onEventGameMessage( wSubCmdID, dataBuffer)
	print("GameLayer:onEventGameMessage游戏消息来了",wSubCmdID)
	
	--showToast(self,"wSubCmdID: "..wSubCmdID,2)
	if wSubCmdID==cmd.SUB_S_ROOMCARD_RECORD then
		return self:OnRCRecord(dataBuffer)
	elseif wSubCmdID==cmd.SUB_S_GAME_START then		--游戏开始
...
function GameLayer:OnRCRecord(dataBuffer)
	local data=ExternalFun.read_netdata(cmd.CMD_S_RoomCardRecord, dataBuffer)
	LogAsset:getInstance():logData(cjson.encode(data))
	local tabScoreList={}
	for i=1,math.min(cmd.MAX_RECORD_COUNT,data.nCount) do
		tabScoreList[i]={}
		for j=1,cmd.GAME_PLAYER do
			tabScoreList[i][self:SwitchViewChairID(j-1)]=data.lDetailScore[j][i]
		end
	end

	for i=1,#tabScoreList do
		for j=1,#tabScoreList[i] do
			print(i,j,tabScoreList[i][j])
		end
	end

	self._gameView:showRoomInfo(tabScoreList,data.nCount)
end
...
function GameLayer:OnSubGameEnd(dataBuffer)
	...
	self._gameView:showGameResult(self.tabHupaiInfo,pGameEnd)
	self.m_bTrustee=false
	self._gameView:setTrustee(false)
	self._gameView:stopAllActions()
	self.m_cbGameStatus = 0
	self._gameView:runAction(cc.Sequence:create(cc.DelayTime:create(30),cc.CallFunc:create(function() 
		self._gameFrame:setEnterAntiCheatRoom(false) 
		if GlobalUserItem.bPrivateGoldRoom and GlobalUserItem.dwCurRoomGroupID == 0 then--and self.m_cbGameStatus > 0 then
            self._gameFrame:onCloseSocket()
            self:dismissPopWait()
            self:stopAllActions()
            self:KillGameClock()
            self._scene:onKeyBack()
            return
        elseif GlobalUserItem.dwCurRoomGroupID ~= 0 and not self.m_bLookOn then
        	-- self._gameView:onButtonClickedEvent(self._gameView.BT_START)
        end
	 end)))
end
...
function GameLayer:OnSubChangeCard(dataBuffer) --所有玩家已互换三张
	...
	-- end
	if self._gameView._cardLayer.bChangeCardStatus then
		self._gameView:showChangCardHint(false)
		self._gameView:removeMyChangeCards()
	end
	self._gameView:runAction(cc.Sequence:create(	
		cc.CallFunc:create(function() self._gameView:setShowChangeTypeSps(true,changeType<=0 and 1 or changeType) end),
...
function GameLayer:OnSubGameStart(dataBuffer)
	...
	--开始发牌
	self._gameView:gameStart(wViewBankerUser, pGameStart.cbCardData[1], cbCardCount, pGameStart.cbUserAction,pGameStart.cbPriorChangeCardData)
	self.cbPriorChangeCardData = pGameStart.cbPriorChangeCardData

	-- 刷新局数
	self._gameView:updatePlayCount()
...
function GameLayer:OnSubCallCard(dataBuffer)
 	...
 	if viewid==cmd.MY_VIEWID then
 		...
 		if not self._gameView._cardLayer.callcardKind then
		print(" ++++++++++++++++++++++++++++++ OnSubCallCard  )",math.ceil(pCallCard.cbCallCard/16))
			self._gameView._cardLayer:setCallcardKind(math.ceil(pCallCard.cbCallCard/16))
...
print("*********用户出牌 self.m_bTrustee",self.m_bTrustee)
...
function GameLayer:OnSubSendCard(dataBuffer)
	...
	if wCurrentViewId==cmd.MY_VIEWID then
		...
		self._gameView:gameSendCard(wCurrentViewId, nil,self.m_bTrustee)
	end
	if wCurrentViewId==cmd.MY_VIEWID then
...
function GameLayer:OnSubOperateNotify(dataBuffer)
	...
	print("*****************OnSubOperateNotify*****************self.cbTimeOperateCard",self.cbTimeOperateCard)
...
function GameLayer:OnSubTrustee(dataBuffer, wDataSize)
	...
	-- if self._gameView.TrustShadow and self._gameView.TrustShadow:isVisible() then
	-- 	return
	-- end
...
function GameLayer:OnSubUserChiHu( dataBuffer, wDataSize )
	...
	self:playCardOperateSound(data.wChiHuUser,GameLogic.WIK_CHI_HU)
...
function GameLayer:queryChangeDesk( )
...
function GameLayer:sendUserTrustee(b)--请求或取消托管
	--showToast(self,"sendUserTrustee",3)
...
function GameLayer:sendAskRCRecord( )
	local cmd_data=CCmd_Data:create()
