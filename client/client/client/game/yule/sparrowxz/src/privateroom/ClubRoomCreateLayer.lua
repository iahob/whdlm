...
function ClubRoomCreateLayer:ctor( scene,param,level )
    ...
    if self.param.tabConfigInfo then
        ...
        for i=1,12 do
            self.lastData[i] = cc.UserDefault:getInstance():getIntegerForKey(GlobalUserItem.tabAccountInfo.dwUserID.."sparrowxzData"..i, -1)
        end
    end
    ...
    self.m_editText = {}
    self.m_editImg = {}
    local epoint = {cc.p(367,168),cc.p(839,168),cc.p(766,413),cc.p(838,79),cc.p(841.7,555)}
    ...
    self.wanfa = {true,false}
    local function wanfaSelect( sender, envntType )
        if envntType == ccui.TouchEventType.ended then
            local tag = sender:getTag()
            local idx = tag - 10
            self.wanfa[idx] = not self.wanfa[idx]
            if self.wanfa[idx]  then
                sender:getChildByName("Sprite_11"):setTexture("room/selectBox1.png")
            else
                sender:getChildByName("Sprite_11"):setTexture("room/selectBox2.png")
            end
        end
    end
    -- 玩法
    for i = 1, 2 do
        ...
        if nil ~= checkbx then
            ...
            if i == 1 then
                checkbx:getChildByName("Sprite_11"):setTexture("room/selectBox1.png")
            end
        end
    end

    --番型
    self.m_nSelectFan = {true,false,false,false}
    local function rulelistener( sender, envntType )
        if envntType == ccui.TouchEventType.ended then
            local tag = sender:getTag()
            local idx = tag - CBT_RULEBEGIN
            self.m_nSelectFan[idx] = not self.m_nSelectFan[idx]
            if self.m_nSelectFan[idx]  then
                sender:getChildByName("Sprite_11"):setTexture("room/selectBox1.png")
            else
                sender:getChildByName("Sprite_11"):setTexture("room/selectBox2.png")
            end
...
function ClubRoomCreateLayer:onUpdateWanfa( )
    for i = 1, 2 do
        local checkbx = self.m_nodeRule:getChildByName("Panel_cell_2"):getChildByName("play" .. i)
        if self.wanfa[i] == true then
            checkbx:getChildByName("Sprite_11"):setTexture("room/selectBox1.png")
        else
            checkbx:getChildByName("Sprite_11"):setTexture("room/selectBox2.png")
        end
...
function ClubRoomCreateLayer:onUpdateFanXing( )
    for i = 1, 4 do
        local checkbx = self.m_nodeRule:getChildByName("Panel_cell_3"):getChildByName("cell" .. i)
        if self.m_nSelectFan[i] == true then
            checkbx:getChildByName("Sprite_11"):setTexture("room/selectBox1.png")
        else
            checkbx:getChildByName("Sprite_11"):setTexture("room/selectBox2.png")
        end
...
function ClubRoomCreateLayer:onRefreshFeeList(isBtn)
    ...
    -- self.m_tabScoreList = {}
    -- for i = 1, 5 do
    --     local checkbx = self.m_nodeRule:getChildByName("Panel_cell_3"):getChildByName("pay" .. i)
    --     if nil ~= checkbx then
    --         checkbx:setVisible(true)
    --         checkbx:setTouchEnabled(true)
    --         checkbx:setTag(CBT_RULEBEGIN + i)
    ...
    --         checkbx:getChildByName("Text_23"):setVisible(false)
...
function ClubRoomCreateLayer:updateConfigSetting()
    ...
    for i = 4, 9 do
        local rule = self.param.tabConfigInfo.cbGameRule[1][i]
        if i <= 5 then
            if rule == 1 then 
                self.wanfa[i - 3] = true
            else
                self.wanfa[i - 3] = false
            end
        else
            if rule == 1 then 
                self.m_nSelectFan[i - 5] = true
            else
                self.m_nSelectFan[i - 5] = false
            end
        end
    end
    self:onUpdateWanfa()
    self:onUpdateFanXing()
...
function ClubRoomCreateLayer:updateLastData()
    ...
    dump(self.m_tabFeeConfigList,"self.m_tabFeeConfigList",self.lastData[2])
    local count = #self.m_tabFeeConfigList
    for i = 1, (count >5 and 5 or count) do
        local config = self.m_tabFeeConfigList[i]
        local checkbx = self.m_nodeRule:getChildByName("Panel_cell_1"):getChildByName("pay" .. i)
    ...
    for i = 3, 4 do
        self.wanfa[i-2] = self.lastData[i] == 1
    end

    for i = 5, 8 do
        self.m_nSelectFan[i-4] = self.lastData[i] == 1
    end
    self:onUpdateWanfa()
    self:onUpdateFanXing()
end
...
function ClubRoomCreateLayer:sendGroupConfig()
            ...
            -- 玩法
            for i = 1, 2 do
                buffer:pushbyte(self.wanfa[i] and 1 or 0)
            end
            -- 番型
            for i = 1, 4 do
                buffer:pushbyte(self.m_nSelectFan[i] and 1 or 0)
            end
            ...
            table.insert(lastData,self.wanfa[1] and 1 or 0)
            table.insert(lastData,self.wanfa[2] and 1 or 0)
            -- 番型
            for i = 1, 4 do
                table.insert(lastData,self.m_nSelectFan[i] and 1 or 0)
            end
            ...
            if self.m_isCreate then --保存当前配置
                for i=1, 12 do
                    cc.UserDefault:getInstance():setIntegerForKey(GlobalUserItem.tabAccountInfo.dwUserID.."sparrowxzData"..i, lastData[i])
                end
            end
...
function ClubRoomCreateLayer:getInviteShareMsg( roomDetailInfo )
    ...
    local shareTxt = "局数:" .. turnCount .. "局。"..baseScore.."血战麻将游戏精彩刺激, 一起来玩吧!"
...
function ClubRoomCreateLayer:updateXunZhang()
    ...
    self.m_nodeRule:setPositionY(self.gx_flag and 426 or 600)

    -- self.m_nodeRule:getChildByName("Panel_cell_3"):setVisible(not self.gx_flag)
end
