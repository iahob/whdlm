...
--设置马牌
function CardSprite:setMaCard(filepath)
   if self.m_maImage then
     return
   end
   self.m_maImage = cc.Sprite:create(filepath)
   self.m_maImage:setAnchorPoint(0.0,1.0)
   self.m_maImage:setPosition(0.0,self:getContentSize().height)
   self.m_maImage:setVisible(false)
   self:addChild(self.m_maImage)
end

function CardSprite:showMaCard( var )
   if not self.m_maImage then
     return
   end

   self.m_maImage:setVisible(var)
...
function CardSprite:createCard( cbCardData, tagParam )
  if tagParam then
    --dump(tagParam, "the cars param", 6)
  end
  ...
  sp.m_strCardFile = tagParam._file or "game_res/game_poker.png"
...
function CardSprite:showSelectEffect(bSelect)
  ...
  if nil == self.m_pMask then
    self.m_pMask = cc.Sprite:create(self.m_strCardFile)
    if nil ~= self.m_pMask then
