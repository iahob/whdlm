...
function GameVideoResultLayer:ctor(scene)
    ...
    print("add searchpath==============", device.writablePath.."game/yule/sparrowxykwx/res/")
   self._searchPath = device.writablePath.."game/yule/sparrowxykwx/res/"
		...
		if i <= #resultList then
			...
			if nil ~= infoNode then
				...
				if resultList[i].dwChiHuRight[1] > 0 and 0 ~= cbHuCard then
					local sprCard = self._cardLayer:createOutOrActiveCardSprite(cmd.MY_VIEWID, cbHuCard, false)
					if nil ~= sprCard then
...
function GameVideoResultLayer:hideLayer()
    ...
    local newPaths = {}
    for k,v in pairs(oldPaths) do
        if tostring(v) ~= tostring(device.writablePath.."game/yule/sparrowxykwx/res/") then
            table.insert(newPaths, v)
        end
    end
    cc.FileUtils:getInstance():setSearchPaths(newPaths)
