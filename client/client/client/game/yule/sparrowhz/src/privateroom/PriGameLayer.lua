...
function PriGameLayer:ctor( gameLayer )
   ...
   -- btn:addTouchEventListener(btncallback)
    ...
    local Sprite_1 = csbNode:getChildByName("Sprite_1")
    Sprite_1:setVisible(false)

    --self.sp_roomHost = csbNode:getChildByName("sp_roomHost"):setVisible(false)
    self._dismissLayer = DismissLayer:create(self):addTo(self._gameLayer._gameView, 88)
end
...
function PriGameLayer:onRefreshInfo()
    ...
    if GlobalUserItem.bPrivateGoldRoom then
        local img = self.m_csbNode:getChildByName("Button_1"):getChildByName("Sprite_2")
        img:setPosition(67, 30)
        img:setTexture("privateRoom/roomInfo.png")
        self.m_atlasCount:setVisible(false)
    end
    --剩余
...
function PriGameLayer:onPriGameEnd( cmd_table,dataBuffer )
    ...
    for i = 1, 4 do
        ...
        csbNode:getChildByName("FileNode_"..i):setVisible(false)

        if userItem  and   (scoreList[i] ~= 0 or self._gameLayer.m_cbPlayStatus[i]==1) then
            curIndex = curIndex + 1
            local nodeUser = csbNode:getChildByName("FileNode_"..curIndex)

            nodeUser:setPositionX(tabPosition[self._gameLayer.chairCount][curIndex])
            --头像
