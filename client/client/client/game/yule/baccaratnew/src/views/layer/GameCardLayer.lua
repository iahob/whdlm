...
function GameCardLayer:ctor(parent)
	...
	self.node_vs = csbNode:getChildByName("node_vs")
	--扑克
	self.m_tabCards = {}	
...
function GameCardLayer:showLayer( var )
	...
	-- if var == false and self.vsSpin then
	-- 	self.vsSpin:removeFromParent()
	-- 	self.vsSpin = nil;
	-- end
end
...
function GameCardLayer:refresh( tabRes, bAni, cbTime )
	...
	if bAni then
		...
		--vs动画
		self.node_vs:runAction(cc.Sequence:create(
			...
			cc.CallFunc:create( function()
				--切换
				if self.vsSpin then
					self.vsSpin:stopAllActions();
			    	self.vsSpin:setAnimation(0, "animation2", true);
				end
		...
		--刷新点数
		self.m_tabCards[kLEFT_ROLE]:updateCardsNode(tabRes.m_idleCards, true, true)
		self.m_tabCards[kLEFT_ROLE]:setScale(0.75)
		self:refreshPoint(kLEFT_ROLE)
		
		self.m_tabCards[kRIGHT_ROLE]:updateCardsNode(tabRes.m_masterCards, true, true)
...
function GameCardLayer:refreshPoint( dir )
	...
	local pos = cc.p(self.m_tabPoint[dir]:getPosition())
	local seq = cc.Sequence:create(sca, call, sca2, cc.CallFunc:create(function () 
																			self.m_tabPoint[dir]:setPosition(cc.p(pos.x+10, pos.y))
																		end))
...
--播放vs动画
function GameCardLayer:PlayGameVSAnim()
	--spin
	if self.vsSpin then
		self.vsSpin:removeFromParent()
		self.vsSpin = nil;
	...
	--播放vs下注声音
	ExternalFun.playSoundEffect("startVS.mp3")
	self.vsSpin = sp.SkeletonAnimation:create("startjetton/skeleton.json","startjetton/skeleton.atlas",1);
    self.vsSpin:setPosition(cc.p(667, 375));
    self.vsSpin:setAnimation(0, "animation", false);    --true是指循环播放walk动作
	-- vsSpin:registerSpineEventHandler(function (event)
 --  	end, sp.EventType.ANIMATION_END)
    self.node_vs:addChild(self.vsSpin);
