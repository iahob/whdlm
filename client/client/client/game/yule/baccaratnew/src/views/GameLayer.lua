...
function GameLayer:sendUserBet( cbArea, lScore )
    self.m_cbGameStatus = g_var(cmd).GAME_PLAY
...
--重复下注
function GameLayer:sendRejectton( )
    print("-------------发送重复下注--------")
    local cmddata = CCmd_Data:create(0)
    self:SendData(g_var(cmd).SUB_C_CONTINUE_JETTON, cmddata)
end
...
function GameLayer:onEventGameSceneFree( dataBuffer )
    ...
    -- 筹码配置
    local ChipArray = cmd_table.ChipArray[1]
    self._gameView.m_pJettonNumber = 
    {
        {k = ChipArray[1], i = 2},
        {k = ChipArray[2], i = 3}, 
        {k = ChipArray[3], i = 4}, 
    ...
    self._gameView:initJettonBtnInfo()
    -- --筹码配置
    -- self._gameView:betServerSet(ChipArray);
...
function GameLayer:onEventGameSceneEnd( dataBuffer )
    ...
    self._gameView:showGameResult(true, cmd_table.cbCardCount[1], cmd_table.cbTableCardArray, cmd_table.cbTimeLeave > 15, true);
...
function GameLayer:onEventGameMessage(sub,dataBuffer)  
    ...
    elseif sub == g_var(cmd).SUB_S_CONTINUE_JETTON then
        --重复下注
        self:onReJetton(dataBuffer);
    elseif sub == g_var(cmd).SUB_S_CONTINUE_JETTON_FAI then
        --重复下注失败
        self:onReJettonFail(dataBuffer);
    elseif sub == g_var(cmd).SUB_S_BANKER_QUIT_RESULT then
        --
        self:onSubBankerQuitResult( dataBuffer )
	else
...
function GameLayer:onSocketInsureEvent( sub,dataBuffer )
    ...
    if sub == g_var(game_cmd).SUB_GR_USER_INSURE_SUCCESS then
        ...
        dump(cmd_table, "onSocketInsureEvent-------->" ,7)
...
function GameLayer:onSubGameEnd( dataBuffer )
    ...
    self._dataModle.m_tabGameEndCmd.lBankerWinScore  = cmd_table.lBankerScore;
    ...
    --结束
    self._gameView:showGameResult(true, cmd_table.cbCardCount[1], cmd_table.cbTableCardArray, cmd_table.cbTimeLeave > 13, false);
end
...
function GameLayer:onSubJettonFail( dataBuffer )
    ...
    local cbFailMask = self.cmd_jettonfail.cbFailMask;
    local szFailDesc = self.cmd_jettonfail.szFailDesc;
    --下注失败提示
    showToast(self, szFailDesc, 2);
    --self._gameView:onGetUserBetFail()
end

--下庄
function GameLayer:onSubBankerQuitResult( dataBuffer )
...
--重复下注
function GameLayer:onReJetton(dataBuffer)
    local cmd_table = ExternalFun.read_netdata(g_var(cmd).CMD_S_ContinuePlaceBet, dataBuffer)
    dump(cmd_table,"---------重复下注-------",6)
    self._gameView:onReJetton(cmd_table)
end

--重复下注失败
function GameLayer:onReJettonFail(dataBuffer)
    local cmd_table = ExternalFun.read_netdata(g_var(cmd).CMD_S_ContinuePlaceBetFail, dataBuffer)
    self._gameView:onReJettonFail(cmd_table)
end
