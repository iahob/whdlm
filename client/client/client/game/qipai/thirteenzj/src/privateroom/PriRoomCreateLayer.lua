...
function PriRoomCreateLayer:ctor( scene,param,level )
    ...
    self.pNode[2] = self.m_spBg:getChildByName("Panel_gold")


            --是否允许陌生人加入
    self.m_nOpenMode = true
    self.m_bIsQuick = false
        --默认是积分房卡
    ...
    local gamemodelistener = function (sender,eventType)
        if this.onGameModeSelectedEvent then
    ...
    self.m_maskDropDown={}
    -- 下拉背景1
    local panel = self.m_spBg:getChildByName("Panel_normal"):getChildByName("Panel_1"):getChildByName("dropdown_panel_1")
    ...
    --     -- 下拉背景2
    -- local panel = self.pNode[2]:getChildByName("dropdown_panel_1")
    -- panel:setTag(MASK_DROPDOWN_1)
    -- panel:addTouchEventListener( btncallback )
    -- panel:setVisible(false)
    -- self.m_maskDropDown[2] = panel
    ...
    self.m_imageDropDownBg2 = self.pNode[1]:getChildByName("dropdown_2")
    --self.m_imageDropDownBg2:setLocalZOrder(6)
    ...
    -- 下拉菜单2
    btn = self.pNode[1]:getChildByName("btn_dropdown_2")
    btn:setTag(MENU_DROPDOWN_2)
    btn:addTouchEventListener(btncallback)
    btn:setEnabled(false)
    self.m_btnDropDown2 = btn
    -- 下拉箭头2
    self.m_btnDropArrow2 = self.pNode[1]:getChildByName("btn_dropdown_arrow_2")

    self.m_imageDropDownBg3 = {}
    self.m_btnDropDown3 = {}
    self.m_btnDropArrow3 = {}
    self.m_txtSelectPlayer = {}

    self.m_imageDropDownBg3[1] = self.pNode[1]:getChildByName("dropdown_3")
    self.m_imageDropDownBg3[1]:setVisible(false)
       
    -- 下拉菜单3
    btn = self.pNode[1]:getChildByName("btn_dropdown_3")
    btn:setTag(MENU_DROPDOWN_3)
    btn:addTouchEventListener(btncallback)
    btn:setEnabled(false)
    self.m_btnDropDown3[1] = btn
    -- 下拉箭头3
    self.m_btnDropArrow3[1] = self.pNode[1]:getChildByName("btn_dropdown_arrow_3")

    -- 选择人数
    self.m_txtSelectPlayer[1] = self.pNode[1]:getChildByName("txt_selectsplayer")
    self.m_txtSelectPlayer[1]:setString("")




    -- 选择底分
    self.m_txtSelectRule = self.pNode[1]:getChildByName("txt_selectscore")
    ...
    for n=1 ,2 do
            if n==2 then
                    self.scrollView = ccui.ScrollView:create()
                        ...
                        self.pNode[1]:retain()
                        self.pNode[1]:removeFromParent()
                        self.pNode[1]:setPosition(cc.p(0,0))
                        self.pNode[1]:setAnchorPoint(cc.p(0,0))
                        self.pNode[1]:addTo(self.scrollView)
                        self.pNode[1]:setName("cellPanel")
                        self.pNode[1]:release()
                        ...
                        self.scrollView:setInnerContainerSize(cc.size(1040, 560))
                        self.scrollView:registerScriptHandler(scrollView1DidScroll)
                        self.scrollView:addTouchEventListener(function (ref, tType)
                            --print("================"..tType)
                             -- if tType ==0 then
            ...
            if n==1 then
                ...
                local playerlistener = function (sender,eventType)
                    ...
                    --for m=1,2 do
                        for k,v in pairs(self.m_playernumlist[n]) do
                            if k == tag then
                                v:setSelected(true)
                            else
                                v:setSelected(false)
                    ...
                    --end
                    self:onButtonClickedEvent(MASK_DROPDOWN_1)
                    self.m_txtSelectPlayer[n]:setString(self.m_nPlayerNum.."人")
                end

                self.m_join = 0
                local checkbox = self.pNode[n]:getChildByName("check_rule_6_0")
                if checkbox ~= nil then
                    checkbox:addEventListener(function ()
                        if self.m_join == 0 then
                            self.m_join = 1
                        else
                            self.m_join = 0
                        end
                        print("self.m_join ",self.m_join)
                    end)
                end

                
                self.m_playernumlist[n] = {}
                for i = 2, 4 do 
                    local checkbx = self.m_imageDropDownBg3[n]:getChildByName("check_cell_" .. i)
                    if nil ~= checkbx then
                        checkbx:setTag(PLAYER_BEGIN + i - 1)
                        checkbx:addEventListener(playerlistener)
                        self.m_playernumlist[n][PLAYER_BEGIN + i - 1] = checkbx
                    end
                end
                self.m_playernumlist[n][PLAYER_BEGIN+3]:setSelected(true)
                self.m_txtSelectPlayer[n]:setString("4人")

            end

            self.m_tableMa[n] = {}
            self.m_nMaSelectIndex = 0
            local function celllistener(ref, tType)
                if tType == ccui.TouchEventType.ended then
                    local tag = ref:getTag()
                    for m=1 ,2 do
                        for k,v in pairs(self.m_tableMa[m]) do
                            if k == tag then
                                v:setSelected(true)
                            else
                                v:setSelected(false)
                    ...
                    self.m_nMaSelectIndex = tag - ( CBT_MA+1)
                end
            end

            for i=1,4 do
                local checkbox =  self.pNode[n]:getChildByName("check_rule_ma_"..i-1)
                if checkbox ~= nil then
                    self.m_tableMa[n][i+CBT_MA] = checkbox
                    checkbox:setTag(i+CBT_MA)
                    checkbox:addTouchEventListener(celllistener)
                end
            end

            self.m_tableMa[n][1+CBT_MA]:setSelected(true)
            ...
            local playlistener = function (sender,eventType)
                    ...
                    local tag = sender:getTag()
                    for m=1 ,2 do
                        for k,v in pairs(self.m_playlist[m]) do
                            if k == tag then
                                v:setSelected(true)
                            else
                                v:setSelected(false)
            ...
            for i=1,2 do
                local checkbox = self.pNode[n]:getChildByName("check_rule_"..i)
                if checkbox ~= nil then
                    self.m_playlist[n][i] = checkbox
                    checkbox:setTag(i)
                    checkbox:addEventListener(playlistener)
                end
            end
            self.m_playlist[n][1]:setSelected(true)
            self._playindex = 0


            self.m_tonghualist[n] = {}
            --玩法选择
            local playlistener = function (sender,eventType)
                   
                    local tag = sender:getTag()
                    for m=1 ,2 do
                        for k,v in pairs(self.m_tonghualist[m]) do
                            if k == tag then
                                v:setSelected(true)
                            else
                                v:setSelected(false)
                    ...
                    self._tonghuaindex = tag-1
            end
            
            for i=1,2 do
                local checkbox = self.pNode[n]:getChildByName("check_rule_"..(i+4))
                if checkbox ~= nil then
                    self.m_tonghualist[n][i] = checkbox
                    checkbox:setTag(i)
                    checkbox:addEventListener(playlistener)
                end
            end
            self.m_tonghualist[n][1]:setSelected(true)
            self._tonghuaindex = 0
            ...
            local gunlistener = function (sender,eventType)
                    ...
                    local tag = sender:getTag()
                    for m=1 ,2 do
                        for k,v in pairs(self.m_gunlist[m]) do
                            if k == tag then
                                v:setSelected(true)
                            else
                                v:setSelected(false)
            ...
            for i=1,2 do
                local checkbox = self.pNode[n]:getChildByName("check_rule_"..(i+2))
                if checkbox ~= nil then
                    self.m_gunlist[n][i] = checkbox
                    checkbox:setTag(i)
                    checkbox:addEventListener(gunlistener)
                end
            end
            self.m_gunlist[n][1]:setSelected(true)
            self._gunnum = 1

            local btn = self.pNode[n]:getChildByName("check_rule_quick")
            btn:setTag(CBT_QUICK)
            btn:addTouchEventListener(btncallback)
    ...
    -- 创建按钮
    self.m_createBtn = self.m_spBg:getChildByName("Panel_normal"):getChildByName("btn_createroom_1")
    ...
    -- 金币房创建
    btn = self.pNode[2]:getChildByName("btn_createroom_1")
...
function PriRoomCreateLayer:onGameModeSelectedEvent( tag, sender )
    ...
    --self.pNode[1]:setPosition(cc.p(0,0))
    self.scrollView:setInnerContainerSize(cc.size(1040, 660))
    self.pNode[2]:setVisible(not self.m_bIsNormal)
end
...
function PriRoomCreateLayer:onRefreshFeeList()
    ...
    dump(scoreList,"===============onRefreshFeeList")
    -- assert(false)
    ...
    for i = 1, nScoreList do 
        ...
        local txtScore = checkbx:getChildByName("txt_cell_1")
    ...
    self.m_nSelectScore[1] = 0
    -- 默认选择底分  
    if nScoreList > 0 then
        self.m_nSelectScoreIdx = CELL_BEGIN + 1
        self.m_tabScoreList[self.m_nSelectScoreIdx]:setSelected(true)
        self.m_nSelectScore[1] = scoreList[1]

        self.m_txtSelectRule:setString(string.format("%d分", self.m_nSelectScore[1]))
    ...
    for i = 1, nConfig do
        ...
        local checkbx = self.m_imageDropDownBg2:getChildByName("check_cell_" .. i)
        if nil ~= checkbx then
            checkbx:setVisible(true)
        ...
        if config.dwDrawCountLimit == 12 then
            index = i
        end
    ...
    if index then
        self.m_nSelectIdx = CBT_BEGIN + index
    end
    ...
    if PriRoom:getInstance().m_bLimitTimeFree then
        ...
        local freeTxt = self.m_spBg:getChildByName("Panel_normal"):getChildByName("txt_feetime")
        freeTxt:setVisible(true)
        freeTxt:setString(szfree)
    ...
    self.m_btnDropDown3[1]:setEnabled(true)
...
function PriRoomCreateLayer:onLoginPriRoomFinish()
    ...
    if ((meUser.cbUserStatus == yl.US_FREE or meUser.cbUserStatus == yl.US_NULL or meUser.cbUserStatus == yl.US_PLAYING or meUser.cbUserStatus == yl.US_SIT)) then
        if PriRoom:getInstance().m_nLoginAction == PriRoom.L_ACTION.ACT_CREATEROOM then
            ...
            buffer:pushbyte(self.m_bIsNormal == true and self.m_nPlayerNum or 4)
            buffer:pushbyte(self._playindex)
            buffer:pushbyte(self._gunnum)
            buffer:pushbyte(self.m_nMaSelectIndex)
            buffer:pushbyte(self._tonghuaindex)
            --dump({self.m_nPlayerNum,self._playindex,self._gunnum,self.m_nMaSelectIndex},"============================11111")
            if self.m_bIsQuick then
                buffer:pushbyte(1)
            else
                buffer:pushbyte(0)
            end
            print("self.m_join",self.m_join)
            buffer:pushbyte(self.m_bIsNormal == true and self.m_join or 1)
...
function PriRoomCreateLayer:getInviteShareMsg( roomDetailInfo )
    ...
    local shareTxt = "局数:" .. turnCount .. "局，十三水游戏精彩刺激, 一起来玩吧!"
...
function PriRoomCreateLayer:getCopyShareMsg(roomDetailInfo)
    ...
    return {content = string.format("十三水, 房号[%06d],您的好友喊你打牌了!", dwRoomID)}
end
...
function PriRoomCreateLayer:onButtonClickedEvent( tag, sender)
    ...
    elseif MASK_DROPDOWN_1 == tag then
        ...
        --self.m_maskDropDown[2]:setVisible(false)
        self.m_imageDropDownBg1:setVisible(false)
        self.m_imageDropDownBg2:setVisible(false)
        self.m_imageDropDownBg3[1]:setVisible(false)
        ...
        self.m_btnDropArrow3[1]:loadTextureNormal("room/btn_down1.png")
    elseif MENU_DROPDOWN_2 == tag then
        self.m_maskDropDown[1]:setVisible(true)
        self.m_imageDropDownBg2:setVisible(true)
        -- 更新箭头
        self.m_btnDropArrow2:loadTextureNormal("room/btn_up1.png")
    
    elseif MENU_DROPDOWN_3 == tag then
        self.m_maskDropDown[1]:setVisible(true)
       -- self.m_maskDropDown[2]:setVisible(true)
        self.m_imageDropDownBg3[1]:setVisible(true)

        -- 更新箭头
        self.m_btnDropArrow3[1]:loadTextureNormal("room/btn_up1.png")
    ...
    elseif CBT_QUICK == tag then
        self.m_bIsQuick = not  self.m_bIsQuick
        sender:setSelected(self.m_bIsQuick  and true or false)
    end
end
...
function PriRoomCreateLayer:onSelectedScoreEvent(tag, sender)
    ...
    self.m_nSelectScore[1] = self.m_scoreList[self.m_nSelectScoreIdx - CELL_BEGIN]
    self.m_txtSelectRule:setString(string.format("%d分", self.m_nSelectScore[1]))
    self:onButtonClickedEvent(MASK_DROPDOWN_1)
end
