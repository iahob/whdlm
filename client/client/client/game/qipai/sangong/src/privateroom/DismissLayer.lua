...
local DismissLayer = class("DismissLayer", function(scene)
	local dismissLayer = cc.CSLoader:createNode(cmd.RES.."privateroom/game/DismissLayer.csb")
	return dismissLayer
end)
...
function DismissLayer:requestDismissRoom(requestUser, remaintime)
	...
	for i = 1, cmd.GAME_PLAYER do
		...
		self:getChildByName("Text_user"..i):setPositionX(self:getChildByName("Text_user"..i):getPositionX() + 40*(i-1))
		self:getChildByName("Text_user"..i.."_action"):setPositionX(self:getChildByName("Text_user"..i.."_action"):getPositionX() + 40*(i-1))
		self:getChildByName("Node_"..i):setPositionX(self:getChildByName("Node_"..i):getPositionX() + 40*(i-1))
	end
	...
	for i = 1, cmd.GAME_PLAYER do
        ...
        if userItem and 1 == self._scene._gameLayer.m_cbPlayStatus[i] then
        	if userItem.dwUserID == requestUser.dwUserID then
                ...
                self:getChildByName("Text_user1_action"):setColor(Color[1])
	        	...
	        	textUserAction:setColor(Color[2])
	        	...
	        	if nUserCount > cmd.GAME_PLAYER then
	        		nUserCount = cmd.GAME_PLAYER
	        	end
        	end
        end
    end
...
function DismissLayer:reStartDismissRoom(requestUserChairId, cbIsDeal, remainTime)
	...
	print("requestUserChairId is ====>",requestUserChairId)
	...
	for i = 1, cmd.GAME_PLAYER do
        ...
        if userItem then
        	 print("userItem.wChairID is =====",userItem.wChairID)
        	if userItem.wChairID == requestUserChairId then 		--请求解散者
        		...
        		textUserAction:setVisible(true)
	        	textUserAction:setColor(Color[1])
...
function DismissLayer:requestReply(replyUser, bAgree)
	...
	local color = (bAgree and Color[3] or Color[4])
