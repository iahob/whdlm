...
function ExternalFun.isContainSpecial( str )

	local sTable = ExternalFun.utf8StringSplit(str)
	for i=1,#sTable do
	        local utfCharLen = string.len(sTable[i])
	        if utfCharLen <= 1 then -- 长度大于1的就认为是中文
	        	--local string = string.gsub(sTable[i], "[%w '|/?·`,;.~!@#$%^&*()-_。，、+]", "")

	           	local ascii = string.byte(sTable[i])
	           	if ascii <48 or (ascii > 57 and ascii < 65 ) or (ascii > 90 and ascii < 96 ) or ascii > 122 then
	           		return true
	           	end
	        end

	        if string.byte(sTable[i]) == 226 then
	        	return true
	        end

	       -- print(utfCharLen.."=="..sTable[i].."=="..string.byte(sTable[i]))
	end
...
--名字省略显示
function ExternalFun.clipText(str,len,needSusp)
	...
	if len < #sTable and needSusp then
		text = text.."..."
	end
...
function ExternalFun.changeTimeText(time)
...
--获取字符个数，包括汉字
function ExternalFun.getWordsNum( str )
	local sTable = ExternalFun.utf8StringSplit(str)
...
function ExternalFun.playSoundEffect( path, useritem )
	...
	if nil == useritem then
		...
		-- 0:女/1:男
		local gender = useritem.cbGender or 0
...
function ExternalFun.keepTwoDecimalPlaces(decimal)

	if type(decimal)=="number" or type(tonumber(decimal))=="number" then
		local temp = tonumber(decimal)
		if math.abs(temp) <0.01 then
			return 0
		end
		if math.mod(temp,1)==0 then
			return temp
		end

		local absValue = math.abs(temp)
		if math.mod(absValue,0.01)>=0.009 then
			
			local str = math.floor(absValue*100) /100  --string.format("%.2f",absValue)

			return temp>0 and tonumber(str) or -(tonumber(str))
		else
			print("<0.009!!")
			local num = absValue - math.mod(absValue,0.01)
			return temp>0 and num or -num
		end
...
--byte数组转int、下标
function ExternalFun.bytesToInt(bytes, startIndex, endIndex)
    local value = 0;
    for i = startIndex, endIndex do
        if (i - startIndex) == 0 then
            value = value + bytes[i];
        else
            value = value + bit:_lshift(bytes[i],(i - startIndex) * 8);
        end
    end
    return value;
end

function ExternalFun.getGameNameByKind(dwKindId)
	local gameName={[3]="德州扑克",[6]="炸金花",[14]="牌九",[25]="五张牌",[27]="四人牛牛",[33]="三公",
[36]="通比牛牛",[49]="十三水",[50]="拼十",[51]="浙江十三水",[53]="欢乐5+2",[55]="拼十",[56]="疯狂三张",[57]="八人牛牛",[59]="4人牌九",
[104]="百人牛牛",[105]="百人红九",[118]="百人骰宝",[122]="30秒",[123]="飞禽走兽",[127]="金鲨银鲨",[140]="豪车俱乐部",[143]="百人二八杠",
[147]="五星宏辉",[148]="红包扫雷",[200]="斗地主",[240]="跑得快",[241]="21点",[302]="血战麻将",[386]="转转麻将",[389]="红中麻将",[391]="广东麻将",
[399]="本溪麻将",[401]="五子棋",[503]="连环夺宝",[507]="人鱼奇缘",[508]="三九连线",[510]="李逵劈鱼",[511]="摇钱树",[516]="水浒传",[522]="超级水果连线",
[529]="鱼虾蟹",[532]="ATT连环炮",[700]="乾安麻将",[703]="大庆麻将",[704]="哈尔滨麻将",[707]="卡五星",[710]="韶关麻将",[145]="龙虎斗",[61]="潮汕三公",[239]="东北510K"}	
	return gameName[dwKindId] or ""
end
