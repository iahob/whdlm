...
function Bridge_ios.getBatteryLevel()
    ...
    local ok,ret =  luaoc.callStaticMethod(BRIDGE_CLASS,"getBatteryLevel")
    if not ok then
        print("luaoc error:" .. ret)
        return 100
    else
...
--getWIFIStatus 3强 2一般 1无信号
function Bridge_ios.getNetworkStrength() 
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"getWIFIStatus")
    if not ok then
        return 3
    else
        return ret
...
--getNetworkType -1,0无网络  1 数据网络 2 WiFi
function Bridge_ios.getNetworkType() 
    local ok,ret = luaoc.callStaticMethod(BRIDGE_CLASS,"getNetType")
    if not ok then
        return 2
    else
        return ret
