...
function Bridge_android.getBatteryLevel()
    local sigs = "()I"    
    local ok,ret = luaj.callStaticMethod(BRIDGE_CLASS,"getBatteryLevel",{},sigs)
    if not ok then
        return 100
    else
        return ret
...
--getNetworkStrength 3强 2一般 1弱
function Bridge_android.getNetworkStrength()
    local sigs = "()I"    
    local ok,ret = luaj.callStaticMethod(BRIDGE_CLASS,"getWIFIStatus",{},sigs)
    if not ok then
        return 3
    else
        return ret
...
function Bridge_android.getNetworkType() --  0无网  1数据网络  2WiFi
    local sigs = "()I"    
    local ok,ret = luaj.callStaticMethod(BRIDGE_CLASS,"getNetType",{},sigs)
    if not ok then
        return 2
    else
        return ret
