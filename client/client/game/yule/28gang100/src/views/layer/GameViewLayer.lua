...
local Game_CMD = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.models.CMD_Game")
local GameLogic = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.models.GameLogic")
local publicFunc = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.models.publicFunc")
local publicFunc = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.models.publicFunc")
local BankLayer = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.views.layer.BankLayer")
local TrendList = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.views.layer.TrendList")
local HelpLayer = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.views.layer.HelpLayer")
local CardSprite = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.views.layer.CardSprite")
local BankerList = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.views.layer.BankerList")
local GameEndLayer = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.views.layer.GameEndLayer")
local SettingLayer = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.views.layer.SettingLayer")
local UserListLayer = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.views.layer.UserListLayer")
...
--下注数值
GameViewLayer.m_BTJettonScore = {100, 1000, 5000, 10000, 50000}

--下注值对应游戏币个数(此处只可为偶数)
GameViewLayer.m_JettonGoldBaseNum = {2, 2, 4, 4, 6}
...
function GameViewLayer:setBankInfo()
    ...
    if self.banker_gold  == -1 then
        bankBg:getChildByName("Atla_gold"):setString("")
    end
end
...
function GameViewLayer:onGameSceneFree( cmd_table )
    dump(cmd_table,"GameViewLayer:onGameSceneFree",12)
    self:resetGameData()
    self.gameState = Game_CMD.GS_SCENE_FREE
    ...
    self:setBTJettonScore(cmd_table.dBet[1])
     --系统坐庄
    if self.m_bEnableSysBanker then
...
function GameViewLayer:onGameScenePlaying( cmd_table, cbGameStatus )
    dump(cmd_table,"GameViewLayer:onGameScenePlaying",12)
    self:resetGameData()
...
function GameViewLayer:onGameFree( cmd_table )
    ...
    if self.bankerLayer and self:isMeChair(self.wBankerUser) then
...
function GameViewLayer:onGetBankInfo(bankinfo)
    ...
    if self.bankerLayer then
        self.bankerLayer:refreshBankRate(bankinfo.wRevenueTake, bankinfo.lBankOPButtonArray[1])
    end
end

function GameViewLayer:setBTJettonScore(tab)
    -- body
    GameViewLayer.m_BTJettonScore = tab
        --设置下注数值
    for i=1, 5 do
        self.csbNode:getChildByName("Node_jetton"):getChildByName(string.format("AtlasLabel_%d", i)):setString(GameViewLayer.m_BTJettonScore[i])
    end
...
function GameViewLayer:canUserBank()
    --空闲状态才能存款
...
print("self.gameState=============================>",self.gameState)
    if (self.wBankerUser == self:getParent():GetMeChairID() and self.gameState ~= Game_CMD.GS_SCENE_FREE) then
