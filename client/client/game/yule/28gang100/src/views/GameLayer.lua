...
local GameViewLayer = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.views.layer.GameViewLayer")
local GameFrame = appdf.req(appdf.GAME_SRC.."yule.28gang100.src.models.GameFrame")
...
function GameLayer:onEventGameMessage(sub, dataBuffer)
	if sub == Game_CMD.SUB_S_GAME_FREE then              --99
		--游戏空闲
		self.m_cbGameStatus = Game_CMD.GS_SCENE_FREE
	...
	elseif sub == Game_CMD.SUB_S_GAME_START then         --100
		...
		if not self._gameView:isMeIsBanker() and self._gameView.myAllJettonScore == 0 then
			...
			self.m_cbGameStatus = Game_CMD.GS_PLACE_JETTON
		end
	...
	elseif sub == Game_CMD.SUB_S_GAME_END then              --102
		...
		if self._gameView:isMeIsBanker() then 
			...
			self.m_cbGameStatus = Game_CMD.GS_GAME_END
		end
...
function GameLayer:sendApplyBanker()
	if self.lApplyBankerCondition > self:GetMeUserItem().lScore then 
		return showToast(self, "您的金币不足！", 2)
	end
