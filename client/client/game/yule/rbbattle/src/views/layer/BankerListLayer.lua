...
function BankerListLayer:ctor()
    ...
    --加载csb资源
    self._csbNode = ExternalFun.loadCSB("bankerlist/BankerListLayer.csb", self)
    ...
    --
    local ranker_limit = self._csbNode:getChildByName("ranker_limit");
...
function BankerListLayer:freshApplyBankerLimit(lApplyBankerCondition)
    ...
    local ranker_limit = self._csbNode:getChildByName("ranker_limit");
    ranker_limit:setString("携带金币数大于" .. roleGoldNum .. "才可申请上庄。");
end
...
function BankerListLayer:tableCellAtIndex(view, idx)
    ...
    if roleItem then
        ...
        local role_rank = role_item:getChildByName("role_rank");
        role_rank:setString(index);

        local role_icon = role_item:getChildByName("role_icon");
        -- 创建头像
        ...
        local role_name = role_item:getChildByName("role_name");
        ...
        local gold_num = role_item:getChildByName("gold_num");
