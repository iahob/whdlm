...
function CardLayer:setCallcardKindList(tabCallcardKind) --video
	self.tabCallcardKind=tabCallcardKind or {}
end
...
function CardLayer:onInitData()
	...
	self.tabCallcardKind={} --for video
	self.changeCards = {}
end
...
function CardLayer:setPopChangeCard(cardData)
	dump(cardData)
	dump(self.cbCardData[cmd.MY_VIEWID])
	local s={}
	self.changeCards = {}
    for index,data in ipairs(self.cbCardData[cmd.MY_VIEWID]) do
    	if cardData[1][1] == data and s[index] ~= true then
    		s[index] = true
    		table.insert(self.changeCards, index)
    		break
    	end
    end
    for index,data in ipairs(self.cbCardData[cmd.MY_VIEWID]) do
    	if cardData[1][2] == data and s[index] ~= true then
    		s[index] = true
    		table.insert(self.changeCards, index)
    		break
    	end
    ...
    for index,data in ipairs(self.cbCardData[cmd.MY_VIEWID]) do
    	if cardData[1][3] == data and s[index] ~= true then
    		s[index] = true
    		table.insert(self.changeCards, index)
    		break
    	end
    end
    dump(self.changeCards)
	dump(s)
end
...
function CardLayer:setMySendcardOffset(sendcard)
	...
	local t=self.cbCardData[cmd.MY_VIEWID]
	if sendcard and not self._scene._scene.m_bLookOn then assert(t[#t]==sendcard) end
	if #t>1 then
...
function CardLayer:exchangeCard(receivedCards,viewid)          --receivedCards为牌值
	viewid=viewid or cmd.MY_VIEWID
	for i=1,#receivedCards do
		print("receivedcard"..i,receivedCards[i])
	end
	self:createHandCard(viewid,receivedCards,#receivedCards)
...
function CardLayer:createHandCard(viewid, cbCardData, cbCardCount)
	...
	for i=1,cbCardCount do
		...
		if nil ~= cbCardData then
			print("viewid "..viewid,"i:",i)
			card = self:createStandCardSprite(viewid, cbCardData[i])
		else
			card = self:createStandCardSprite(viewid, nil)
		end

		if (cmd.MY_VIEWID==viewid and not self._scene._scene.m_bLookOn  and card~=nil and math.ceil(cbCardData[i]/16)==self.callcardKind)
		or (self._scene.bIsVideo and card~=nil and math.ceil(cbCardData[i]/16)==self.tabCallcardKind[viewid] ) --added for video
			then
			card.bGray=true
...
function CardLayer:sortHandCard(viewid,sendcard) --sendcard最右有bug
	...
	if viewid == cmd.MY_VIEWID or self._scene.bIsVideo then 
		local callcardKind=self.callcardKind
		if self._scene.bIsVideo then
			callcardKind=self.tabCallcardKind[viewid]
		end

		if callcardKind==nil then
			table.sort(self.cbCardData[viewid], function(a, b) return a < b end)
		else
			table.sort(self.cbCardData[viewid], function(a, b) 
				local kindA=math.ceil(a/16)
				local kindB=math.ceil(b/16)
				if kindA==callcardKind and kindB~=callcardKind then
					return false
				elseif kindA~=callcardKind and kindB==callcardKind then 
					return true
				else
					return a < b 
				end
		...
		local index=-1
		local tabC=self.cbCardData[viewid]
		for i=1,#tabC do
		...
		for i=1, self.cbCardCount[viewid] do
			...
			if nil ~=  card then
				...
				if card and nil ~= cardValue then
		...
		if self.cbCardCount[viewid]==14 and (viewid==cmd.MY_VIEWID or self._scene.bIsVideo) then
			...
			if lastcard:getPositionX()-x<lastcard:getContentSize().width+10 then
				lastcard:setPositionX(lastcard:getPositionX()+pramHandCard[viewid][3]/2)
			end
		end
		for i=1,#self.cbCardData[viewid] do
			if (viewid==cmd.MY_VIEWID and math.ceil(self.cbCardData[viewid][i]/16)==self.callcardKind )
				or (self._scene.bIsVideo and math.ceil(self.cbCardData[viewid][i]/16)==self.tabCallcardKind[viewid]) --added for video
				then
				local card=self.nodeHandCard[viewid]:getChildByTag(i)
				if card and card.bGray~=true then
					convertToGraySprite(card:getChildByName("card_value"))
					if not self._scene._scene.m_bLookOn then
						convertToGraySprite(card:getChildByName("card_bg"))
					end
...
function CardLayer:createActiveCard(viewid, tagAvtiveCard, wProvideViewId) --传入tagAvtiveCard
	...
	-- -- 如果是碰最后一张偏移表示预出牌
	-- if GameLogic.SHOW_PENG == tagAvtiveCard.cbType then
	-- 	local card = self.nodeHandCard[viewid]:getChildByTag(self.cbCardCount[viewid])
	-- 	card:setPosition((self.cbCardCount[viewid]-1) *pramHandCard[viewid][3] + pramHandCard[viewid][3]/2 , self.cbCardCount[viewid] * pramHandCard[viewid][4] + pramHandCard[viewid][4]/2 )
	-- end
...
function CardLayer:setCardMarkWithValue(cardData)
	-- print("########################setCardMarkWithValue#########################cardData = ",cardData)
	-- dump(self.cbActiveCardData,"self.cbActiveCardData",6)
	-- 先隐藏以显示的
	self:removeCardMark()
		...
		if #activelist > 0 then
			for k, tab in ipairs(activelist) do
				if tab.cbCardValue[1] == cardData then
					...
					-- for i, sp in ipairs(self.nodeActiveCard[i]:getChildren()) do
					-- 	print("*********************sp  tag = ", sp:getTag())
					-- end
				end
			end
		end
	end
	-- dump(self.showCards,"self.showCards",3)
	-- print("########################setCardMarkWithValue#########################")
...
function CardLayer:sendCardToPlayer(viewid, cbCardData, cbCardCount,bTrustee)
	...
	--assert(#self.cbCardData[viewid]<14,#self.cbCardData[viewid])
	-- if #self.cbCardData[viewid]>=14 then
	-- 	showToast(self._scene,"sendCardToPlayer #self.cbCardData[viewid]>=14",3)
	-- end
	local heap = nil
	...
	function callbackWithArgs(viewid, cbCardData, cbCardCount)
          local ret = function ()
          print("callbackWithArgs createHandCard",viewid, cbCardCount)
...
function CardLayer:outCard(viewid, cbCardInfo) --viewid为自己，则cbCardInfo为索引；否则cbCardInfo为牌值
	...
	-- print("function CardLayer:outCard(viewid, cbCardInfo)")
	-- print("function CardLayer:outCard(viewid, cbCardInfo)")
	if viewid==cmd.MY_VIEWID and not self.canOutCard then
		return
	end
	...
	function callbackWithArgs(viewid, cbCardInfo, index)
		local ret = function ()
			...
			print("function CardLayer:outCard callbackWithArgs")
			--创建麻将，非回放：自己出牌传索引，别人出牌算牌值 回放传牌值
			--如果是自己出牌，跟据当前选中的卡牌索引
			local cbCardData = {}
			if cmd.MY_VIEWID == viewid and (not self._scene.bIsVideo) then
	...
	elseif cmd.MY_VIEWID == viewid then
		index = cbCardInfo
	else
    ...
    -- callbackWithArgs(viewid, cbCardInfo, index)() --temp delete this line and uncomment codes below

    local callFun = cc.CallFunc:create(callbackWithArgs(viewid, cbCardInfo, index))
    --assert(nil ~= card )
    if card==nil then
    	print(string.format("玩家%d找不到牌%d",viewid, cbCardInfo))
    	--showToast(self._scene,string.format("玩家%d找不到牌%d",viewid, cbCardInfo),10)
    end
    if nil ~= card then
...
function CardLayer:outCardTrustee(cbCardInfo) --托管时同一张牌的SUB_S_OUT_CARD可能比cmd.SUB_S_SEND_CARD先到导致index为0吗
	print("托管出牌动画===cbCardInfo",cbCardInfo)
...
function CardLayer:removeHandCard(viewId, index)
	...
	local childN=self.nodeHandCard[viewId]:getChildrenCount()
	--assert(sprCard)
	if nil ~= sprCard or #self.cbCardData[viewId]>childN then
		sprCard:removeFromParent()
		if viewId~=cmd.MY_VIEWID or self.cbCardData[viewId][index] then self.cbCardCount[viewId] = self.cbCardCount[viewId] -1 end
...
function CardLayer:removeHandCards(viewId, tabCard)
	...
	print("viewId:",viewId)
	print("videogamelayercards:",table.concat(self._scene._scene.cbHandCardData[viewId],","))
	assert(#t==10 or #t==11)
end
...
function CardLayer:tileHuMajong(provideViewID,huViewID,card)
	if provideViewID==huViewID then
		...
		table.remove(self.showCards[provideViewID]) -- 移除供牌玩家最后记录的出牌
	...
	if self.isMoving then
		...
		if pos.y > pramHandCard[cmd.MY_VIEWID][2] then --移动生效，出牌
			...
			self.cbHuCardLeft = {}
				...
				if self.nCurrentTouchCardTag == #self.cbCardData[cmd.MY_VIEWID] then
					posx = (self.nCurrentTouchCardTag - 0.5 ) * pramHandCard[cmd.MY_VIEWID][3]
				end
				if nil ~=  card then
...
function CardLayer:getActiveCardData()
	return self.cbActiveCardData
end
return CardLayer