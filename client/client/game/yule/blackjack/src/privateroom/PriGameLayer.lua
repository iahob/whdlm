...
local DismissLayer = appdf.req(appdf.GAME_SRC.."yule.blackjack.src.privateroom.DismissLayer")
...
function PriGameLayer:ctor( gameLayer )
    ...
    -- 房间ID
    self.m_atlasRoomID = csbNode:getChildByName("Text_roomId")
    self.m_atlasRoomID:setString("000000")

    -- 局数
    self.m_atlasCount1 = csbNode:getChildByName("Text_round_1")
    self.m_atlasCount1:setString("")

    self.m_atlasCount2 = csbNode:getChildByName("Text_round_2")
    self.m_atlasCount2:setString("")
                    ...
                    if nil ~= scene.popTargetShare then
                        scene:popTargetShare(function(target)
                            if nil ~= target then
                                ...
                                local shareTxt = "局数:" .. drawCount .. "局。"..strPlayerCount.."21点精彩刺激, 一起来玩吧!"
                                local url = yl.PRIVATE_SHARE_URL .. "?g=" .. GlobalUserItem.tabAccountInfo.dwGameID .. "&r=" .. szRoomID .. "&k=241&a=1" .. "&p=" .. passwd
...
function PriGameLayer:onRefreshInfo()
    ...
    self.m_atlasCount1:setString("局数      / "..PriRoom:getInstance().m_tabPriData.dwDrawCountLimit)
    self.m_atlasCount2:setString(PriRoom:getInstance().m_tabPriData.dwPlayCount)
...
function PriGameLayer:onPriGameEnd( cmd_table )
    ...
    --规则
    local rule = image_bg:getChildByName("text_rule")
    if nil ~= rule then
        rule:setString(self._gameLayer:getShareText())
    end
    --时间
    local labTime = image_bg:getChildByName("text_time")
    if nil ~= labTime and not GlobalUserItem.bVideo then
        labTime:setString(cmd_table.sysStartTime.wYear.."-"..cmd_table.sysStartTime.wMonth.."-"..cmd_table.sysStartTime.wDay.."("
    ...
    for i = 1, chairCount do
            ...
            if nil ~= useritem and (scoreList[i] ~= 0 or self._gameLayer.m_cbPlayStatus[i]==1) then
                cutIndex = cutIndex+1
                local cellbg = image_bg:getChildByName("Node_info" .. cutIndex)
                ...
                nickText:setString(useritem.szNickName)

                local idText = cellbg:getChildByName("Text_id") --CSB上字串
                idText:setString("ID:"..useritem.dwGameID)
                

                local score = scoreList[useritem.wChairID + 1] or 0
                -- 成绩
                local strscore = score .. ""
                local cpscore = cellbg:getChildByName("AtlasLabel_score")
                if score >= 0 then
                    strscore = "/" .. score
                    cpscore:setProperty(strscore, cmd.RES.."info/num_win.png", 20, 26, "/")
                else
                    strscore = "/" .. math.abs(score)--num_lose
                    cpscore:setProperty(strscore, cmd.RES.."info/num_lose.png", 20, 26, "/")
                end

                
                local sp_win = cellbg:getChildByName("Node_win")
                if maxuser==i then
                    sp_win:setVisible(true)
                else
