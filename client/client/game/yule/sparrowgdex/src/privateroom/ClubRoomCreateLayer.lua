...
function ClubRoomCreateLayer:ctor( scene,param,level )
    ...
    if self.param.tabConfigInfo then
        ...
        for i=1,11 do
            self.lastData[i] = cc.UserDefault:getInstance():getIntegerForKey(GlobalUserItem.tabAccountInfo.dwUserID.."sparrowgdexData"..i, -1)
        end
    end
    ...
    local epoint = {cc.p(360,67),cc.p(862,67),cc.p(1000,99),cc.p(868,-21),cc.p(872,572)} 
    ...
    for i = 1, 4 do
        ...
        if nil ~= checkbx then
            checkbx:setVisible(true)
            checkbx:setTouchEnabled(true)
            checkbx:setTag(CBT_GUI + i)
            checkbx:addTouchEventListener(cbtlistener)
            checkbx:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
            self.m_tabRuleCheckBox[CBT_GUI + i] = checkbx
        end
    end    
    self.selectGui = CBT_GUI + 1
    -- self.m_tabRuleCheckBox[self.m_nSelectDiZhuIdx]:setSelected(true)
    self.m_tabRuleCheckBox[self.selectGui]:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")

    self.m_noMagicDoubleText = nil
    --规则配置
    for i = 1, 4 do
        ...
        if nil ~= checkbx then
            checkbx:setVisible(true)
            checkbx:setTouchEnabled(true)
            checkbx:setTag(CBT_GUIZE + i)
            checkbx:addTouchEventListener(cbtlistener)
            checkbx:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
            self.m_tabRuleCheckBox[CBT_GUIZE + i] = checkbx
            if i==2 then
                self.m_noMagicDoubleText = checkbx:getChildByName("Text_22")
                self.m_noMagicDoubleText:setColor(cc.c3b(127,127,127))
            end
        end
    end    
    
    self.selectConfig = {0, 0, 1, 0}--默认勾选可抢杠胡
    self.m_tabRuleCheckBox[CBT_GUIZE+3]:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")
...
function ClubRoomCreateLayer:updateConfigSetting()
    ...
    self.selectMa = self.param.tabConfigInfo.cbGameRule[1][4]/2 + 1 + CBT_MA
    self.m_tabRuleCheckBox[self.selectMa]:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")


    self.m_tabRuleCheckBox[self.selectGui]:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
    self.selectGui = self.param.tabConfigInfo.cbGameRule[1][5] +1 + CBT_GUI
    self.m_tabRuleCheckBox[self.selectGui]:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")

    self.selectConfig[1] = self.param.tabConfigInfo.cbGameRule[1][6] == 0 and 1 or 0
    self.m_tabRuleCheckBox[CBT_GUIZE+1]:getChildByName("Sprite_11"):setTexture("room/dt_dian"..(self.selectConfig[1] == 1 and 2 or 1)..".png")


    for i=2,4 do
            self.selectConfig[i] = self.param.tabConfigInfo.cbGameRule[1][5+i]
            self.m_tabRuleCheckBox[CBT_GUIZE+i]:getChildByName("Sprite_11"):setTexture("room/dt_dian"..(self.selectConfig[i] == 1 and 2 or 1)..".png")
    end

    if self.selectGui-CBT_GUI==1 then
        self.m_tabRuleCheckBox[CBT_GUIZE+2]:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
        self.m_tabRuleCheckBox[CBT_GUIZE+2]:setEnabled(false)
        self.m_noMagicDoubleText:setColor(cc.c3b(127,127,127))
        self.selectConfig[2]=0
    else
        self.m_tabRuleCheckBox[CBT_GUIZE+2]:setEnabled(true)
        self.m_noMagicDoubleText:setColor(cc.c3b(255,255,255))
    end
...
function ClubRoomCreateLayer:updateLastData()
    ...
    self.m_tabRuleCheckBox[self.selectMa]:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
    self.selectMa = self.lastData[6]/2 + 1 + CBT_MA
    self.m_tabRuleCheckBox[self.selectMa]:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")


    self.m_tabRuleCheckBox[self.selectGui]:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
    self.selectGui = self.lastData[7] +1 + CBT_GUI
    self.m_tabRuleCheckBox[self.selectGui]:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")

    self.selectConfig[1] = self.lastData[8] == 0 and 1 or 0
    self.m_tabRuleCheckBox[CBT_GUIZE+1]:getChildByName("Sprite_11"):setTexture("room/dt_dian"..(self.selectConfig[1] == 1 and 2 or 1)..".png")


    for i=2,4 do
            self.selectConfig[i] = self.lastData[7+i]
            self.m_tabRuleCheckBox[CBT_GUIZE+i]:getChildByName("Sprite_11"):setTexture("room/dt_dian"..(self.selectConfig[i] == 1 and 2 or 1)..".png")
    end
...
function ClubRoomCreateLayer:sendGroupConfig()
            ...
            buffer:pushbyte(maTable[self.selectMa -CBT_MA])


            local magicTable = {0 , 1, 2, 3}
            buffer:pushbyte(magicTable[self.selectGui -CBT_GUI])

            --玩法配置
            if self.selectConfig[1] ~= 0  then
                buffer:pushbyte(0)
                print("创建房卡房间，没有字牌")
            else
                buffer:pushbyte(1)
            ...
            table.insert(lastData,maTable[self.selectMa -CBT_MA])

            table.insert(lastData,magicTable[self.selectGui -CBT_GUI])

            table.insert(lastData,self.selectConfig[1] == 1 and 0 or 1)
            for i=2,4 do
                table.insert(lastData,self.selectConfig[i])
            end

            if self.m_isCreate then --保存当前配置
                for i=1, 11 do
                    cc.UserDefault:getInstance():setIntegerForKey(GlobalUserItem.tabAccountInfo.dwUserID.."sparrowgdexData"..i, lastData[i])
                end
            end
...
function ClubRoomCreateLayer:getInviteShareMsg( roomDetailInfo )
    ...
    local shareTxt = "局数:" .. turnCount .. "局。广东麻将游戏精彩刺激, 一起来玩吧!"
...
function ClubRoomCreateLayer:updateXunZhang()
    ...
    self.m_nodeRule:setPositionY(self.gx_flag and 429 or self.m_nodeScore:getPositionY())
...
--self.selectConfig
    ...
    if tag > CBT_MA and tag < CBT_GUI then --马牌配置
        for k,v in pairs(self.m_tabRuleCheckBox) do
            if k ~= tag and k > CBT_MA and k < CBT_GUI then
    ...
    if tag > CBT_GUI and tag < CBT_GUIZE then --鬼牌配置
        for k,v in pairs(self.m_tabRuleCheckBox) do
            if k ~= tag and k > CBT_GUI and k < CBT_GUIZE then
                v:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
            end
            if k == tag then
                v:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")
                if tag-CBT_GUI==1 then
                    self.m_tabRuleCheckBox[CBT_GUIZE+2]:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
                    self.m_tabRuleCheckBox[CBT_GUIZE+2]:setEnabled(false)
                    self.m_noMagicDoubleText:setColor(cc.c3b(127,127,127))
                    self.selectConfig[2]=0
    ...
    if tag > CBT_GUIZE then
        for k,v in pairs(self.m_tabRuleCheckBox) do
            if tag == k then
                if self.selectConfig[tag-CBT_GUIZE]==0 then
                    v:getChildByName("Sprite_11"):setTexture("room/dt_dian2.png")
                    self.selectConfig[tag-CBT_GUIZE]=1
                else
                    v:getChildByName("Sprite_11"):setTexture("room/dt_dian1.png")
                    self.selectConfig[tag-CBT_GUIZE]=0
                end
            end
        end
    end
