...
local UserItem = appdf.req(appdf.GAME_SRC.."yule.oxbattle.src.views.layer.UserItemApply")
...
function ApplyListLayer:ctor( viewParent)
	...
	self.condition = self.csbNode:getChildByName("applycondition"):getChildByName("Text_content")
	self.condition:setString("大于"..ExternalFun.formatScoreText(self.m_parent.m_lApplyBankerCondition).."才可以上庄")
...
function ApplyListLayer:refreshList( userlist )
    ...
    if state == self.m_parent._apply_state.kCancelState then
        str1 = "ForBanker/bt_apply_0_0.png"
        str2 = "ForBanker/bt_apply_0_0.png"
        str3 = "ForBanker/bt_apply_0_0.png"

        --申请条件限制
        ExternalFun.enableBtn(self.m_btnApply, self.m_parent:getApplyable())
    elseif state == self.m_parent._apply_state.kApplyState then
        str1 = "ForBanker/bt_cancel_banker_0.png"
        str2 = "ForBanker/bt_cancel_banker_0.png"
        str3 = "ForBanker/bt_cancel_banker_0.png"
    ...
    if nil ~= str1 and nil ~= str2 then
        ...
        btn:loadTextures(str1, str2, str3)
    end
...
function ApplyListLayer:tableCellAtIndex( view, idx )
	...
	if nil == cell then
        ...
        local _,h = UserItem.getSize()
		item:setPosition(view:getViewSize().width * 0.5, h*0.5-3)
