...
function GameLayer:onEventGameScene(cbGameStatus,dataBuffer)
    ...
    self._isBetOk = true
    print("266 self._isBetOk",self._isBetOk)
    --断线重连 清空数据
	...
	elseif cbGameStatus == g_var(cmd).GS_PLACE_JETTON 	then                      
        ...
        self._isBetOk = false
        print("283 self._isBetOk",self._isBetOk)
...
function GameLayer:onEventGameSceneFree(buffer)    --空闲 
    ...
    -- print("···cmd_table.dUserMaxScore is ================== >"..cmd_table.dUserMaxScore)
    dump(cmd_table,"cmd_table",12)
    self.BetArray = self:readOnly(cmd_table.lBetScore[1])
    ...
    if userItem.wChairID == self.m_wBankerUser then
        --申请按钮状态
        self._gameView:SetApplyStatus(GameViewLayer.applyed)
    end
...
function GameLayer:onEventGameSceneStatus(buffer)   
    ...
    -- self.m_lCurrentAddscore = cmd_table.lUserJettonScore[1];
    
    --是否已下注
    local isBet = false;
    for i = 1 ,#cmd_table.lUserJettonScore[1] do
        if i > 1 then
            self.m_lCurrentAddscore[i-1] = cmd_table.lUserJettonScore[1][i]
        end
    ...
    for i = 1 ,#cmd_table.lAllJettonScore[1] do
        if i > 1 then
            self.m_lAllJettonScore[i-1] = cmd_table.lAllJettonScore[1][i]
        end
    end
    self._gameView:UpdateAreaJetton2();
    ...
    if self._gameView.m_cbGameStatus == g_var(cmd).GS_GAME_END then
       ...
       clockType = g_var(cmd).GS_GAME_END
        ...
        if nil ~= userItem then
            str1 = ExternalFun.numberThousands(userItem.lScore+cmd_table.lEndBankerScore)
...
function GameLayer:onEventGameMessage(sub,dataBuffer)  
    ...
    elseif sub == g_var(cmd).SUB_S_GAME_START then --游戏开始
        self._isBetOk = true
        print("514 self._isBetOk",self._isBetOk)
...
function GameLayer:onSubGameFree(dataBuffer)  --游戏空闲
     ...
     if self.m_wBankerUser == self:GetMeUserItem().wChairID then
        showToast(self._gameView._bank,"现在是空闲时间，可以进行银行操作",1)
    end
...
function GameLayer:onSubGamePlaceJetton( dataBuffer ) --用户下注
    ...
    if cmd_table.wChairID ~= self:GetMeUserItem().wChairID then 
        ...
        self.m_lAllJettonScore[cmd_table.cbJettonArea] = self.m_lAllJettonScore[cmd_table.cbJettonArea] + cmd_table.lJettonScore
    else
        self.m_cbGameStatus =  g_var(cmd).GS_PLACE_JETTON
        --刷新玩家信息
        --print("self._gameView.m_scoreUser is ================ >"..self._gameView.m_scoreUser)
        -- print("cmd_table.lJettonScore is ============================== >"..cmd_table.lJettonScore)
        self._gameView:updateScore(self._gameView.m_scoreUser - cmd_table.lJettonScore)
        ...
        self.m_lContinueRecord[cmd_table.cbJettonArea] = self.m_lContinueRecord[cmd_table.cbJettonArea] + cmd_table.lJettonScore  --续压记录
    end
...
function GameLayer:onSubJettonFail( dataBuffer )
     ...
     if cmd_table.wPlaceUser == self:GetMeUserItem().wChairID then
        self.m_lAllJettonScore[cmd_table.cbJettonArea] =  self.m_lAllJettonScore[cmd_table.cbJettonArea] - cmd_table.lPlaceScore
        local viewIndex = self._gameView:GetViewAreaIndex(cmd_table.cbJettonArea)
        self.m_lCurrentAddscore[viewIndex] = self.m_lCurrentAddscore[viewIndex] - cmd_table.lPlaceScore
        --刷新显示
        self._gameView:UpdateAreaJetton()
     end
end
...
function GameLayer:onSubGameEnd( dataBuffer ) --游戏结束
    ...
    self.m_lBankerWinScore  = cmd_table.lBankerTotallScore
    ...
    if  self.m_lEndUserScore == 0 and lallJettonScore > 0 then
        self._gameView.m_scoreUser = self:GetMeUserItem().lScore
    end
