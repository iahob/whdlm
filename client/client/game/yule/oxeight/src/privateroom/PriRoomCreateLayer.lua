...
------------------------------------积分房卡-----------------------------------------
    ...
    for n = 1, 2 do
        ...
        local btn_help = self.m_cellNode[n][8]:getChildByName("btn_help")
        local txtTouchBg = self.m_cellNode[n][8]:getChildByName("Panel_4")
    ...
    --=================================================================
        ...
        self._joinMode = 1
        local  checkbx = self.panel[n]:getChildByName("cell10"):getChildByName("checkzhu1")
        ...
        for i=1,2 do
           local  checkbx = self.panel[n]:getChildByName("cell1"):getChildByName("checkModel"..i)
            checkbx:setTag(CBT_MODE + i)
            checkbx:setTouchEnabled(true)
            checkbx:setSelected(false)
            checkbx:addTouchEventListener(btncallback)
            self.m_checkTable[n][CBT_MODE+i] = checkbx
        end

        self.m_mode_index = CBT_MODE+1
        self.m_checkTable[n][self.m_mode_index]:setSelected(true)

        for i=1,2 do
            local checkbx = self.panel[n]:getChildByName("cell3"):getChildByName("touchBg"):getChildByName("downbg"):getChildByName("checkfb"..i)
        ...
        for i=1,7 do
            local checkbx = self.panel[n]:getChildByName("cell9"):getChildByName("touchBg"):getChildByName("downbg"):getChildByName("checkfb"..i)
        ...
        for i=1,4 do
            local checkbx = self.panel[n]:getChildByName("cell4"):getChildByName("checkadd"..i)
        ...
        for i=1,4 do
            local checkbx = self.panel[n]:getChildByName("cell8"):getChildByName("checkzhu"..i)
        ...
        for i=1,5 do
            local checkbx = self.panel[n]:getChildByName("cell7"):getChildByName("checkMore"..i)
            ...
            checkbx:addTouchEventListener(function (sender,eventType)
            --checkbx:addEventListener(function (sender,eventType)
               print("====================22"..eventType)
                if eventType == ccui.TouchEventType.began then 
                     
                    self:onButtonClickedEvent(sender:getTag(),sender)
                else
                    if sender:getTag() == CBT_GAOJI +2 then
                        if self.m_bIsNormal then
                            sender:setSelected(self.m_gaoji_index[sender:getTag() - CBT_GAOJI] == 1 and true or false)
                        else
            ...
            self.m_checkTable[n][CBT_GAOJI+i] = checkbx

            if n==2 and self.m_nOpenMode  and i == 2 then
                checkbx:setSelected(true)
            end
        end
        ...
        for i=1,3 do
           local  checkbx = self.panel[n]:getChildByName("cell5"):getChildByName("checkbei"..i)
            checkbx:setTag(CBT_Z_MAX + i)
            checkbx:setTouchEnabled(true)
            checkbx:setSelected(false)
            checkbx:addTouchEventListener(btncallback)
            self.m_checkTable[n][CBT_Z_MAX+i] = checkbx
        end

        self.m_zmax_index = CBT_Z_MAX+3
        self.m_checkTable[n][self.m_zmax_index]:setSelected(true)


        for i=1,9 do
            local checkbx = self.panel[n]:getChildByName("cell6"):getChildByName("checksp"..i)
            ...
            checkbx:addTouchEventListener(function (sender,eventType)
                if eventType == ccui.TouchEventType.began then 
                    ...
                    sender:setSelected(self.m_teshu_index[sender:getTag() - CBT_TESHU] == 1 and true or false)
                end

            end)
            self.m_checkTable[n][CBT_TESHU+i] = checkbx
        end

        self.m_teshu_index = {1,1,1,1,1,1,1,1,1}

        self.downBg[n] = self.panel[n]:getChildByName("cell3"):getChildByName("touchBg")
        self.downBg[n]:addTouchEventListener(function ()
                self.downBg[n]:setVisible(false)
                self.downBg[n]:setTouchEnabled(false)
        end)

        local btn =  self.panel[n]:getChildByName("cell3"):getChildByName("Button_9")
        btn:addTouchEventListener(function ()
                self.downBg[n]:setVisible(true)
                self.downBg[n]:setTouchEnabled(true)
        end)
        
        self.downBgPlayer[n] = self.panel[n]:getChildByName("cell9"):getChildByName("touchBg")
        self.downBgPlayer[n]:addTouchEventListener(function ()
                self.downBgPlayer[n]:setVisible(false)
                self.downBgPlayer[n]:setTouchEnabled(false)
        end)

        local btn =  self.panel[n]:getChildByName("cell9"):getChildByName("Button_9")
        btn:addTouchEventListener(function ()
                self.downBgPlayer[n]:setVisible(true)
                self.downBgPlayer[n]:setTouchEnabled(true)
        end)
           ...
           if n==1 then --积分
                    ...
                    -- 一人付
                    checkbx = self.panel[n]:getChildByName("cell2"):getChildByName("checkfee1")
                    ...
                    -- AA付
                    checkbx = self.panel[n]:getChildByName("cell2"):getChildByName("checkfee2")
                    ...
                    local btn = self.panel[n]:getChildByName("cell2"):getChildByName("btn_difen")
                    btn:setTag(TAG_ENUM.BTN_CELLSCORE)
                    btn:addTouchEventListener(btncallback)

                     local btn = self.panel[n]:getChildByName("cell2"):getChildByName("btn_ruchang")
                    btn:setTag(TAG_ENUM.BTN_ENTERCONDITION)
                    btn:addTouchEventListener(btncallback)

                    local btn = self.panel[n]:getChildByName("cell2"):getChildByName("btn_lichang")
                    ...
                    self.m_txtCellScoreLimit = self.panel[n]:getChildByName("cell2"):getChildByName("txt_difen")
                    self.m_txtCellScoreRange = self.panel[n]:getChildByName("cell2"):getChildByName("txt_difen_range")
                    self.m_txtEnterLimit = self.panel[n]:getChildByName("cell2"):getChildByName("txt_ruchang")
                    self.m_txtExitLimit = self.panel[n]:getChildByName("cell2"):getChildByName("txt_lichang")
   ...
   -- local passwdmodelistener = function (sender,eventType)
    ...
    -- 需要密码
    --checkbx = spbg:getChildByName("Panel_1"):getChildByName("cell7"):getChildByName("checkMore2")
    --checkbx:setTag()
    --checkbx:setSelected(true)
   ...
   -- checkbx.nPasswdMode = self._cmd_pri.define.tabPasswdMode.NO_PASSWD
    ...
    -- self.slider = spbg:getChildByName("Slider_1")
    -- self.slider:onEvent(handler(self,PriRoomCreateLayer.SlideEvent))
    -- self.slider:setPercent(0)


    -- 创建按钮
    -- btn = spbg:getChildByName("Panel_2"):getChildByName("bt_createRoom2")
    ...
    self.m_ruleCell = {}
    self.m_ruleCell[1] = {{{1,3,4,2,8,7},{1,4,2,8,7}},
                                    {{1,9,3,6,5,2,8,7},{1,9,6,5,2,8,7}},
                                    {{1,9,3,6,2,8,7},{1,9,6,2,8,7}},
                                    {{1,9,3,6,2,8,7},{1,9,6,2,8,7}},
                                    {{1,9,3,6,2,8,7},{1,9,6,2,8,7}},
                                    {{1,9,3,6,2,7},{1,9,6,2,7}}}

    self.m_ruleCell[2] = {{{2,1,3,4,7},{2,1,4,7}},
                                    {{2,1,3,6,5,10,7},{2,1,6,5,10,7}},
                                    {{2,1,3,6,10,7},{2,1,6,10,7}},
                                    {{2,1,3,6,10,7},{2,1,6,10,7}},
                                    {{2,1,3,6,10,7},{2,1,6,10,7}},
                                    {{2,1,3,6,10,7},{2,1,6,10,7}}}

    self.m_cellHeigh ={80,160,80,80,80,240,240,80,80,80}
    self.m_rule = 1
...
function PriRoomCreateLayer:updateCell(tag,index)
    ...
    for n = 1, 2 do
            ...
            if tag == 6 then
                self.m_gaoji_index[1] = 0

                self.m_checkTable[n][CBT_GAOJI+1]:setSelected(false)
                self.m_checkTable[n][CBT_GAOJI+1]:setEnabled(false)
                self.m_checkTable[n][CBT_GAOJI+1]:getChildByName("Text_7"):setColor(cc.c3b(102, 102, 102))
                
            else
                self.m_checkTable[n][CBT_GAOJI+1]:setEnabled(true)
                self.m_checkTable[n][CBT_GAOJI+1]:getChildByName("Text_7"):setColor(cc.c3b(255, 255, 255))
            end

            self.m_cellNode[n][8]:getChildByName("Panel_4"):setVisible(false)
            self.downBgPlayer[n]:setVisible(false)
            self.downBg[n]:setVisible(false)
    end
...
function PriRoomCreateLayer:onRefreshFeeList()
 ...
 local nConfig = #PriRoom:getInstance().m_tabFeeConfigList
    ...
    for i = 1, nConfig do
        ...
        local checkbx = panel:getChildByName("cell2"):getChildByName("checkju"..i)
        if nil ~= checkbx then
    ...
    dump(cmd_table)
        ...
        for i=1,6 do
            if cmd_table.cbBankerModeEnable[1][i] == 0 then
                self.m_ruleTxt[i]:setVisible(false)
                num = num +1
            else
                -- self.m_ruleBtn[i]:setPositionY(self.m_ruleBtn[i]:getPositionY() + num*75)
                table.insert(visibleBtn,self.m_ruleTxt[i])
                if index == 0 then
                    index = i
                    self.m_rule = i 
                end
        ...
        if count % 2 == 1 then
            for i, node in ipairs(visibleBtn) do
                node:setPositionX(RuleBtnPosX[i])
                node:setVisible(true)
                if count > 1 and i ~= count then
                    self.m_line[i]:setPositionX(RuleBtnPosX[i] + 109)
                    self.m_line[i]:setVisible(true)
                end
            end
        else
            local c = count/2
            for k = 1, count-1 do
                self.m_line[k]:setPositionX(RuleBtnPosX[k])
                self.m_line[k]:setVisible(true)
            end
            for i, node in ipairs(visibleBtn) do
                if i <= c then
                    node:setPositionX(705 - (0.5 + i - 1)*218 )
                else
                    node:setPositionX(705 + (i-c - 0.5)*218 )
                end
                node:setVisible(true)
            end
        end
        ...
        for n=1 ,2 do
                    ...
                    num = 0
                    for i=1,2 do
                        local mode = panel:getChildByName("cell1"):getChildByName("checkModel"..i)
                        if cmd_table.cbGameModeEnable[1][i] == 0 then
                            mode:setVisible(false)

                            local other = panel:getChildByName("cell1"):getChildByName("checkModel"..(i ==1 and 2 or 1))
                    ...
                    num = 0
                    for i=1,4 do
                        local mode = panel:getChildByName("cell4"):getChildByName("checkadd"..i)
                        if cmd_table.cbBeBankerCon[1][i] == 0 then
                            mode:setVisible(false)

                            num = num +1
                        else
                            local str = mode:getChildByName("Text_7")
                    ...
                    if num ==4 then
                            ...
                            table.remove(self.m_ruleCell[n][1][1],3)
                            table.remove(self.m_ruleCell[n][1][2],2)
                    end
                    ...
                    --panel:getChildByName("cell8"):getChildByName("btn_help"):setVisible(false)
                    -- num = 0
                    -- for i=1,4 do
                    --     local mode = panel:getChildByName("cell8"):getChildByName("checkzhu"..i)
                    --     if cmd_table.cbUserbetTimes[1][i] == 0 then
                    --         mode:setVisible(false)

                    --         num = num +1
                    --     else
                    --         local str = mode:getChildByName("Text_7")
                    --         str:setString(cmd_table.cbUserbetTimes[1][i].."倍")
                    ...
                    --                 for k,v in pairs(self.m_ruleCell[n][i][j]) do
                    --                     if v== 8 then
                    --                         table.remove(self.m_ruleCell[n][i][j],k)
                    --                     end
                    --                 end
                    --             end
                    --         end
                    ...
                    for i=1,5 do
                        local mode = panel:getChildByName("cell7"):getChildByName("checkMore"..i)
                        if cmd_table.cbAdvancedConfig[1][i] == 0 or (i == 2 and n == 2) then
                            mode:setVisible(false)
                        else
                    ...
                    local point = {cc.p(270,200),cc.p(620,200),cc.p(270,120),cc.p(620,120),cc.p(270,40)}
                    if #ruleT < 5 then
                        for i=1, #ruleT do
                    ...
                    local ruleT= {}
                    for i=1,9 do
                        local mode = panel:getChildByName("cell6"):getChildByName("checksp"..i)
                    ...
                    if #ruleT < 9 then
                        for i=1, #ruleT do
                            ruleT[i]:setPosition(270 +((i-1)%4)*180,200-80*math.floor((i-1)/4))
                        end
                    end

                    if #ruleT<= 4 then
                        self.m_cellHeigh[6] = 80
                    elseif #ruleT<=8 then
                        self.m_cellHeigh[6] = 160
                    end
           end
    --self:updateCell(self.m_rule,self.m_mode_index-CBT_MODE)
    self:updateRule(self.m_rule)
...
function PriRoomCreateLayer:onLoginPriRoomFinish()
    ...
    if ((meUser.cbUserStatus == yl.US_FREE 
        ...
        if PriRoom:getInstance().m_nLoginAction == PriRoom.L_ACTION.ACT_CREATEROOM then
            ...
            if self.m_gaoji_index[2] == 0 or not self.m_bIsNormal then
                self.m_nSelectPasswd = self._cmd_pri.define.tabPasswdMode.NO_PASSWD
            else
            ...
            buffer:pushbyte(1)
            if self.m_bIsNormal then
                buffer:pushbyte(self.playerNum_index - CBT_PLAYER_NUM +1)
            else
                buffer:pushbyte(8)
            end
 ...
 --[[]]
            --cc.UserDefault:getInstance():setIntegerForKey("PlayerModeconfig_fightten", self.m_nPlayerMode)
            --cc.UserDefault:getInstance():setIntegerForKey("GameModeconfig_fightten", self.m_nGameMode+22)
            --cc.UserDefault:getInstance():setIntegerForKey("PokerModeconfig_fightten", self.m_nPokerMode+32)
            --cc.UserDefault:getInstance():setIntegerForKey("BankerModeconfig_fightten", self.m_nBankerMode+52)
...
function PriRoomCreateLayer:getInviteShareMsg( roomDetailInfo )
    ...
    local PlayerMode = cc.UserDefault:getInstance():getIntegerForKey("PlayerModeconfig_fightten",0)
    local chairCountstr = PlayerMode == 0 and "2-8人" or string.format("%d人", PlayerMode)
    local configStr = string.format("%s,%d局,", chairCountstr,turnCount)
    configStr = configStr..cardType[cc.UserDefault:getInstance():getIntegerForKey("GameModeconfig_fightten",0)-22+1]..","..sendType[cc.UserDefault:getInstance():getIntegerForKey("PokerModeconfig_fightten",0)-32+1]..","..bankerType[cc.UserDefault:getInstance():getIntegerForKey("BankerModeconfig_fightten",0)-52+1]
    local shareTxt = configStr .. "。八人牛牛精彩刺激, 一起来玩吧!"
...
--print(tag,"=======================>1")
    ...
    elseif  tag >= CBT_MODE and tag<(CBT_MODE+10) then

           
            local last = self.m_mode_index
    ...
    elseif  tag >= CBT_FANBEI and tag<(CBT_FANBEI+10) then 
            ...
            for n=1,2 do
                    ...
                    local str =panel:getChildByName("cell3"):getChildByName("Button_9"):getChildByName("Text_33")
                    if (self.m_fanbei_index - CBT_FANBEI) == 1 then
                        str:setString("牛牛x4 牛九x3 牛八x2 牛七x2")
                    else
                        str:setString("牛牛x3 牛九x2 牛八x2")
                    end
                    self.downBg[n]:setVisible(false)
                    self.downBg[n]:setTouchEnabled(false)
            end
    ...
    elseif  tag >= CBT_PLAYER_NUM + 1 and tag<(CBT_PLAYER_NUM+8) then 
            ...
            for n=1,2 do
                    ...
                    local str =panel:getChildByName("cell9"):getChildByName("Button_9"):getChildByName("Text_33")
                    local num = self.playerNum_index - CBT_PLAYER_NUM + 1
                    str:setString(num.."人")
                    self.downBgPlayer[n]:setVisible(false)
                    self.downBgPlayer[n]:setTouchEnabled(false)
            end
    ...
    elseif  tag >= CBT_GAOJI and tag<(CBT_GAOJI+10) then 

        if tag == CBT_GAOJI +2 then

            if self.m_bIsNormal then
                self.m_gaoji_index[tag - CBT_GAOJI] = (self.m_gaoji_index[tag - CBT_GAOJI] +1)%2
                self.m_checkTable[1][tag]:setSelected(self.m_gaoji_index[tag - CBT_GAOJI] == 1 and true or false)
            else
                self.m_nOpenMode = not self.m_nOpenMode
                self.m_checkTable[2][tag]:setSelected(self.m_nOpenMode)
            end

        else
            self.m_gaoji_index[tag - CBT_GAOJI] = (self.m_gaoji_index[tag - CBT_GAOJI] +1)%2
            for n=1,2 do
                self.m_checkTable[n][tag]:setSelected(self.m_gaoji_index[tag - CBT_GAOJI] == 1 and true or false)

            end
        end

    elseif  tag >= CBT_Z_MAX and tag<(CBT_Z_MAX+10) then 

            local last = self.m_zmax_index
    ...
    elseif  tag >= CBT_TESHU and tag<(CBT_TESHU+10) then 

        self.m_teshu_index[tag - CBT_TESHU] = (self.m_teshu_index[tag - CBT_TESHU] +1)%2
        for n=1, 2 do
            self.m_checkTable[n][tag]:setSelected(self.m_teshu_index[tag - CBT_TESHU] == 1 and true or false)
        end
...
function PriRoomCreateLayer:setConfigString(tag, scoere)
    ...
    elseif TAG_ENUM.BTN_ENTERCONDITION == tag then
        ...
        local cell = cell1 > cell2  and cell1 or  cell2
        --入场设定要判断底分,大于最小值才生效
        if scoere > cell then
            ...
            if self.m_enterScore < cell then
                self.m_enterScore = cell
            end
...
function PriRoomAAPayLayer:ctor( scene, param, level )
    ...
    if nil ~= buffer and nil ~= buffer.readbyte then
        ...
        --规则
        local sendMode = buffer:readbyte()
        print("发牌模式..",sendMode)
        local szSendMode=""
        if sendMode == 32 then
            szSendMode = "发四等五"
        ...
        local jokerMode = buffer:readbyte()
        print("Joker模式..",jokerMode)
        local szJokerMode=""
        if jokerMode == 42 then
            szJokerMode = "有大小王"
        elseif jokerMode == 43 then
            szJokerMode = "无大小王"   
        end

        --庄家玩法
        local bankerMode = buffer:readbyte()
        print("庄家模式..",bankerMode)
        local szbankerMode=""
        ...
        -- 规则
        spbg:getChildByName("txt_guize"):setString(szSendMode..","..szJokerMode..","..szbankerMode)
    end
