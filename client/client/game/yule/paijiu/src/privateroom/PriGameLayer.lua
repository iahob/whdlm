...
local DismissLayer = appdf.req(appdf.GAME_SRC.."yule.paijiu.src.privateroom.DismissLayer")
local MarkLayer = appdf.req(appdf.GAME_SRC.."yule.paijiu.src.privateroom.MarkLayer")
...
function PriGameLayer:onRefreshInfo()
    ...
    if GlobalUserItem.bPrivateGoldRoom then
        self.m_atlasCount:setVisible(false)
         self.btnRound:getChildByName("Text_1"):setString("房间信息")
        ...
        self.btnRound:getChildByName("Text_1"):setString("局数：")
...
function PriGameLayer:onPriGameEnd( cmd_table ) 
    ...
    self:runAction(cc.Sequence:create(cc.DelayTime:create(3.0),
        cc.CallFunc:create(function()
            self:onPriGameEndDelay( cmd_table )         
        end)))
end

-- 私人房游戏结束
function PriGameLayer:onPriGameEndDelay( cmd_table ) 
    ...
    for i = 1, chairCount do --chairid
        ...
        if chairCount<6 then cellbg:setPositionY(cellbg:getPositionY()-30) end
        ...
        if nil ~= useritem then
           ...
           local a,b,c=0,0,0
           if self._gameLayer.winInfo then
                a=self._gameLayer.winInfo[i][1]
                b=self._gameLayer.winInfo[i][2]
                c=self._gameLayer.winInfo[i][3]
           end
