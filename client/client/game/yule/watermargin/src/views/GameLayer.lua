...
function GameLayer:onSubGame1End( dataBuffer )
    ...
    if self.m_lTotalYafen > self.m_lCoins + self.m_lGetCoins then
        ...
        if self.m_bIsAuto == true then
            print(self:getGameMode(),GAME_STATE.GAME_STATE_END,"------游戏1结束------")
...
function GameLayer:onGameStart()
    ...
    --自动开始
    print(self:getGameMode(),GAME_STATE.GAME_STATE_WAITTING_GAME2,self.m_bIsAuto,"------------自动开始---------------")
    if self.m_bIsAuto == true then
        ...
        elseif self:getGameMode() == GAME_STATE.GAME_STATE_WAITTING_GAME2 or self:getGameMode() == GAME_STATE.GAME_STATE_WAITTING then
            print("游戏开始")
