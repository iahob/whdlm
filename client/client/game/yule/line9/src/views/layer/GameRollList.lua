...
function GameRollList:ctor(gameView)
    ...
    self.m_listdata = {}
    self.ListWidth      = 210    --滚动宽度
    self.ListHeight     = 507    --滚动高度
...
function GameRollList:initImage(playout, rand)
    ...
    if rand then
        ...
        for i = 1, 3 + self.hideItem do
            ...
            local pSprite = display.newSprite(string.format("#game/im_football_%d.png", imagetype))
...
--        end
        ...
        for index = 1, self.m_RollLayout:getChildrenCount() do
            ...
            if self.m_RollLayout:getChildByTag(index):getPositionY() <= (- self.itemHeight + self.bottomHeight) and index == self.hideItem then
                ...
                for index = 1, self.m_RollLayout:getChildrenCount() do
                    self.m_RollLayout:getChildByTag(index):runAction(cc.Sequence:create(cc.CallFunc:create(function() end),
        ...
        for index = 1, self.m_RollLayout:getChildrenCount() do
            ...
            if self.m_RollLayout:getChildByTag(index):getPositionY() <= (- self.itemHeight + self.bottomHeight) then
                self.bollPos[index] = 1245 + (self.m_RollLayout:getChildByTag(index):getPositionY() + self.itemHeight - self.bottomHeight)
                self.m_RollLayout:getChildByTag(index):setPosition(cc.p(self.ListWidth / 2,self.bollPos[index]))
                if index == 1 then
                    for i = 1, 3 do
                        local imagetype = self.m_listdata[i - 1]
                        self.m_RollLayout:getChildByTag(i + self.hideItem):setSpriteFrame(string.format("game/im_football_%d.png", imagetype))
                    end
                    self.gameEnd = true
                end
            end
        ...
        for index = 1, self.m_RollLayout:getChildrenCount() do
            ...
            if self.m_RollLayout:getChildByTag(index):getPositionY() <= (- self.itemHeight + self.bottomHeight) then
                ...
                self.m_RollLayout:getChildByTag(index):setPosition(cc.p(self.ListWidth / 2,self.bollPos[index]))
                local imagetype = math.random(100) % 10
                self.m_RollLayout:getChildByTag(index):setSpriteFrame(string.format("game/im_football_%d.png", imagetype))
            end
        end
    end
end
