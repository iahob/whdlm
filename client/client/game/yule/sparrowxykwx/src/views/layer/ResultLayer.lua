...
local ResultLayer = class("ResultLayer", function(scene)
	local resultLayer = cc.CSLoader:createNode(cmd.RES_PATH.."GameResultLayer.csb")
	return resultLayer
end)
	...
	btContinue:addClickEventListener(function(ref)
	  	if PriRoom and GlobalUserItem.bPrivateRoom then
	        if PriRoom:getInstance().m_tabPriData.dwPlayCount < PriRoom:getInstance().m_tabPriData.dwDrawCountLimit then
	        	...
	        	--显示结算界面
		    	local layerEnd = self._scene:getChildByName("private_end_layer")
		    	if nil ~= layerEnd then
		    		layerEnd:setVisible(true)
		    		self._scene._scene.isCloseGameEnd = true
		    	end
	        end
	    else
	    	self._scene:onButtonClickedEvent(self._scene.BT_START)
...
function ResultLayer:showLayer(resultList, wBankerChairId, myChirID,cbHuCard, maData, validMaNum, validMaData, wFleeUser, isNoMagicCard)
	print("ResultLayer:showLayer ignored!!")
	do return end
