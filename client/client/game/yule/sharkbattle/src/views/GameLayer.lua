...
function GameLayer:onEventGameScene(cbGameStatus,dataBuffer)
	...
	elseif cbGameStatus==cmd.GAME_SCENE_BET then
		...
		local totalBet = 0
		for i=1,#pStatusPlay.lPlayBet[1] do
			totalBet = totalBet + pStatusPlay.lPlayBet[1][i]
		end
   		self._gameView:UpdateShowTotalScore(GlobalUserItem.tabAccountInfo.lUserScore - totalBet)
		self._gameView:UpdateShowTotalBet(totalBet)
	...
	elseif cbGameStatus==cmd.GAME_SCENE_END then
		...
		local totalBet = 0
		for i=1,#pStatusEnd.lPlayBet[1] do
			totalBet = totalBet + pStatusEnd.lPlayBet[1][i]
		end
   		self._gameView:UpdateShowTotalScore(GlobalUserItem.tabAccountInfo.lUserScore - totalBet)
		self._gameView:UpdateShowTotalBet(totalBet)
