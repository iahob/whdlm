...
local posHandDownCard = {cc.p(955, 616), cc.p(251, 655), cc.p(25, 70), cc.p(1110, 215)}
local posDiscard = {cc.p(922, 510), cc.p(374, 535), cc.p(436, 258), cc.p(986, 204)}
local posBpBgCard = {cc.p(980, 616), cc.p(251, 700), cc.p(0, 81), cc.p(1111, 165)}
...
CardLayer.TAG_SEND_CARD_2_PLAYER = 1000
		...
		for j = 1, cmd.MAX_COUNT do
			...
			if i == cmd.MY_VIEWID then
				...
				if self._scene._scene.m_bLookOn then 
					...
					card:setTexture(cmd.RES_PATH.."game/font_big/card_back.png")
				else
					font:setVisible(true)
					card:setTexture(cmd.RES_PATH.."game/font_big/card_up.png")
				end
...
function CardLayer:onTouchBegan(touch, event)
	...
	if self.nCurrentTouchCardTag ~= 0  or self._scene._scene.m_bLookOn then
		return
	end
	...
	for i = 1, cmd.MAX_COUNT do
		...
		if cc.rectContainsPoint(cardRect, posRelativeCard) and card:isVisible() and not self.bCardGray[i] then
			...
			local selCardValue = self.cbCardData[index]
			print("选中的牌 cardValue",selCardValue)
			self:OutCardTip(selCardValue)
			...
			if self.nCurrentTouchCardTag == i then
				if self.cbCardStatus[index] == CardLayer.ENUM_CARD_NORMAL then 		--原始状态
					...
					if math.mod(self.cbCardCount[cmd.MY_VIEWID], 3) == 2 then
						print("self.cbCardData[index] = ",self.cbCardData[index])
						local cbPromptHuCard,leftCount = self._scene._scene:getListenPromptHuCard(self.cbCardData[index])
						self._scene:setListeningCard(cbPromptHuCard,leftCount)
					end
...
function CardLayer:sendCard(cardData, cardCount)
	...
	for i=1,4 do
		for k,v in ipairs(self._discardDataIdx[i]) do
			local card = self.nodeDiscard[i]:getChildByTag(k)
			if card then
				card:setColor(cc.c3b(0xFF,0xFF,0xFF))
			end
		end
	end
end
...
function CardLayer:catchCard(viewId, cardData, bTail)
	...
	if viewId == cmd.MY_VIEWID then
		...
		if self._scene._scene.m_bLookOn then 
			...
			HandCard:setTexture(cmd.RES_PATH.."game/font_big/card_back.png")
		else
			cardFont:setVisible(true)
			HandCard:setTexture(cmd.RES_PATH.."game/font_big/card_up.png")
...
function CardLayer:setHandCard(viewId, cardCount, cardData)
	...
	if self.cbCardCount[viewId] ~= 0 then
		...
		for j = 1, self.cbCardCount[viewId] do
			...
			if viewId == cmd.MY_VIEWID and card then
				...
				if j<= #self.cbCardData then
					...
					if self.cbCardData[j] == GameLogic.MAGIC_DATA and not self._scene._scene.m_bLookOn then
...
function CardLayer:discard(viewId, cardData)
	...
	table.insert(self._discardDataIdx[viewId], cardData)
end
...
function CardLayer:recycleDiscard(viewId)
	...
	self.nDiscardCount[viewId] = self.nDiscardCount[viewId] - 1
	table.remove(self._discardDataIdx[viewId])
end
...
--发牌
function CardLayer:sendCardToPlayer(viewid, cardData, cardCount)
	local function callbackSendFinish(sender, args)
		local viewid, cardCount = unpack(args)
		local cardNodeParent = self.nodeHandCard[viewid]
		for i=1,cardCount do
			local tag = cmd.MAX_COUNT - (self.curCardCount[viewid]+i) +1
			print("tag===",tag)
			local card = cardNodeParent:getChildByTag(tag)
			if card and not tolua.isnull(card) then
	      		card:setVisible(true)
			end
		end

    	self.curCardCount[viewid] = self.curCardCount[viewid] + cardCount
	end

	local nextIdx = 1
	if cardCount > 1 then
		nextIdx = math.ceil(cardCount*0.5)
	end
	local tag = self.curCardCount[viewid]+nextIdx
	tag = cmd.MAX_COUNT - tag +1

	print("sendCardToPlayer  nextIdx, tag", nextIdx, tag)

	local cardNodeParent = self.nodeHandCard[viewid]
	local card = cardNodeParent:getChildByTag(tag)
	local x,y = card:getPosition()
	local pos = cc.pAdd(posHandCard[viewid], cc.p(x,y))

	local heap = cc.CSLoader:createNode(cmd.RES_PATH.."card/Node_heap.csb")
	heap:setPosition(cc.p(667, 375))
	heap:setTag(self.TAG_SEND_CARD_2_PLAYER+viewid)
	self:addChild(heap)

	local callFun = cc.CallFunc:create(callbackSendFinish, {viewid, cardCount})
...
function CardLayer:sendCard(cardData, cardCount)
	...
	for i = 1, cmd.GAME_PLAYER do
		...
		--先全部隐藏
		for j=1, cmd.MAX_COUNT do
			self.nodeHandCard[i]:getChildByTag(j):setVisible(false)
			self.nodeHandDownCard[i]:getChildByTag(j):setVisible(false)
		end
	end
	----------------------------------------------------先创建牌

	local cardCounts = {4,4,4,1}
	local actionList = {}
	local curCardIdx = 0
	local gameLayer = self._scene._scene

	for k,slice in ipairs(cardCounts) do
		for i=1,4 do --这里要遍历4次,因为两个人的时候还有座位id是3的情况
			if gameLayer.m_cbPlayStatus[i] ==1 or (GlobalUserItem.bVideo and self.cbCardCount[i]>0) then    -- 回放要特殊处理
	            ...
	            local cardData = {}
	            if viewid == cmd.MY_VIEWID then
	            	for cardIdx=1,slice do
	            		table.insert(cardData, self.cbCardData[curCardIdx+cardIdx])
	            	end
	            	curCardIdx = curCardIdx + slice
	            end

				local function callback(sender, args)
					local viewid, cardData, slice = unpack(args)
			      	self:sendCardToPlayer(viewid, cardData, slice)
			    end

	            local callfunc = cc.CallFunc:create(callback, {viewid, cardData, slice})
	...
	local viewid
	local cardData = {}
	for k,v in pairs(self.cbCardCount) do
		if v == 14 then
			viewid = k
			break
		end
	...
	if viewid == cmd.MY_VIEWID then
		table.insert(cardData, self.userCardData[viewid][14])
	end

	local function callback(sender, args)
		local viewid, cardData, cardCount = unpack(args)
      	self:sendCardToPlayer(viewid, cardData, cardCount)
    end
    local callfunc = cc.CallFunc:create(callback, {viewid, cardData, 1})
    table.insert(actionList, cc.DelayTime:create(0.2))
    table.insert(actionList, callfunc)
	------------------------------------------------------给庄家多发一张

	local finCallback = cc.CallFunc:create(function()
		self._scene:sendCardFinish()
		end)
    table.insert(actionList, finCallback)

	self:runAction(cc.Sequence:create(actionList))
end

function CardLayer:OutCardTip(cardValue)
	-- for viewid,cards in pairs(self._discardDataIdx[viewId]) do
	-- 	for idx,value in ipairs(cards) do
	-- 		if value > 0 and cardValue == value then
	-- 			table.insert(self.records, {viewid,idx})
	-- 		end
	-- 	end
	-- end

	-- for k,viewid_idx in pairs(self._discardDataIdx[viewId]) do
	-- 	local viewid,idx = unpack(viewid_idx)
	-- 	local card = self.nodeDiscard[viewid]:getChildByTag(idx)
	-- 	if card then
	-- 		card:setColor(cc.c3b(0xff,0x00,0x00))
	-- 	end
	-- end
	...
	for i=1,4 do
		for k,v in ipairs(self._discardDataIdx[i]) do
			...
			if card then
				if v == cardValue then
					card:setColor(cc.c3b(0x7f,0x7f,0x7f))
				else
					card:setColor(cc.c3b(0xFF,0xFF,0xFF))
				end
			end
