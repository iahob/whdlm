...
function ResultLayer:ctor(scene)
    ...
    local btn_share = cc.funAddChildListener(self, "Button_share", function() self:recodeShow() end)
    local btn_leave = cc.funAddChildListener(self, "Button_leave", function()
    	self:hideLayer()
	...
	local node_private = cc.find(self, "Node_private")
	local node_gold = cc.find(self, "Node_gold")
	local lab_time = cc.find(self, "Text_time")
	local lab_rule = cc.find(self, "lab_rule")
    ...
    if cbMaCount == 1 then
        ...
        strMa = string.format("%s个扎马", cbMaCount)
    end

	lab_time:setString((os.date("%Y-%m-%d %H:%M:%S", os.time())))
	lab_rule:setString(string.format("玩法:%s", strMa))

	node_private:setVisible(GlobalUserItem.bPrivateRoom and true)
	node_gold:setVisible(not(GlobalUserItem.bPrivateRoom and true))
	btn_leave:setVisible(not(GlobalUserItem.bPrivateRoom and true))

	if GlobalUserItem.bPrivateRoom then
		btn_share:setPosition(500, 60)
		btContinue:setPosition(850, 60)

		local roomNum = PriRoom:getInstance().m_tabPriData.dwPersonalRoomID
		local playedNum = PriRoom:getInstance().m_tabPriData.dwPlayCount
		-- local totalRoundNum = PriRoom:getInstance().m_tabPriData.dwDrawCountLimit

		-- cc.find(node_private, "lab_0"):setString("房号:")
		cc.find(node_private, "lab_1"):setString(roomNum)
		-- cc.find(node_private, "lab_2"):setString("局数:")
		cc.find(node_private, "lab_3"):setString(playedNum)
	else
		-- btn_leave:setPosition(23.68, 70)
		-- btContinue:setPosition(49.93, 70)
		-- btChange:setPosition(76.18, 70)

		-- cc.find(node_gold, "lab_0"):setString("金币场:")
		cc.find(node_gold, "lab_1"):setString("中级场")
		-- cc.find(node_gold, "lab_2"):setString("底分:")
		cc.find(node_gold, "lab_3"):setString(self._scene._scene.lCellScore)
	end

	local btClose = self:getChildByName("Button_1")
...
function ResultLayer:showLayer(resultList, cbAwardCard, cbRemainCard, wBankerChairId, cbHuCard)
	...
	for i = 1, cmd.GAME_PLAYER do
		if i <= #resultList then
			...
			--奖码
			fX = 900-(width+3)*(self._scene._scene.cbMaCount/2)
