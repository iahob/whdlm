...
function GameLayer:onEventGameScene(cbGameStatus, dataBuffer)
	...
	if cbGameStatus == cmd.GAME_SCENE_FREE then
		...
		self.chairCount = cmd_data.cbPlayerCount or 4
		        ...
		        --游戏模式
    		self.cbPlayMode = cmd_data.cbPlayMode
	...
	elseif cbGameStatus == cmd.GAME_SCENE_PLAY then
		...
		self.cbOutCardData = cmd_data.cbDiscardCard
	...
	if GlobalUserItem.dwCurRoomGroupID ~= 0 and GlobalUserItem.bPrivateGoldRoom then 
	    ...
	    if PriRoom and GlobalUserItem.bPrivateRoom then
            ...
            if not GlobalUserItem.bPrivateGoldRoom then
                self._gameView.btTrustee:setVisible(false)
            end
	    end
    	self._gameView:onRefreshInfo()
	self._gameView:refreshMaCount()
	...
	for i = 1, self.chairCount do
		...
		if self._gameView.nodePlayer[wViewChairId] then
			self._gameView.nodePlayer[wViewChairId]:getChildByName("sp_roomHost"):setVisible(false)
		end
...
function GameLayer:getGameRule(buffer)
        ...
        local maMode =  buffer[4]--buffer:readbyte()

        local maTable ={"一码全中","","",""}
        if maMode == 1 then
        	table.insert(strTable,"一码全中")
        else
        	table.insert(strTable,maMode.."个扎码")
        end
...
function GameLayer:isSeatDown(sid)
	local userItem = self._gameFrame:getTableUserItem(self:GetMeTableID(), sid)
	return userItem and true
end
...
function GameLayer:onSubOutCard(dataBuffer)
	...
	-----------------------------------------------------------------记录出牌
	for k,v in ipairs(self.cbOutCardData[wViewId]) do
		if v == 0 then
			self.cbOutCardData[wViewId][k] = cmd_data.cbOutCardData
			break
		end
	end
	-----------------------------------------------------------------记录出牌
...
function GameLayer:onSubSendCard(dataBuffer)
	...
	-- if wCurrentViewId ~= self:SwitchViewChairID(self.wCurrentUser) then 
	-- 	self._gameView:gameSendCard(wCurrentViewId,)
	-- else
	...
	self._gameView:HideGameBtn()
	if self.wCurrentUser == self:GetMeChairID() and not self.m_bLookOn then
...
function GameLayer:onSubGameConclude(dataBuffer)
	...
	for i = 1, self.chairCount do
		...
		if user and self.m_cbPlayStatus[i] == 1 then
			local result = {}
			...
			maCount = math.max(maCount, cmd_data.cbMaCount[1][i])
	...
	local function afterMoMa()
		self:runAction(cc.Sequence:create(cc.DelayTime:create(1), cc.CallFunc:create(function(ref)
			...
			self._gameView.btStart:setVisible(true)
                                      self:removeChildByName("_maLayer_")
		end)))
	end
	...
	if bMeWin == nil then
	           afterMoMa()
                        ...
                        self:removeChildByName("_maLayer_")
	           maLayer = uiMaCollect:ctor(self, afterMoMa)
                        maLayer.widget:setName("_maLayer_")
	           maLayer:refresh(cbAwardCardTotal or {0x15,0x15,0x15,0x16,0x15,0x15})
	end
...
function GameLayer:sendCardFinish()
	...
	if self.cbActionMask then
		self._gameView:recognizecbActionMask(self.cbActionMask, self.cbActionCard, true) --davdi 20180904 noclock为true，结束不重新起定时器
	end
...
function GameLayer:sendRequestRecord()
	...
	return self:SendData(cmd.SUB_C_REQUEST_RECORD, cmd_data)
end
--操作扑克
...
--     if PriRoom and true == GlobalUserItem.bPrivateRoom and false == GlobalUserItem.bPrivateGoldRoom then  --金币房卡可以直接退出
...
--         self._queryDialog = QueryDialog:create("您要退出游戏么？现在退出游戏系统会自动接管游戏噢~", function(ok)
--             if ok == true then
--                 --退出防作弊
--                 self._gameFrame:setEnterAntiCheatRoom(false)
--                 if GlobalUserItem.bPrivateGoldRoom and self.m_cbGameStatus > 0 then
