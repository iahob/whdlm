...
function GameChatLayer:ctor(netframe, tabconfig)
    ...
    -- if 55 ~= GlobalUserItem.nCurGameKind then --牛牛走传递的CSB
    --     csbfile = "public/GameChatLayer.csb"
    -- end
    --加载csb资源
...
function GameChatLayer:onButtonClickedEvent( tag, sender )
    ...
    elseif CBT_TEXT == tag then
        ...
        self.m_spSelect:setPositionX(self.m_checkText:getPositionX())
        self.m_spSelect:setScaleX(1)
        -- self.m_spSelect_1:setVisible(true);
        -- self.m_spSelect_2:setVisible(false);
    ...
    elseif CBT_BROW == tag then
        ...
        self.m_spSelect:setScaleX(-1)
        -- self.m_spSelect_1:setVisible(false);
        -- self.m_spSelect_2:setVisible(true);
...
function ChatItem:ctor( nType, tabconfig, callFun )
    if 1 == nType then --聊天
        local framename = tabconfig.szItemFrameName or "public/public_chatLine.png"
        ...
        --选中效果 
        local framename = tabconfig.szItemSelectFrameName or "public/public_chat_txtSelect.png"
        if cc.FileUtils:getInstance():isFileExist(framename) then
...
function ChatItem:refreshTextItem(str,sendusernick)
    if nil == self.m_labelChat then
        ...
        self.m_labelChat:setColor(cc.c3b(55,99,155))
