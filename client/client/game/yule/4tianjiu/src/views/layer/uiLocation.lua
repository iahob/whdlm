...
function uiLocation:ctor(...)
    -- cc.FileUtils:getInstance():addSearchPath("game/yule/4tianjiu/res/")
    self:init(...)
...
function uiLocation:init(parent, playerList, playerNum)
    dump(playerList , "定位的玩家列表")
    local csbNode = cc.CSLoader:createNode("localtion/LocationLayer.csb")
...
local pos = {{x=667,y=136},{x=1100,y=375},{x=195,y=375},{x=667,y=600}}
...
function uiLocation:drawLine(src, dst, distance_km)
    ...
    local sprite = cc.Sprite:createWithSpriteFrame(cc.SpriteFrame:create(cmd.RES.."localtion/line_long.png", cc.rect(0,0,length,3)))
