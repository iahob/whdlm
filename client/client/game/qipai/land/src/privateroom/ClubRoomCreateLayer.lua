...
function ClubRoomCreateLayer:ctor( scene,param,level )
    ...
    self.m_nodeScore = spbg:getChildByName("Panel_xz")
    self.m_nodeRule = spbg:getChildByName("Panel_rule")
    ...
    if self.param.tabConfigInfo then
        ...
        for i=1,5 do
            self.lastData[i] = cc.UserDefault:getInstance():getIntegerForKey(GlobalUserItem.tabAccountInfo.dwUserID.."landData"..i, -1)
        end
    end
    ...
    -- 下拉背景1
    local panel = spbg:getChildByName("Panel_3")
    ...
    self.m_editText = {}
    self.m_editImg = {}
    local epoint = {cc.p(459,481),cc.p(959,480),cc.p(832,413),cc.p(963,392),cc.p(878,571)}
...
function ClubRoomCreateLayer:sendGroupConfig()
            ...
            buffer:pushbyte(3)
            -- 底分
            buffer:pushbyte(self.gx_flag and self.m_nSelectScore or 1)
            ...
            if self.m_isCreate then --保存当前配置
                for i=1, 5 do
                    cc.UserDefault:getInstance():setIntegerForKey(GlobalUserItem.tabAccountInfo.dwUserID.."landData"..i, lastData[i])
                end
            end
...
function ClubRoomCreateLayer:updateGongxianMode(gxMode)
    ...
    self.m_spBg:getChildByName("Panel_3"):getChildByName("Text_19"):setString(gxMode==1 and "所有玩家贡献" or "所有赢家贡献")
