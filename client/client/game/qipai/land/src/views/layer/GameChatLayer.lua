...
function GameChatLayer:ctor(netframe, tabconfig)
    ...
    if 200 ~= GlobalUserItem.nCurGameKind then --牛牛走传递的CSB
        csbfile = "public/GameChatLayer.csb"
    end
    --加载csb资源
...
function GameChatLayer:cellSizeForTable( view, idx )
    if CBT_TEXT == self.m_nSelect then
        ...
        if nil ~= record then
            return self.m_common_region_szie.width, 60--self:getTextChatSize(record.strChat)
        end
        return 0,0
...
function ChatItem:refreshTextItem(str,sendusernick)
    if nil == self.m_labelChat then
        ...
        self.m_labelChat:setFontFillColor(cc.c3b(146, 74, 16))
