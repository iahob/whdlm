...
GameLogic.CT_FIVE_STRAIGHT_FLUSH			= 11							--同花顺牌
GameLogic.CT_FIVE_STRAIGHT_FLUSH_FIRST_A	= 12							--A同花顺
GameLogic.CT_FIVE_STRAIGHT_FLUSH_BACK_A	= 13							--A同花顺
...
GameLogic.CT_EX_QIHUA						= 18								--七花
GameLogic.CT_EX_SITAOSANTIAO				= 19								--四套三条
GameLogic.CT_EX_QUANHEIYIDIANHONG			= 20								--全黑一点红
GameLogic.CT_EX_QUANHONGYIDIANHEI			= 21								--全红一点黑
GameLogic.CT_EX_QUANHEI						= 22								--全黑
GameLogic.CT_EX_QUANHONG					= 23								--全红
...
GameLogic.CT_EX_NAME ={
[GameLogic.CT_EX_SANTONGHUA] = "三同花",
[GameLogic.CT_EX_SANSHUNZI] = "三顺子",
[GameLogic.CT_EX_LIUDUIBAN] = "六对半",
[GameLogic.CT_EX_QIHUA] = "七花",
[GameLogic.CT_EX_SITAOSANTIAO] = "四套三条",
[GameLogic.CT_EX_QUANHEIYIDIANHONG] = "全黑一点红",
[GameLogic.CT_EX_QUANHONGYIDIANHEI] = "全红一点黑",
[GameLogic.CT_EX_QUANHEI] = "全黑",
[GameLogic.CT_EX_QUANHONG] = "全红",
...
GameLogic.CT_NAME ={
[GameLogic.CT_SINGLE] = "单牌",
[GameLogic.CT_ONE_DOUBLE] = "对子",
[GameLogic.CT_TWO_DOUBLE] = "两对",
[GameLogic.CT_THREE] = "三条",
[GameLogic.CT_FIVE_MIXED_FLUSH_NO_A] = "顺子",
[GameLogic.CT_FIVE_MIXED_FLUSH_FIRST_A] = "顺子",
[GameLogic.CT_FIVE_MIXED_FLUSH_BACK_A] = "顺子",
[GameLogic.CT_FIVE_FLUSH] = "同花",
[GameLogic.CT_FIVE_THREE_DEOUBLE] = "葫芦",
[GameLogic.CT_FIVE_FOUR_ONE] = "炸弹",
[GameLogic.CT_FIVE_STRAIGHT_FLUSH] = "同花顺",
[GameLogic.CT_FIVE_STRAIGHT_FLUSH_FIRST_A] = "同花顺",
[GameLogic.CT_FIVE_STRAIGHT_FLUSH_BACK_A] = "同花顺",
}
...
function GameLogic:SearchSameCard(cbHandCardData, cbHandCardCount, cbSameCardCount)
	...
	local tempResult = {}
	tempResult.cbResultCard = {}
	tempResult.cbCardCount = pSearchCardResult.cbCardCount
	tempResult.cbSearchCount = pSearchCardResult.cbSearchCount
	for i=1,pSearchCardResult.cbSearchCount do
		table.insert(tempResult.cbResultCard, pSearchCardResult.cbResultCard[i])
	end
	return cbResultCount, tempResult
end
...
function GameLogic:SearchTakeCardType(cbHandCardData, cbHandCardCount, cbSameCount, cbTakeCardCount)
	...
	if cbSameCardResultCount > 0 then
		table.sort( SameCardResult.cbResultCard, function ( a,b )
			return GameLogic:GetCardLogicValue(a[1]) > GameLogic:GetCardLogicValue(b[1])
		end )

		local cbTakeCardResultCount, TakeCardResult = GameLogic:SearchSameCard(cbHandCardData, cbHandCardCount, cbTakeCardCount)
		table.sort( TakeCardResult.cbResultCard, function ( a,b )
			return GameLogic:GetCardLogicValue(a[1]) > GameLogic:GetCardLogicValue(b[1])
		end )
		dump(TakeCardResult, "the take result ====	", 6)

		--可以组成带牌
		if cbTakeCardResultCount > 0 then
			for i=cbSameCardResultCount,1,-1 do
				for j=cbTakeCardResultCount,1,-1 do
					--搜索三条：AAA
					--搜索一对：AA 33 66 99
					--忽略组合 AAAAA
...
--比较扑克
function GameLogic:CompareCard(bInFirstList, bInNextList, bFirstCount, bNextCount, bComperWithOther,cbTonghua)
	...
	--同段比较
	print(cbNextType.."================bComperWithOther================"..cbFirstType)
	if true == bComperWithOther then
		if cbNextType == cbFirstType then
			...
			elseif cbFirstType == GameLogic.CT_FIVE_MIXED_FLUSH_FIRST_A or 
				...
				return GameLogic.enCREqual
			elseif cbFirstType == GameLogic.CT_FIVE_MIXED_FLUSH_NO_A then 	--没A杂顺
				for i=1,5 do
					if GameLogic:GetCardLogicValue(bNextList[i]) > GameLogic:GetCardLogicValue(bFirstList[i]) then
						return GameLogic.enCRGreater
					elseif GameLogic:GetCardLogicValue(bNextList[i]) < GameLogic:GetCardLogicValue(bFirstList[i]) then
			...
			elseif cbFirstType == GameLogic.CT_FIVE_FLUSH then --同花五牌

				if cbTonghua == 0 then --先比点数
...
function GameLogic:getSingleCard(cbCardData,removeCard,isSingle)
	local value,cbTempCardData = GameLogic:RemoveCard(removeCard,#removeCard,cbCardData,#cbCardData)
	if not value then
		return  nil
	end
	...
	--分析扑克
	local Distributing = GameLogic:AnalysebDistributing(cbTempCardData, #cbTempCardData)
	local getData = nil
	for i=2,14 do
		if i==14 then
			i=1
		end
		if (isSingle and Distributing.cbDistributing[i][5] ==1) or not isSingle then
			for j=1,4 do
				if Distributing.cbDistributing[i][j] >=1 then
					getData = GameLogic:MackCardData(i, j)
					return getData
				end
			end
	...
	if not getData and isSingle then
		return GameLogic:getSingleCard(cbTempCardData,{},false)
	end
	return getData
end
...
function GameLogic:sortAllCarsType(cbCardData,cbCardCount)
	...
	if SearchCardCount > 0 then
		...
		table.sort(SearchCardResult.cbResultCard,function( a,b )
			return self:GetCardLogicValue(a[1]) < self:GetCardLogicValue(b[1])
		end)

		dump(SearchCardResult.cbResultCard, "jack	", 6)
		...
		for i=1,SearchCardCount do
			...
			for n=1,3 do
				local cardSingle = GameLogic:getSingleCard(cbCardData,two,true)
				if cardSingle then
					table.insert(two, cardSingle)
				end
			end
	...
	if SearchCardCount > 1 then
		...
		for i=1, SearchCardCount-1 do
			for j=i+1, SearchCardCount do
				...
				local cardSingle = GameLogic:getSingleCard(cbCardData,double,true)
				if cardSingle then
					table.insert(double, cardSingle)
				end
	...
	--三条
	SearchCardCount,SearchCardResult = GameLogic:SearchSameCard(cbCardData, cbCardCount, 3)
	if SearchCardCount > 0 then
		...
		for i=1, SearchCardCount do
			...
			for n=1,2 do
				local cardSingle = GameLogic:getSingleCard(cbCardData,three,true)
				if cardSingle then
					table.insert(three, cardSingle)
				end
			end

			table.insert(SortResult.cbThreeList, three)
		end
	end
