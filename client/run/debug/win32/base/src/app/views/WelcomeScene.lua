...
local URL_REQUEST =  appdf.GAME_DEBUG_ADDRESS
...
function WelcomeScene:onCreate()
	...
	if nil ~= sp then
		sp:setPosition(display.cx,appdf.HEIGHT/2)
		--self:addChild(sp)
	end

    local csbnode = cc.CSLoader:createNode("base/res/splash.csb");

    local bg = ccui.Layout:create()
		:setContentSize(display.width,750) --这个是资源设计尺寸
        :setColor(cc.c3b(255,255,255))
        :setBackGroundColorType(LAYOUT_COLOR_SOLID)
        :setOpacity(255)
    self:addChild(bg)
    csbnode:setPositionX(display.cx - 1334/2)
    bg:addChild(csbnode)
	--标签
	...
	if nil ~= sp then
		sp:setPosition(display.cx, 750/2)
		self:addChild(sp)
...
--	end
	...
	local loading_bg = cc.Sprite:create("loding_bg.png"):move(display.cx, 105):addTo(self.m_progressLayer)
	--总进度
