                             ...
                             const int &nMaskingNo /*= -1*/)
{
    ...
    if (nullptr != qr && qr->init() && qr->initQrNode(content, nNodeSize, nQrSize, nLevel, nVersion, bAutoExtent, nMaskingNo))
    {
        qr->autorelease();
        return qr;
    }
    ...
    do
    {
        ...
        if (m_QREncode.EncodeData(nLevel, nVersion, bAutoExtent, nMaskingNo, (char*)content.c_str()))
        {
            ...
            {          
                ...
                this->setContentSize(Size(nNodeSize / m_fScaleRate, nNodeSize / m_fScaleRate));
                this->addChild(qrNode);
                qrNode->setName(QR_NODE_NAME);
                qrNode->setPositionY(nNodeSize / m_fScaleRate);
                bRes = true;
            }
        }
    } while (false);
...
int register_all_QrNode()
{
    ...
    tolua_cclass(tolua_S,"QrNode","cc.QrNode","ccui.Widget",nullptr);
