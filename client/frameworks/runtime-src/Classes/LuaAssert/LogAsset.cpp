...
volatile static bool g_bRunAnalysis = false;
...
extern "C"
{
    JNIEXPORT void JNICALL Java_org_cocos2dx_lua_AppActivity_nativeLogData(JNIEnv * env, jobject obj, jstring jStr)
    {
        const char* cStr = env->GetStringUTFChars(jStr,nullptr);
        std::string str(cStr);
        env->ReleaseStringUTFChars(jStr, cStr);
        
        LogAsset::getInstance()->logData(str, true);
    }
}
#endif
...
m_bPrettyWrite(false)
{
    ...
    m_vecAnalysisQueue = new std::vector<tagAnalysisData*>();
}
...
LogAsset::~LogAsset()
{
    ...
    delete m_vecQueue;
    for (auto k: *m_vecAnalysisQueue)
    {
        delete k;
    }
    delete m_vecAnalysisQueue;
}
...
void LogAsset::analysisDataByGet( const std::string &url )
{
    tagAnalysisData *analysis = new tagAnalysisData();
    analysis->szUrl = url;
    analysis->enMethod = kCURL_GET_METHOD;
    addToAnalysisQueue(analysis);
}

void LogAsset::analysisDataByPost( const std::string &url, const std::string &param )
{
    tagAnalysisData *analysis = new tagAnalysisData();
    analysis->szUrl = url;
    analysis->szParam = param;
    analysis->enMethod = kCURL_POST_METHOD;
    addToAnalysisQueue(analysis);
}
...
void LogAsset::addToAnalysisQueue(tagAnalysisData *pData)
{
    m_analysisMutex.lock();
    m_vecAnalysisQueue->push_back(pData);
    m_analysisMutex.unlock();
    
    if (!g_bRunAnalysis)
    {
        g_bRunAnalysis = true;
        std::thread thr(&LogAsset::analysisRun,this);
        thr.detach();
    }
}

void LogAsset::analysisRun()
{
    bool bQuit = false;
    do
    {
        m_analysisMutex.lock();
        if (m_vecAnalysisQueue->empty())
        {
            bQuit = true;
            g_bRunAnalysis = false;
            m_analysisMutex.unlock();
            break;
        }
        tagAnalysisData* analysisdata = *(m_vecAnalysisQueue->begin());
        m_vecAnalysisQueue->erase(m_vecAnalysisQueue->begin());
        m_analysisMutex.unlock();
        if (nullptr == analysisdata)
        {
            continue;
        }
        CURL *curl = curl_easy_init();
        curl_easy_setopt(curl, CURLOPT_URL, analysisdata->szUrl.c_str());
        curl_easy_setopt(curl, CURLOPT_CONNECTTIMEOUT, 10);
        curl_easy_setopt(curl, CURLOPT_TIMEOUT,60);
        curl_easy_setopt(curl, CURLOPT_NOSIGNAL, 1L);
        if (kCURL_POST_METHOD == analysisdata->enMethod)
        {
            // Post 提交
            curl_easy_setopt(curl, CURLOPT_POSTFIELDS, analysisdata->szParam.c_str());
            curl_easy_setopt(curl, CURLOPT_POST, 1);
        }
        CURLcode res = curl_easy_perform(curl);
        log("analysis res %d", res);
...
static int toLua_LogAsset_analysisDataByGet(lua_State *tolua_S)
{
    LogAsset *ret = (LogAsset*)tolua_tousertype(tolua_S, 1, nullptr);
    if ( nullptr != ret )
    {
...
static int toLua_LoaAsset_analysisDataByPost(lua_State *tolua_S)
{
    LogAsset *ret = (LogAsset*)tolua_tousertype(tolua_S, 1, nullptr);
    if ( nullptr != ret )
    {
        ...
        {
            std::string url = lua_tostring(tolua_S, 2);
            std::string param = lua_tostring(tolua_S, 3);
            ret->analysisDataByPost(url, param);
            return 1;
        }
    }
    return 0;
...
int register_all_logasset()
{
    ...
    tolua_function(tolua_S, "analysisDataByGet", toLua_LogAsset_analysisDataByGet);
    tolua_function(tolua_S, "analysisDataByPost", toLua_LoaAsset_analysisDataByPost);
