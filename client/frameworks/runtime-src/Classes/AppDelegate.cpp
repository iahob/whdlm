...
#include "LuaAssert/Gif/CacheGif.h" // gif 
#include "LuaAssert/Gif/InstantGif.h" // gif
#include "LuaAssert/PageTurn.h"
...
static unsigned char* getBMPFileData(const std::string &path, bool &haveData)
{
    ...
    unsigned char *bytecs = new(std::nothrow) unsigned char[96 * 96 * 4 + 1];
    if (nullptr == bytecs)
    {
    	log("new unsigned char fail");
    	return nullptr;
    }
    int idx1 = 0;
    int idx2 = 0;
...
// 读取游戏回放消息文件
static int toLua_AppDelegate_readGameVideoFile(lua_State* tolua_S)
{
    auto argc = lua_gettop(tolua_S);
    if (1 == argc)
    {
    	std::string filepath = lua_tostring(tolua_S, 1);
    	if (FileUtils::getInstance()->isFileExist(filepath))
    	{
    		// 读取数据
    		Data pData = FileUtils::getInstance()->getDataFromFile(filepath);
            ssize_t size = pData.getSize();
            if (0 != size)
            {
            	CCmd_Data *buffer = CCmd_Data::create(size);
                buffer->PushByteData(pData.getBytes(), size);
                buffer->ResetCurrentIndex();
...
static int toLua_AppDelegate_createInstantGif(lua_State* tolua_S)
{
	auto argc = lua_gettop(tolua_S);
    if (1 == argc)
    {
        ...
        GifBase *gif = InstantGif::create(gifpath.c_str());
        if (nullptr != gif)
        {
        	cocos2d::log("create ");
...
static int register_all_packages()
{
    ...
    lua_register(tolua_S,"readGameVideoFile", toLua_AppDelegate_readGameVideoFile);
    lua_register(tolua_S,"createInstantGif", toLua_AppDelegate_createInstantGif);
    lua_register(tolua_S,"createCacheGif", toLua_AppDelegate_createCacheGif);
...
bool AppDelegate::applicationDidFinishLaunching()
{
	//进行资源的加解密
#if CC_TARGET_PLATFORM != CC_PLATFORM_WIN32
	unsigned char ivec[] = { 0x08,0xc6,0xeb,0xde,0x20,0x78,0x7b,0xa1,0x8f,0x7e,0xfa,0x82,0x13,0xbb,0x37,0x0c, };
	unsigned char secret[] = { 0xdc,0x6a,0xe9,0xd5,0x4a,0x8b,0xaf,0x62,0x0c,0xcb,0x2c,0x22,0xb1,0xfe,0x3b,0x39,0x03,0x99,0xab,0xe3,0x3d,0x2f,0x52,0x1e,0x64,0xcf,0xea,0xa9,0x96,0xa6,0x94,0x4d, };
...
#if CC_TARGET_PLATFORM != CC_PLATFORM_WIN32
    #if CC_TARGET_PLATFORM == CC_PLATFORM_ANDROID
    	CrashReport::initCrashReport("cff3fb703a", isDebug);
    #elif CC_TARGET_PLATFORM == CC_PLATFORM_IOS
    	CrashReport::initCrashReport("586b84be84", isDebug);
    #endif    
...
#if CC_TARGET_PLATFORM != CC_PLATFORM_WIN32 
        //加密
    engine->getLuaStack()->setLuaStackCryTo(this);
#endif
...
void AppDelegate::applicationWillEnterForeground()
{
    ...
    for (auto it = m_cachedBmpTex.cbegin(); it != m_cachedBmpTex.cend(); ++it)
    {
	    ...
	    {
	    	if ( false == it->second->initWithData(data, 96 * 96 * 4, Texture2D::PixelFormat::RGBA8888, 96, 96, cocos2d::Size(96, 96)) )
	    	{
	    		it->second->release();
	    		m_cachedBmpTex.erase(it++);
	    	}
...
unsigned char *AppDelegate::script_encrypt(unsigned char *data, int data_len, int& result_len)
{
    return 0;
}

unsigned char *AppDelegate::script_decrypt(unsigned char *data, int data_len, int& result_len)
{
    IMCKernel *kernel = GetMCKernel();
    if(kernel)
    {
        return kernel->script_decrypt(data, data_len, result_len);
    }
    else
    {
        CCLOG("GlobalUpdate m_pClientKernel is null");
