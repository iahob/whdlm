...
BEGIN
	...
	SELECT @Insure=InsureScore FROM  WHQJTreasureDB.dbo.GameScoreInfo WHERE UserID=@dwUserID
...
-- 输出变量
	SELECT @UserID AS UserID, @GameID AS GameID, @GroupID AS GroupID, @NickName AS NickName, @UnderWrite AS UnderWrite,  @FaceID AS FaceID, 0 AS Dimand,
		...
		@DwellingPlace AS DwellingPlace,@PostalCode AS PostalCode,0 AS GroupIDArray,0 AS DayPlayTime,0 AS DayWinLostScore
END
...
BEGIN
	...
	BEGIN
		...
		DECLARE @LockKindID INT
		DECLARE @LockServerID INT
		SELECT @LockKindID=KindID, @LockServerID=ServerID FROM  WHQJTreasureDB.dbo.GameScoreLocker WHERE UserID=@dwUserID --AND ServerID=@wServerID
...
-- 进入记录
	...
	--查询每日
	SELECT @DayPlayTime=DayPlayTimeCount FROM WHQJAccountsDB.dbo.AccountsDayPlayTime WHERE UserID=@dwUserID AND DATEDIFF(dd,CollectDate,GETDATE())=0
	...
	SELECT @HistoryKindIDWin=SUM(Score) FROM WHQJGameScoreDB.dbo.RecordDrawScore WHERE UserID=@UserID AND DrawID IN (SELECT DrawID FROM WHQJGameScoreDB.dbo.RecordDrawInfo)
	...
	SELECT @UserID AS UserID, @GameID AS GameID, @GroupID AS GroupID, @NickName AS NickName, @UnderWrite AS UnderWrite,  @FaceID AS FaceID, @Dimand AS Dimand,
		...
		@IntegralCount AS IntegralCount,@AgentID AS AgentID,@QQ AS QQ, @EMail AS EMail, @SeatPhone AS SeatPhone, @MobilePhone AS MobilePhone,
		@DwellingPlace AS DwellingPlace,@PostalCode AS PostalCode,@GroupIDArray AS GroupIDArray,@HistoryKindIDWin AS HistoryKindIDWin,@DayPlayTime AS DayPlayTime,0 AS DayWinLostScore
END
