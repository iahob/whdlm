...
BEGIN	
	...
	SET @ReturnValue=0
	SELECT @UserRight=UserRight FROM WHQJAccountsDB..AccountsInfo WHERE UserID=@dwUserID 
	SELECT @UserRight=@UserRight|UserRight FROM GameScoreInfo WHERE UserID=@dwUserID

	IF EXISTS(SELECT * FROM WHQJTreasureDB..GameScoreInfo WHERE UserID=@dwUserID)
	BEGIN
		...
		ELSE
		BEGIN
			UPDATE WHQJTreasureDB..GameScoreInfo SET Score=Score-@dwMatchFee WHERE UserID=@dwUserID
