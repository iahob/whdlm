...
INT CUserListControl::SortItemData(LPARAM lParam1, LPARAM lParam2, WORD wColumnIndex, bool bAscendSort)
{
	...
	case UD_IS_ANDROID:		//AI？
		{			
			//获取变量
			bool bAndroid1 = pIClientUserItem1->IsAndroidUser();
			bool bAndroid2 = pIClientUserItem2->IsAndroidUser();

			//对比数据
			INT nResult = bAndroid1 > bAndroid2 ? SORT_AFTER : SORT_FRONT;
			return (bAscendSort == true) ? nResult : -nResult;
		}
	...
	case UD_ONLINE_TIME:	//在线时长
		{
			//获取变量
			DWORD dwLogonTime1 = pIClientUserItem1->GetLogonTime();
			DWORD dwLogonTime2 = pIClientUserItem2->GetLogonTime();

			//对比数据
			INT nResult = dwLogonTime1>dwLogonTime2 ? SORT_AFTER : SORT_FRONT;
			return (bAscendSort == true) ? nResult : -nResult;
		}
	case UD_CARRY_GRADE:	//携带等级
		{
			//获取变量
			WORD wCarryGrade1 = pIClientUserItem1->GetAndroidGrade() & AI_CARRY_GRADE_MASK;
			WORD wCarryGrade2 = pIClientUserItem2->GetAndroidGrade() & AI_CARRY_GRADE_MASK;

			//对比数据
			INT nResult = wCarryGrade1>wCarryGrade2 ? SORT_AFTER : SORT_FRONT;
			return (bAscendSort == true) ? nResult : -nResult;
		}
	case UD_LEAVE_GRADE:	//离开等级
		{
			//获取变量
			WORD wLeaveGrade1 = pIClientUserItem1->GetAndroidGrade() & AI_LEAVE_GRADE_MASK;
			WORD wLeaveGrade2 = pIClientUserItem2->GetAndroidGrade() & AI_LEAVE_GRADE_MASK;

			//对比数据
			INT nResult = wLeaveGrade1>wLeaveGrade2 ? SORT_AFTER : SORT_FRONT;
			return (bAscendSort == true) ? nResult : -nResult;
		}
	}
...
VOID CUserListControl::ConstructString(VOID * pItemData, WORD wColumnIndex, LPTSTR pszString, WORD wMaxCount)
{
	...
	case UD_IS_ANDROID:									//AI？
		{
			bool bAndroid = pIClientUserItem->IsAndroidUser();
			lstrcpyn(pszString, (bAndroid == true) ? TEXT("AI") : TEXT("玩家"), wMaxCount);
			break;
		}
	...
	case UD_TABLE:			//桌子号码
		{
			...
			{
				//无效
				TCHAR szInValidTable[16] = TEXT("无效");
				_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%s"), szInValidTable);
			}
	...
	case UD_CHAIR:			//椅子号码
		{
			...
			{
				_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%ld"), wChairID + 1);
			}
			else
			{
				//无效
				TCHAR szInValidChair[16] = TEXT("无效");
				_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%s"), szInValidChair);
			}
	...
	case UD_GAME_LEVEL:		//游戏等级
		{
			...
			//获取等级
			if (CServerRule::IsAllowAvertDebugMode(m_dwServerRule) && (pIClientUserItem->GetUserID()!=m_dwMySelfUserID))
	...
	case UD_ONLINE_TIME:	//在线时长
		{
			if (pIClientUserItem->GetLogonTime() != 0 && pIClientUserItem->IsAndroidUser())
			{
				DWORD dwCurTickCount = (DWORD)time(NULL);
				DWORD dwOnlineTimeTickCount = dwCurTickCount - pIClientUserItem->GetLogonTime();
				DWORD dwOnlineTimeMin = dwOnlineTimeTickCount / 60;

				_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%d Min"), dwOnlineTimeMin);
			}
	...
	case UD_CARRY_GRADE:	//携带等级
		{
			WORD wAndroidGrade = AI_CARRY_GRADE_NONE | AI_LEAVE_GRADE_NONE;

			if (pIClientUserItem->IsAndroidUser() && pIClientUserItem->GetAndroidGrade() != wAndroidGrade)
			{
				TCHAR szCarryGradeDesc[AI_CARRY_GRADE][128] = { TEXT("等级A"), TEXT("等级B"), TEXT("等级C"), TEXT("等级D") };

				BYTE cbCarryStandardVal = AI_CARRYGRADE_STANDARD;
				WORD wCarryGrade = pIClientUserItem->GetAndroidGrade() & AI_CARRY_GRADE_MASK;
				...
				if (cbCarryGradeIndex >= 0 && cbCarryGradeIndex < AI_CARRY_GRADE)
				{
					_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%s"), szCarryGradeDesc[cbCarryGradeIndex]);
				}
			}
	...
	case UD_LEAVE_GRADE:	//离开等级
		{
			...
			if (pIClientUserItem->IsAndroidUser() && pIClientUserItem->GetAndroidGrade() != wAndroidGrade)
			{
				TCHAR szLeaveGradeDesc[AI_LEAVE_GRADE][128] = { TEXT("次优先"), TEXT("最优先"), TEXT("最高优先") };

				BYTE cbLeaveStandardVal = AI_LEAVEGRADE_STANDARD;
				WORD wLeaveGrade = pIClientUserItem->GetAndroidGrade() & AI_LEAVE_GRADE_MASK;
				wLeaveGrade >>= 4;
				...
				if (cbLeaveGradeIndex >= 0 && cbLeaveGradeIndex < AI_LEAVE_GRADE)
				{
					_sntprintf_s(pszString, wMaxCount, wMaxCount, TEXT("%s"), szLeaveGradeDesc[cbLeaveGradeIndex]);
				}
			}
