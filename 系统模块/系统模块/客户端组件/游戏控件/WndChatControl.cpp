...
VOID CWndChatControl::OnPaint()
{
	...
	if (m_pIClientUserItem!=NULL)
	{
		...
		_sntprintf_s(szString,CountArray(szString),TEXT("目标用户“%s”"),m_pIClientUserItem->GetNickName());
...
VOID CWndChatControl::OnBnClickedControl()
{
	...
	if (m_pIClientUserItem!=NULL)
	{
		//构造菜单
		TCHAR szMenuString[64]=TEXT("");
		_sntprintf_s(szMenuString,CountArray(szMenuString),TEXT("取消与 [ %s ] 交谈"),m_pIClientUserItem->GetNickName());
