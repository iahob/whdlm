...
VOID CUserItemElement::ConstructNameCard(IClientUserItem * pIClientUserItem, IGameLevelParser * pIGameLevelParser, CImage&ImageNameCard)
{
	...
	_sntprintf_s(szString,CountArray(szString),TEXT("昵称：%s [ %s ]"),pIClientUserItem->GetNickName(),pszUserOrder);
	...
	_sntprintf_s(szString,CountArray(szString),TEXT("级别：%s [ %s ]"),pszGameLevel,pszKindName);
	pDC->DrawText(szString,lstrlen(szString),&rcGameLevel,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);

	//QQ号码	
	_sntprintf_s(szString,CountArray(szString),TEXT("  QQ：%s"),pUserInfo->szQQ);
	pDC->DrawText(szString,lstrlen(szString),&rcExperience,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);

	//手机号码
	_sntprintf_s(szString,CountArray(szString),TEXT("手机：%s"),pUserInfo->szMobilePhone);
	pDC->DrawText(szString,lstrlen(szString),&rcUserGameID,DT_VCENTER|DT_SINGLELINE|DT_END_ELLIPSIS);

	//个性签名
	_sntprintf_s(szString,CountArray(szString),TEXT("签名：%s"),pUserInfo->szUnderWrite);
...
bool CUserItemElement::EfficacyEmail(LPCTSTR pszEmail, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount,TEXT("邮箱包含非法字符串，请重新输入！"));
		return false;
	}
...
bool CUserItemElement::EfficacyAccounts(LPCTSTR pszAccounts, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	for (INT i=0;i<nCharLength;i++)
	{
		if (EfficacyChineseCharacter(pszAccounts[i])==false)
		{
			...
			//非法字符
			_sntprintf_s(szDescribe, wMaxCount, wMaxCount,TEXT("游戏账号必须由字母、数字、下划线和汉字组成！"));
	...
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("游戏帐号必须大于%ld位以上，请重新输入！"), LEN_LESS_ACCOUNTS);
		return false;
	}
	...
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount,TEXT("游戏帐号包含非法字符串，请重新输入！"));
		return false;
	}
...
bool CUserItemElement::EfficacyNickName(LPCTSTR pszNickName, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	if (nCharLength<LEN_LESS_NICKNAME)
	{
		...
		{
			_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("游戏昵称必须大于%ld位以上，请重新输入！"), LEN_LESS_NICKNAME);
			return false;
		}
	}
	...
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount,TEXT("游戏昵称包含非法字符串，请重新输入！"));
		return false;
	}
...
bool CUserItemElement::EfficacyPassword(LPCTSTR pszPassword, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	if(nCharLength == 0)
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("密码不能为空，请重新输入！"), LEN_LESS_PASSWORD);
		return false;
	}
	...
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("密码必须大于 %ld 位以上，请重新输入！"), LEN_LESS_PASSWORD);
		return false;
	}
	...
	for (INT i=0;i<nCharLength;i++)
	{
		...
		//非法字符
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("密码必须由字母、数字、下划线组成！"), LEN_LESS_ACCOUNTS);
	...
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount,TEXT("密码包含非法字符串，请重新输入！"));
		return false;
	}
...
bool CUserItemElement::EfficacyPassPortID(LPCTSTR pszPassPortID, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	if(nCharLength==0L)
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("身份证号不能为空，请重新输入！"), LEN_LESS_PASSWORD);
		return false;
	}
...
bool CUserItemElement::EfficacyCompellation(LPCTSTR pszCompellation, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	if(nCharLength == 0)
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("真实姓名不能为空，请重新输入！"), LEN_LESS_ACCOUNTS);
	...
	for (INT i=0;i<nCharLength;i++)
	{
		...
		{
			//非法字符
			_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("真实姓名只能由汉字组成，请重新输入！"), LEN_LESS_ACCOUNTS);
	...
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("真实姓名字符必须小于等于%ld位，请重新输入！"), LEN_COMPELLATION);
		return false;
	}
	...
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("真实姓名字符必须大于等于%ld位，请重新输入！"), LEN_LESS_COMPELLATION);
		return false;
	}	
	...
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount,TEXT("真实姓名包含非法字符串，请重新输入！"));
		return false;
	}
...
bool CUserItemElement::EfficacyQQ(LPCTSTR pszQQ, TCHAR szDescribe[], WORD wMaxCount)
{
	...
	for (int i=0;i<nQQCount;i++)
	{
		...
		{
			_sntprintf_s(szDescribe, wMaxCount, wMaxCount,TEXT("QQ号码不正确，请重新输入！"));
			return false;
		}
	}
	...
	if ((nQQCount)<5)
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("QQ号码必须大于等于%ld位，请重新输入！"), 5);
		return false;
	}	

	if ((nQQCount)>12)
	{
		_sntprintf_s(szDescribe, wMaxCount, wMaxCount, TEXT("QQ号码必须小于%ld位，请重新输入！"), 12);
		return false;
	}	
...
VOID CUserItemElement::DrawNumber(CDC * pDC,CPngImage * pImageNumber,INT nXPos,INT nYPos,LPCTSTR pszNumber,INT nNumber,LPCTSTR pszFormat,UINT nFormat)
{
	...
	_sntprintf_s(szValue,CountArray(szValue),pszFormat,nNumber);
