...
void  CDlgTrumpetItem::UpdateControlSurface()
{
	...
	if(pButtonTrumpet->GetCheck()==true)
	{
		...
		//计算价格
		DOUBLE lPrice = 1;//(DOUBLE)pPropertyInfo->lPropertyGold;
...
void CDlgTrumpetItem::OnBnClickedTrumpet()
{
	...
	_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("剩余小喇叭<c #FF6600>%d</c>个，大喇叭<c #FF6600>%d</c>个。"), m_dwTrumpetCount, m_dwTyphonCount,TRUMPET_MAX_CHAR);	
