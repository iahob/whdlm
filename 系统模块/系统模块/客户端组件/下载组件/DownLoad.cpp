...
bool CDownLoad::PerformDownLoad(LPCTSTR pszUrl,DWORD dwDownLoadPos,IUnknownEx * pIUnknownEx)
{
	...
	//_sntprintf_s(szWholeUrl,CountArray(szWholeUrl),TEXT("%s?x=%d"),pszUrl,GetTickCount());
	...
	case HTTP_STATUS_REDIRECT_METHOD:	//目标跳转
		{
			...
			if (szLocation[0]=='/')
			{
				...
				//添加参数
				//_sntprintf_s(m_DownLoadInfo.szUrlPath,CountArray(m_DownLoadInfo.szUrlPath),TEXT("%s?x=%I64d"),m_DownLoadInfo.szUrlPath,GetTickCount());
			...
			else
			{
				...
				//添加参数
				/*TCHAR szWholeUrl[256]=TEXT("");
				_sntprintf_s(szWholeUrl,CountArray(szWholeUrl),TEXT("%s"),((CA2CT)szLocation));
				_sntprintf_s(szWholeUrl,CountArray(szWholeUrl),TEXT("%s?x=%d"),szWholeUrl,GetTickCount());*/
