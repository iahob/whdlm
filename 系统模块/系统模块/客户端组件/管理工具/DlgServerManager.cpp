...
VOID CDlgServerManager::OnOK()
{
	...
	//隐藏信息
	CButton * pAllowAvertDebugMode=(CButton *)GetDlgItem(IDC_ALLOW_AVERT_DEBUG_MODE);
	CServerRule::SetAllowAvertDebugMode(ServerOption.ServerOptionInfo.dwServerRule,(pAllowAvertDebugMode->GetCheck()==BST_CHECKED));
...
//消息解释
BOOL CDlgServerManager::PreTranslateMessage(MSG * pMsg)
{
	//按键过虑
...
VOID CDlgServerManager::SetServerOptionInfo(tagServerOptionInfo & ServerOptionInfo, DWORD dwRuleMask)
{
	...
	//隐藏信息
	bool bAllowAvertDebugMode=CServerRule::IsAllowAvertDebugMode(ServerOptionInfo.dwServerRule);
	((CButton *)GetDlgItem(IDC_ALLOW_AVERT_DEBUG_MODE))->EnableWindow(CServerRule::IsAllowAvertDebugMode(dwRuleMask));
	((CButton *)GetDlgItem(IDC_ALLOW_AVERT_DEBUG_MODE))->SetCheck((bAllowAvertDebugMode)?BST_CHECKED:BST_UNCHECKED);
