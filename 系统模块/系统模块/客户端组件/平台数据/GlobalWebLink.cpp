...
LPCTSTR CGlobalWebLink::GetValidateLink()
{
	...
	_sntprintf_s(szSignature,CountArray(szSignature),TEXT("%d%s%d%s"),dwUserID,pGlobalUserData->szDynamicPass,dwTickCount,szValidateKey);
	...
	//生成链接
	_sntprintf_s(m_szValidateLink,CountArray(m_szValidateLink),szValidateLink,dwUserID,dwTickCount,szMD5Signature);
...
UINT CGlobalWebLink::ReadFileValue(LPCTSTR pszKeyName, LPCTSTR pszItemName, UINT nDefault)
{
	...
	_sntprintf_s(szServerInfoPath,CountArray(szServerInfoPath),TEXT("%s\\PlatformLink.INI"),szDirectory);
