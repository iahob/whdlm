...
bool CServiceUnits::ConcludeService()
{
	...
	//
	if (m_GameServiceOption.wKindID == FISH_3D || m_GameServiceOption.wKindID == FISH_LK || m_GameServiceOption.wKindID == FISH_DR || m_GameServiceOption.wKindID == CANDY_PARTY_KIND_ID)
	{
		bImmediate = true;
	}

	//校验所有桌子游戏是否结束
	if (bImmediate && m_AttemperEngineSink.CheckUserPlaying())
	{
		//弹出提示
		int nResult = AfxMessageBox(TEXT("还有玩家在游戏中，是否通知游戏中的玩家写分"), MB_YESNOCANCEL);

		if (nResult == IDYES)//通知玩家写分
		{
			TCP_Command Command;
			Command.wMainCmdID = MDM_GF_FRAME;
			Command.wSubCmdID = SUB_GF_WRITE_REMAIN_SCORE;

			m_AttemperEngineSink.OnEventTCPNetworkRead(Command, NULL, 0, 0xFFFFFFFF);
			return false;
		}
		else if (nResult == IDNO)//直接停止服务
		{

		}
		else if (nResult == IDCANCEL)//取消停止服务
		{
			return false;
		}
	}
	...
	if (m_AndroidTCPSocketService.GetInterface() != NULL) m_AndroidTCPSocketService->ConcludeService();
	...
	//最小积分
	if (m_GameServiceOption.wServerType&GAME_GENRE_GOLD && m_GameServiceOption.lMinTableScore > m_GameServiceOption.lServiceScore)
	{
...
bool CServiceUnits::CreateServiceDLL()
{
	...
	//AI服务网络组件
	if ((m_AndroidTCPSocketService.GetInterface() == NULL) && (m_AndroidTCPSocketService.CreateInstance() == false))
	{
		CTraceService::TraceString(m_AndroidTCPSocketService.GetErrorDescribe(), TraceLevel_Exception);
		return false;
	}
...
bool CServiceUnits::InitializeService()
{
	...
	//AI分配服务器
	if (m_AndroidTCPSocketService->SetServiceID(NETWORK_AI_DISTRIBUTE_SERVER) == false) return false;
	if (m_AndroidTCPSocketService->SetTCPSocketEvent(pIAttemperEngine) == false) return false;
	...
	m_AttemperEngineSink.m_pAndroidITCPSocketService = m_AndroidTCPSocketService.GetInterface();
...
bool CServiceUnits::StartKernelService()
{
	...
	//AI分配引擎
	if (m_AndroidTCPSocketService->StartService() == false)
	{
		//ASSERT(FALSE);
		return false;
	}
...
bool CServiceUnits::RectifyServiceParameter()
{
	...
	if ((m_GameServiceOption.cbDistributeRule&DISTRIBUTE_ALLOW)!=0)
	{
		...
		//设置
		CServerRule::SetAllowAvertDebugMode(m_GameServiceOption.dwServerRule,true);		
	...
	//设置支持大联盟
	if ((((m_GameServiceOption.wServerType)& GAME_GENRE_PERSONAL) != 0) && (m_GameServiceAttrib.wChairCount == MAX_CHAIR))
	{
		CServerRule::SetSuportGroup(m_GameServiceOption.dwServerRule, true);
	}
	...
	//大联盟百人游戏
	if ((((m_GameServiceOption.wServerType)& GAME_GENRE_PERSONAL) != 0) && CServerRule::IsSuportGroup(m_GameServiceOption.dwServerRule) && (m_GameServiceAttrib.wChairCount == MAX_CHAIR))
	{
		wMaxPlayer = MAX_CHAIR; 
		m_GameServiceOption.wMaxPlayer = wMaxPlayer + RESERVE_USER_COUNT;
	}
	else
	{
		wMaxPlayer = m_GameServiceOption.wTableCount*m_GameServiceAttrib.wChairCount;
...
LRESULT CServiceUnits::OnUIControlRequest(WPARAM wParam, LPARAM lParam)
{
	...
	case UI_AI_DISTRIBUTE_SERVER_RESULT:
		{
			//效验消息
			//ASSERT(DataHead.wDataSize == sizeof(CP_ControlResult));
			if (DataHead.wDataSize != sizeof(CP_ControlResult)) return 0;
	...
	case UI_SERVICE_CONFIG_RESULT:	//配置结果
		{
			...
			if ((m_ServiceStatus!=ServiceStatus_Service)&&(pControlResult->cbSuccess==ER_SUCCESS))
			{
				...
				if((m_GameServiceOption.wServerType&GAME_GENRE_PERSONAL)!=0)
				{
					...
					//连接AI分配服务
					SendControlPacket(CT_CONNECT_AI_DISTRIBUTE_SERVER, NULL, 0);
				}
...
{
	if (m_GameServiceOption.wServerID == 0) return false;
	...
	try
	{
		...
		//读取列表
		if (PlatformDBAide.ExecuteProcess(TEXT("GSP_GS_UnLock_Server"), true) != DB_SUCCESS)
		{
			//获取信息
			PlatformDBAide.GetParameter(TEXT("@strErrorDescribe"), szDescribeString, CountArray(szDescribeString));
	...
	catch (IDataBaseException * pIException)
	{
		...
		//错误提示
		AfxMessageBox(szInfo, MB_ICONERROR);
