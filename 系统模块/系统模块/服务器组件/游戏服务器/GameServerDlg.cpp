...
BEGIN_MESSAGE_MAP(CGameServerDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_SERVICE_LOG, OnBnClickedLogService)
...
VOID CGameServerDlg::OnBnClickedOpenServer()
{
	...
	if (DlgServerItem.m_ModuleInitParameter.GameServiceOption.wServerType == GAME_GENRE_PERSONAL)
	{
		//游戏类型
		byte cbPlayMode = 0;
		if ((lstrcmp(DlgServerItem.m_ModuleInitParameter.GameServiceOption.szDataBaseName, szTreasureDB) == 0))
		{
			cbPlayMode = 1;
		}
		DlgServerItem.LoadPersonalRoomPatameter(DlgServerItem.m_ModuleInitParameter.GameServiceOption.wKindID, cbPlayMode);
	}
...
//配置比赛
VOID CGameServerDlg::OnBnClickedLogService()
{
	byte cbLogStatus = WHLogControl::GetLogStatus();
	if (cbLogStatus == 0)
	{
		WHLogControl::SetLogStatus(1);
		SetDlgItemTextW(IDC_SERVICE_LOG, TEXT("关闭日志"));
	}
	else if (cbLogStatus == 1)
	{
		WHLogControl::SetLogStatus(0);
		SetDlgItemTextW(IDC_SERVICE_LOG, TEXT("开启日志"));
	}
}
