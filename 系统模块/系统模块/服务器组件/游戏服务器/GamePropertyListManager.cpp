...
bool CGamePropertyListManager::InsertGamePropertyItem(tagPropertyItem * ptagPropertyItem)
{
	...
	//获取子项
	if (m_GamePropertyItemMap.Lookup(ptagPropertyItem->dwPropertyID * 10 + ptagPropertyItem->cbExchangeType, pGamePropertyItem) == FALSE)
	{
		//创建对象
		try
		{
	...
	//设置索引
	m_GamePropertyItemMap[ptagPropertyItem->dwPropertyID * 10 + ptagPropertyItem->cbExchangeType] = pGamePropertyItem;
