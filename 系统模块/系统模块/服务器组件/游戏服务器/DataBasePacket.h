...
#define	DBR_GR_LOGON_AI				104									//I D 登录
...
#define DBR_GR_UNLOCK_ANDROID_USER	206									//解锁AI
#define DBR_GR_GROUP_SCORE_RECORD	207									//大联盟记录
...
#define DBR_GR_LOAD_GAME_CHAT		304									//加载聊天
...
#define DBR_GR_MATCH_ROUNDS_OVER	613									//每轮结束
#define DBR_GR_MATCH_ANDROID_DEBUG	614									//AI调试
...
#define DBR_GP_TASK_PERFORM			705									//推进任务
...
#define DBR_GR_WRITE_GROUP_TABLE_INFO		1310						//大联盟一轮信息
#define DBR_GR_AI_CREATE_TABLE				1311						//AI创建桌子
...
#define DBR_GR_MODIFY_CONFIG_SUBSTORAGE_ITEM 1701						//修改配置库存子项
#define DBR_GR_MODIFY_CUR_SUBSTORAGE_ITEM	 1702						//修改当前库存子项
#define DBR_GR_LOAD_SHARESTORAGE_ITEM		 1703						//加载共享库存
#define DBR_GR_LOAD_SINGLE_GROUPSHARESTORAGE_ITEM	 1704				//加载单个群组共享库存
...
#define DBO_GR_AI_LOGON_SUCCESS		102									//登录成功
...
#define DBO_GR_GAME_CHAT_INFO		205									//聊天信息
...
#define DBO_GR_MATCH_ROUNDS_RANK_LIST	510								//轮数排行
#define DBO_GR_MATCH_ANDROID_DEBUG	511									//AI调试
//系统事件
...
//道具事件
//道具命令
#define DBO_GP_GAME_PROPERTY_TYPE_ITEM		901							//类型节点
#define DBO_GP_GAME_PROPERTY_RELAT_ITEM		902							//道具关系
#define DBO_GP_GAME_PROPERTY_ITEM			903							//道具节点
#define DBO_GP_GAME_PROPERTY_SUB_ITEM		904							//道具节点
#define DBO_GP_GAME_PROPERTY_LIST_RESULT	905							//道具结果
...
#define DBO_GR_GROUP_CURRENCE_FEE					1306					//支付结果
#define DBO_GR_AI_CREATE_SUCCESS					1308					//AI创建成功

//共享库存事件
#define DBO_GR_LOAD_SHARESTORAGE_ITEM_RESULT		1309					//加载共享库存结果
...
struct DBR_GR_GroupScoreRecord
{
	...
	//积分记录
	WORD							wRecordCount;						//记录数目
	tagGroupScoreRecord				GroupScoreRecord[MAX_CHAIR];			//游戏记录
};
...
struct DBO_GR_LogonSuccess
{
	...
	TCHAR							szUserGameData[LEN_GAME_DATA];		//游戏数据
	//群组信息
	TCHAR							szGroupID[GROUP_ID_ARRAY_LEN];		//俱乐部ID数组
...
struct DBO_GR_LogonFailure
{
	...
	WORD							wKindID;							//游戏类型 错误码为100时使用
	WORD							wServerID;							//服务器ID 错误码为100时使用
	TCHAR							szDescribeString[128];				//错误消息
};
...
{
	LONG							lResultCode;						//结果代码
	tagAndroidParameter				AndroidParameter;					//机器信息
};
...
struct DBO_GR_UserInsureInfo
{
	...
	WORD							wRevenueTake;						//服务比例
	WORD							wRevenueTransfer;					//服务比例
	WORD							wRevenueTransferMember;				//服务比例
	WORD							wServerID;							//房间标识
	SCORE							lUserScore;							//用户游戏币
	SCORE							lUserInsure;						//银行游戏币
	SCORE							lTransferPrerequisite;				//转帐条件
	DWORD							dwUserRight;						//用户权限
	SCORE							lBankOPButtonArray[LEN_BANKEROP_BT];//银行存取操作按钮数组
};
...
struct DBR_GR_WritePersonalGameScore
{
	...
	DWORD							dwPersonalRoomTax;						//私人房服务比例
...
struct DBR_GR_CreateTable
{
	...
	LONGLONG						lEnterScore;						//进入房间分数下限
	BYTE							cbJoinMode;							//是否允许陌生人加入	0 不允许	1 允许
	DWORD							dwGroupID;							//俱乐部id， 非俱乐部创建 填 0
	DWORD							dwConfigID;							//玩法ID
	bool							bGroupPersonalMaxChair;				//大联盟百人游戏标识
};
...
struct DBO_GR_CreateSuccess
{
	...
	LONGLONG						lEnterScore;						//进入房间分数下限
	LONGLONG						lLeaveScore;						//离开房间分数下限
	BYTE							cbJoinMode;							//是否允许陌生人参与	0 允许	1 不允许
	DWORD							dwGroupID;							//大联盟id， 非大联盟模式 填 0
	BYTE							cbGroupTaxMode;						//服务模式
	DWORD							dwGroupTaxCount;					//服务比例数值
	DWORD							dwConfigID;							//玩法ID
...
struct DBO_GR_LoadShareStorageItemResult
{
	...
	tagShareStorageItem				ShareStorageItem[MAX_SHARESTORAGEITEM];		//共享库存子项
};
