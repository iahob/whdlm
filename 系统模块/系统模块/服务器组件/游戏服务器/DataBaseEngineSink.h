...
protected:
	...
	//AI 登录
	bool OnRequestLogonAI(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
...
protected:
	...
	//大联盟记录
	bool OnRequestGroupScoreRecord(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//加载参数
	...
	//加载聊天
	bool OnRequestLoadGameChat(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//等级配置
...
protected:
	...
	//AI调试
	bool OnRequestMatchAndroidDebug(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//每轮结束
	bool OnRequestMatchRoundsOver(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//比赛结束
...
protected:
	...
	//修改配置库存子项
	bool OnRequestModifyConfigSubStorageItem(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//修改当前库存子项
	bool OnRequestModifyCurSubStorageItem(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//加载共享库存
	bool OnRequestLoadShareStorageItem(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//加载单个群组共享库存
	bool OnRequestLoadSingleGroupShareStorageItem(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//推进任务
	bool OnRequestTaskPerform(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
...
protected:
	...
	//AI创建桌子
	bool OnRequestAICreateTable(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//插入创建桌子记录
	...
	//大联盟勋章对局信息
	bool OnRequestWriteGroupBattleInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID);
	//签到服务
protected:
	//加载签到
...
private:
	...
	//登录结果
	VOID OnAILogonResult(DWORD dwContextID, WORD wTableID, DWORD dwErrorCode, LPCTSTR pszErrorString, bool bMobileClient, BYTE cbDeviceType = DEVICE_TYPE_PC, WORD wBehaviorFlags = 0, WORD wPageTableCount = 0);
	//银行结果
