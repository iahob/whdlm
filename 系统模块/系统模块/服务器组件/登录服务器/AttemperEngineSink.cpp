...
#define IDI_CONNECT_LOG_SERVER		5									//日志服务器重连标识
#define IDI_CONNECT_TEST_LOGSERVER	6									//测试日志
//时间定义
	...
	case CT_CONNECT_CORRESPOND:		//连接协调
		{
			...
			_snwprintf_s(szString, CountArray(szString), TEXT("正在连接协调服务器 [ %s:%d ]"), pCorrespondAddress->szAddress, m_pInitParameter->m_wCorrespondPort);
	...
	case CT_CONNECT_PERSONAL_ROOM_CORRESPOND:		//连接约战
		{
			...
			_snwprintf_s(szString, CountArray(szString), TEXT("正在连接约战服务器 [ %s:%d ]"), pPrsnlRmCorrespondAddress->szAddress, m_pInitParameter->m_wCorrespondPort);
...
bool CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBO_GP_GAMESERVER_LEVEL_INFO:	//等级信息
		{
	...
	case DBO_GP_CHECKIN_GETREWARD:					//返回抽奖
		{
	...
	case DBO_MB_PERSONAL_GET_OFFLINE_INFO_RESULT:		//断线结果
		{
...
bool CAttemperEngineSink::OnEventTCPSocketLink(WORD wServiceID, INT nErrorCode)
{
	...
	else if (wServiceID==NETWORK_PERSONAL_ROOM_CORRESPOND)
	{
		...
		if (nErrorCode!=0)
		{
			...
			_snwprintf_s(szDescribe, CountArray(szDescribe), TEXT("约战服务器连接失败 [ %ld ]，%ld 秒后将重新连接"),
				nErrorCode,m_pInitParameter->m_wConnectTime);
...
bool CAttemperEngineSink::OnTCPSocketPersonalServiceInfo(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	case SUB_CS_C_QUERY_GOLD_ROOM_LIST_RESULT:		//金币房间列表结果
		{
			//如果没有处理结束不接受新请求
	...
	case SUB_CS_C_QUERY_GOLD_ROOM_LIST_FINISH:		//金币房间列表结果
		{
			//处理结束
			m_bQueryFinish = false;
...
bool CAttemperEngineSink::OnTCPNetworkMainPCUserService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_GP_MODIFY_LOGON_PASS_H5:		  //修改密码
		{
			//效验参数
	...
	case SUB_GP_MODIFY_LOGON_PASS_H5_Ex:		  //修改密码
		{
			//效验参数
	...
	case SUB_GP_QUERY_USER_INFO_REQUEST:  //查询用户
		{
			...
			if(pQueryUserInfoRequest->cbByNickName==FALSE)
			{
				...
				if(nLen>=8)
				{
					SendInsureFailure(dwSocketID,0,TEXT("请输入正确的玩家ID！"));
					return true;
				}
	...
	case SUB_GP_CHECKIN_GETCYCREWARD:	//领取礼包
		{
			//参数校验
			...
			if (bSuccess == false)
			{
				SendOperateFailure(dwSocketID, 0, _T("抱歉，该礼包不存在！"));
				return true;
			}
			//构造结构
...
bool CAttemperEngineSink::OnTCPNetworkSubMBQueryVideoEx(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	TCHAR szInfo[260] = { 0 };
	wsprintf(szInfo, TEXT("ptdt *** 大厅 szPersonalGUID = %s"), pQueryVideo->szPersonalGUID);
...
bool CAttemperEngineSink::OnTCPNetworkMainMBPersonalService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	case SUB_MB_QUERY_GOLD_ROOM_LIST:		//获取私人房定制配置
		{
			return OnTCPNetworkSubMBGetGoldRoomList(pData, wDataSize, dwSocketID);
		}
	case SUB_MB_PERSONAL_GET_OFFLINE_INFO://获取断线信息
		{
...
bool CAttemperEngineSink::OnDBPCUserCheckInInfo(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	for (int i = 0; i < m_CheckInReward.GetCount(); i++)
	{
		...
		{
			lstrcpyn(RewardInfo.szPackageInfo, _T("无"), CountArray(RewardInfo.szPackageInfo));
		}
...
bool CAttemperEngineSink::OnDBPCUserCheckInResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	if (CheckInResult.bSuccessed && CheckInResult.nPackageID != 0)
	{
		...
		if (m_MapRewardInfo.Lookup(CheckInResult.nPackageID, RewardInfo))
			...
			lstrcpyn(CheckInResult.szNotifyContent, _T("无"), CountArray(CheckInResult.szNotifyContent));
