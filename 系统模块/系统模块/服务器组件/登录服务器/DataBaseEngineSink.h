...
protected:
	...
	//登录之前修改登录密码
	bool OnRequestModifyLogonPassH5(DWORD dwContextID, VOID * pData, WORD wDataSize);
...
protected:
	...
	//加载房间等级
	bool OnRequestLoadGameServerLevel(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//查询签到
	...
	//签到奖励
	bool OnRequestCheckInGetReward(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//签到奖励
	bool OnRequestCheckInGetCycReward(DWORD dwContextID, VOID * pData, WORD wDataSize);
...
protected:
	...
	//获取玩家断线信息
	bool OnRequestGetOfflineInfo(DWORD dwContextID, VOID * pData, WORD wDataSize);
...
protected:
	...
	//查询玩家所有种类钱
	bool OnRequestGetAllKindMoney(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//查询玩家财富信息
	bool OnRequestGetUserWealth(DWORD dwContextID, VOID * pData, WORD wDataSize);
	//获取奖励类型
	int GetAwardType(LPCTSTR pszAwardValue);
...
protected:
	//加载游戏列表
	VOID LoadClientGameList(DWORD dwContextID, VOID * pData, WORD wDataSize);

	//结果处理
protected:
	//登录结果
	VOID OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString,DWORD dwCheckUserRight,bool bMobileClient,bool bNewUser=false);
	//银行结果
