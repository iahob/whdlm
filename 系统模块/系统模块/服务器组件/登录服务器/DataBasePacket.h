...
#define DBR_GP_MODIFY_LOGON_PASS_H5	16									//H5登录之前修改登录密码
...
//房间等级
#define DBR_GP_LOAD_GAMESERVER_LEVEL  65								//加载房间等级
...
#define DBR_GP_CHECKIN_REWARD		82									//签到奖励
#define DBR_GP_CHECKIN_CYCREWARD	83									//签到奖励
...
//查询玩家的所有币种
#define DBR_GP_GET_ALL_KIND_MONEY	500									//获取所有钱种类

//查询玩家财富信息
#define DBR_GP_GET_USER_WEALTH		501									//查询玩家财富信息
...
struct DBR_GP_ModifyLogonPass
{
	...
	BYTE							cbPlatform;							//1 标识 H5
};

//修改密码
struct DBR_GP_ModifyLogonPass_H5
{
	TCHAR							szAccounts[LEN_ACCOUNTS];		//登录帐号
	DWORD							dwClientAddr;						//连接地址
	TCHAR							szDesPassword[LEN_PASSWORD];		//用户密码
...
struct DBO_GP_CheckInReword
{
	...
	//礼包信息
	int								nPackageID;							//礼包ID
	int								nProbability;						//礼包概率
	int								nSeriesDate;						//签到天数
};

//签到抽奖
struct DBO_GP_CheckInCycReword
{
	//用户信息
	DWORD							dwUserID;							//用户标识
	TCHAR							szPassword[LEN_PASSWORD];			//登录密码
...
//获取玩家财富信息
struct DBR_GP_GetUserWealth
{
	DWORD							dwUserID;							//用户 I D
	TCHAR							szDynamicPassword[LEN_PASSWORD];	//用户动态密码
};
...
#define DBO_GP_LUA_GAME_KIND_ITEM	135									//LUA游戏列表
#define DBO_GP_H5_GAME_KIND_ITEM	136									//H5游戏列表
#define DBO_GP_U3D_GAME_KIND_ITEM	137									//U3D游戏列表
...
#define DBO_GP_CHECKIN_GETREWARD	163									//签到抽奖
#define DBO_GP_CHECKIN_PACKAGEINFO	164									//礼包详情
...
//房间等级命令
#define DBO_GP_GAMESERVER_LEVEL_INFO 193								//等级信息
...
{
	int								nPackCount;							//礼包数目
	tagSignInReward					SignInReward[30];					//礼包详情
};

//签到奖励
struct DBO_GP_PackageInfo
{
	int								nPackCount;							//礼包数目
	tagPackageInfo					PackageInfo[30];				//礼包详情
};
...
//等级信息
struct DBO_GP_GameServerLevelInfo
{
...
#define DBR_MB_QUERY_VIDEO_INFO_SCORE					805				//积分约战查询视频
#define DBR_MB_QUERY_VIDEO_INFO_GOLD					806				//金币约战查询视频
...
#define DBR_GP_GET_ALL_KIND_MONEY_RESULT				1008			//获取钱种类结果


#define DBO_MB_PERSONAL_GET_OFFLINE_INFO				1009			//获取断线信息
#define DBO_MB_PERSONAL_GET_OFFLINE_INFO_RESULT			1010			//获取断线信息结果

#define DBO_MB_PERSONAL_GET_USER_WEALTH_RESULT			1011			//获取断线信息结果
...
//开通结果
struct DBO_GP_GetAllKindMoneyResult
{
	SCORE							lInsure;				//使能标识
	SCORE							lDiamond;				//钻石
};
...
//开通结果
struct DBO_GP_GetUserWealthResult
{
	SCORE							lScore;					//携带金币
	SCORE							lInsure;				//银行金币
	SCORE							lDiamond;				//钻石
...
//用户断线信息
struct DBO_GP_GetOfflineInfo
{
	DWORD						dwUserID;								//用户标识
};

//用户断线信息
struct DBO_GP_GetOfflineInfoResult
{
	DWORD						dwServerID;								//用户标识
};
