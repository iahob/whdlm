...
public:
	...
	//获取房间
	DWORD GetFreeServer(DWORD dwUserID, DWORD dwKindID, BYTE cbIsJoinGame, BYTE cbPlayMode, DWORD dwGroupID);
	//获取房间数目
	DWORD GetFreeServerCount(DWORD dwKindID, BYTE cbPlayMode);
	DWORD GetFreeServerCount(DWORD dwKindID, BYTE cbPlayMode, bool bSupportGroup);
	//生成房间ID
	DWORD RandPersonalRoomID();

	//获取房主创建的所有房间
	int GetHostCreatePersonalRoom(tagHostCreatRoomInfo & HostCreatRoomInfo);

	//获取房主创建的房间的数量
	INT GetHostCreatePersonalRoomCount(DWORD dwUserID, DWORD dwKindID, BYTE cbPlayMode);
	//添加桌子
	bool AddPersonalMaxCreate(CMD_CS_S_RegisterPersonal RegisterPersonal);
	//是否可以再创建房间
	bool CanCreatePersonalRoom(DWORD dwKindID, DWORD dwUserID, BYTE cbPlayMode, DWORD dwGroupID);
...
public:
	...
	//激活房间
	bool ActiveServerItem(WORD wBindIndex, BYTE	cbPlayMode, tagGameServer &GameServer);

	//激活俱乐部房间
	bool ActiveGroupRoom(DWORD dwGroupID, DWORD dwRoomID);

	//获取俱乐部房间
	CRoomIDArray * GetGroupRoom(DWORD dwGroupID);

	//删除俱乐部房间
	BOOL DeleteGroupRoom(DWORD dwGroupID, DWORD dwRoomID);
...
public:
	...
	//遍历一个类型游戏的桌子
	tagPersonalTableInfo * SearchTableByPosition(WORD wKindID, BYTE cbPlayMode,	DWORD dwGroupID, POSITION & Position, DWORD & dwRoomID);
