...
protected:
	...
	DWORD							m_dwDayPlayTime;					//每日时间
	SCORE							m_lDayWinLostScore;					//每日成绩
	//托管变量
protected:
...
protected:
	...
	DWORD							m_dwDayLogonTime;					//登陆时间(当日)

	//客户类型
protected:
	bool							m_bMobileUser;						//手机用户
	bool							m_bAndroidUser;						//机器用户
	bool							m_bAndroidUserHanging;				//机器挂机;	
...
protected:
	TCHAR							m_szGroupID[GROUP_ID_ARRAY_LEN];	//俱乐部ID数组
	//函数定义
protected:
	//构造函数
	CServerUserItem();
...
public:
	...
	//每日时间
	virtual DWORD  GetDayPlayTime();
	//每日成绩
	virtual SCORE  GetDayWinLostScore(){ return m_lDayWinLostScore; }
...
public:
	...
	//挂机状态
	virtual bool IsAndroidUserHanging() { return m_bAndroidUserHanging; };
	//设置挂机状态
	virtual VOID SetAndroidUserHanging(bool bAndroidUserHanging);
	//设置机器等级(携带和离开等级)
	virtual VOID SetAndroidGrade(WORD wAndroidGrade);
	//获取机器等级(携带和离开等级)
	virtual WORD GetAndroidGrade() { return m_wAndroidGrade; }
...
public:
	...
	//获取名次
	virtual BYTE GetUserMatchRank(){return m_cbUserMatchRank;};
	//设置名次
	virtual VOID SetUserMatchRank(BYTE rank){m_cbUserMatchRank=rank;};
	//记录接口
public:
	//变更判断
...
public:
	...
	//写入积分
	virtual bool WriteUserScore(SCORE lScore, SCORE lGrade, SCORE lRevenue, SCORE lChipSerialScore, SCORE lIngot, BYTE cbScoreType, DWORD dwPlayTimeCount, DWORD dwWinExperience, DWORD dwGroupID);
	//领取奖励
...
public:
	//俱乐部数据
	virtual TCHAR *	GetGroupIDInfo(){ return m_szGroupID; }
	//设置俱乐部
	virtual VOID SetUserGroup(TCHAR *pszGroupID);
