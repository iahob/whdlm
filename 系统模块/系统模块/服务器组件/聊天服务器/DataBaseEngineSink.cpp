...
bool CDataBaseEngineSink::OnDataBaseEngineStart(IUnknownEx * pIUnknownEx)
{
	...
	//创建对象
	if ((m_GameMatchDBModule.GetInterface() == NULL) && (m_GameMatchDBModule.CreateInstance() == false))
	{
		//ASSERT(FALSE);
		return false;
	}
	...
	try
	{
		...
		tagDataBaseParameter *pGameMatchDBParameter = &m_pInitParameter->m_GameMatchDBParameter;
		...
		m_GameMatchDBModule->SetConnectionInfo(pGameMatchDBParameter->szDataBaseAddr, pGameMatchDBParameter->wDataBasePort,
			pGameMatchDBParameter->szDataBaseName, pGameMatchDBParameter->szDataBaseUser, pGameMatchDBParameter->szDataBasePass);
		...
		//发起连接
		m_GameMatchDBModule->OpenConnection();
		m_GameMatchDBAide.SetDataBase(m_GameMatchDBModule.GetInterface());
...
bool CDataBaseEngineSink::OnDataBaseEngineConclude(IUnknownEx * pIUnknownEx)
{
	...
	//关闭连接
	if (m_GameMatchDBModule.GetInterface() != NULL)
	{
		m_GameMatchDBModule->CloseConnection();
		m_GameMatchDBModule.CloseInstance();
	}
	//组件变量
	m_pInitParameter=NULL;
	m_pIDataBaseEngineEvent=NULL;
...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	case DBR_GR_MATCH_SIGNUP:			//比赛报名
		{
			return OnRequestMatchSignup(dwContextID, pData, wDataSize);
		}
	case DBR_GR_MATCH_UNSIGNUP:			//取消报名
		{
			return OnRequestMatchUnSignup(dwContextID, pData, wDataSize);
		}
	case DBO_GR_MY_MATCH_HISTORY:
		{
			return OnRequestMyMatchHistory(dwContextID, pData, wDataSize);
		}
	default:
		{
			//ASSERT(FALSE);
...
bool CDataBaseEngineSink::OnRequestMatchSignup(DWORD dwContextID, VOID *pData, WORD wDataSize)
{
	...
	try
	{
		...
		//构造参数
		m_GameMatchDBAide.ResetParameter();
		m_GameMatchDBAide.AddParameter(TEXT("@dwUserID"), pMatchSignup->dwUserID);
		m_GameMatchDBAide.AddParameter(TEXT("@InitScore"), pMatchSignup->lInitScore);
		m_GameMatchDBAide.AddParameter(TEXT("@cbSignupMode"), pMatchSignup->cbSignupMode);
		m_GameMatchDBAide.AddParameter(TEXT("@wMaxSignupUser"), pMatchSignup->wMaxSignupUser);
		//	m_GameMatchDBAide.AddParameter(TEXT("@wKindID"), m_pGameServiceOption->wKindID);
		//	m_GameMatchDBAide.AddParameter(TEXT("@wServerID"), m_pGameServiceOption->wServerID);
		m_GameMatchDBAide.AddParameter(TEXT("@strClientIP"), szClientAddr);
		m_GameMatchDBAide.AddParameter(TEXT("@dwMatchID"), pMatchSignup->dwMatchID);
		m_GameMatchDBAide.AddParameter(TEXT("@lMatchNo"), pMatchSignup->lMatchNo);
		m_GameMatchDBAide.AddParameter(TEXT("@strMachineID"), pMatchSignup->szMachineID);

		//输出参数
		TCHAR szDescribeString[128] = TEXT("");
		m_GameMatchDBAide.AddParameterOutput(TEXT("@strErrorDescribe"), szDescribeString, sizeof(szDescribeString), adParamOutput);

		//结果处理
		LONG lReturnValue = m_GameMatchDBAide.ExecuteProcess(TEXT("GSP_GR_UserMatchFee"), true);
		...
		{
			MatchSignupResult.lCurrGold = m_GameMatchDBAide.GetValue_LONGLONG(TEXT("Score"));
			MatchSignupResult.lCurrIngot = m_GameMatchDBAide.GetValue_LONGLONG(TEXT("UserIngot"));
			MatchSignupResult.cbSignupMode = m_GameMatchDBAide.GetValue_BYTE(TEXT("SignupMode"));
		}
		else
		{
			//错误描述
			CDBVarValue DBVarValue;
			m_GameMatchDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
...
bool CDataBaseEngineSink::OnRequestMatchUnSignup(DWORD dwContextID, VOID *pData, WORD wDataSize)
{
	...
	try
	{
		...
		//构造参数
		m_GameMatchDBAide.ResetParameter();
		m_GameMatchDBAide.AddParameter(TEXT("@dwUserID"), pMatchUnSignup->dwUserID);
		//		m_GameMatchDBAide.AddParameter(TEXT("@wKindID"), m_pGameServiceOption->wKindID);
		//		m_GameMatchDBAide.AddParameter(TEXT("@wServerID"), m_pGameServiceOption->wServerID);
		m_GameMatchDBAide.AddParameter(TEXT("@strClientIP"), szClientAddr);
		m_GameMatchDBAide.AddParameter(TEXT("@dwMatchID"), pMatchUnSignup->dwMatchID);
		m_GameMatchDBAide.AddParameter(TEXT("@lMatchNo"), pMatchUnSignup->lMatchNo);
		m_GameMatchDBAide.AddParameter(TEXT("@strMachineID"), pMatchUnSignup->szMachineID);
		...
		//结果处理
		LONG lReturnValue = m_GameMatchDBAide.ExecuteProcess(TEXT("GSP_GR_UserMatchQuit"), true);
...
//我的比赛
bool CDataBaseEngineSink::OnRequestMyMatchHistory(DWORD dwContextID, VOID *pData, WORD wDataSize)
{
	//效验参数
	//ASSERT(wDataSize == sizeof(DBO_GR_MyMatchHistory));
	if (wDataSize != sizeof(DBO_GR_MyMatchHistory))
	{
		return false;
	}

	//变量定义
	DBO_GR_MyMatchHistory *pMyMatch = (DBO_GR_MyMatchHistory *)pData;
	...
	{	
		//构造参数
		m_GameMatchDBAide.ResetParameter();
		m_GameMatchDBAide.AddParameter(TEXT("@dwUserID"), pMyMatch->dwUserID);
		m_GameMatchDBAide.AddParameter(TEXT("@strPassword"), pMyMatch->szPassword);
		...
		//结果处理
		LONG lReturnValue = m_GameMatchDBAide.ExecuteProcess(TEXT("GSP_GR_MyMatchHistory"), true);
		...
		{		
			...
			CMD_GC_MyMatchHistoryResult HistoryResult;
			ZeroMemory(&HistoryResult, sizeof(HistoryResult));

			BYTE cbDataBuffer[SOCKET_TCP_PACKET - 1024] = { 0 };
			LPBYTE pDataBuffer = cbDataBuffer;

			//变量定义
			while (m_GameMatchDBModule->IsRecordsetEnd() == false)
			{				
				HistoryResult.bResultCode = true;
				m_GameMatchDBAide.GetValue_String(TEXT("MatchName"), HistoryResult.szMatchName, CountArray(HistoryResult.szMatchName));
				HistoryResult.wKindID = m_GameMatchDBAide.GetValue_WORD(TEXT("KindID"));
				HistoryResult.wRandID = m_GameMatchDBAide.GetValue_WORD(TEXT("RandID"));
				HistoryResult.lRewardGold = m_GameMatchDBAide.GetValue_LONGLONG(TEXT("RewardGold"));
				HistoryResult.lRewardDiamond = m_GameMatchDBAide.GetValue_LONGLONG(TEXT("RewardDiamond"));
				m_GameMatchDBAide.GetValue_SystemTime(TEXT("MatchStartTime"), HistoryResult.MatchStartTime);

				//发送判断				
				if (wSendDataSize + sizeof(CMD_GC_MyMatchHistoryResult) > sizeof(cbDataBuffer))
				{					
					//发送数据
					m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_MATCH_HISTORY_RESULT_T, dwContextID, cbDataBuffer, wSendDataSize);

					//重置变量										
					wSendDataSize = 0;
					pDataBuffer = cbDataBuffer + sizeof(CMD_GC_MyMatchHistoryResult);
				}

				//拷贝数据
				CopyMemory(pDataBuffer, &HistoryResult, sizeof(CMD_GC_MyMatchHistoryResult));

				//设置变量				
				wSendDataSize += sizeof(CMD_GC_MyMatchHistoryResult);
				pDataBuffer += sizeof(CMD_GC_MyMatchHistoryResult);

				//移动记录
				m_GameMatchDBModule->MoveToNext();
			}
			//剩余发送
			if (wSendDataSize > 0)						
			{	//发送数据
		...
		{
			//构造结构
			DBO_GR_MyMatchHistoryResult MyMatchResult;
			ZeroMemory(&MyMatchResult, sizeof(MyMatchResult));

			//错误描述
			CDBVarValue DBVarValue;
			
			MyMatchResult.bResultCode = false;
			m_GameMatchDBModule->GetParameter(TEXT("@strErrorDescribe"), DBVarValue);
			lstrcpyn(MyMatchResult.szDescribeString, CW2CT(DBVarValue.bstrVal), CountArray(MyMatchResult.szDescribeString));
		
			//发送结果		
			m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_MATCH_HISTORY_RESULT_F, dwContextID, &MyMatchResult, sizeof(MyMatchResult));
	...
	catch (IDataBaseException *pIException)
	{
		...
		//设置变量
		MyMatchResult.bResultCode = false;
		lstrcpyn(MyMatchResult.szDescribeString, TEXT("由于数据库操作异常，请您稍后重试！"), CountArray(MyMatchResult.szDescribeString));
...
VOID CDataBaseEngineSink::OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, bool bMobileClient,WORD wBehaviorFlags,WORD wPageTableCount)
{
	if (dwErrorCode==DB_SUCCESS)
	{
		...
		m_AccountsDBAide.GetValue_String(TEXT("FaceUrl"), m_LogonSuccess.szFaceUrl, CountArray(m_LogonSuccess.szFaceUrl)); 
