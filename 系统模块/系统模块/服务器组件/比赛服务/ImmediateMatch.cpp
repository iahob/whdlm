...
{	
	if(dwTimerID>=IDI_GROUP_TIMER_START)
	{
		...
		{
			//查看是哪场比赛的定时器
			CImmediateGroup * pIG = m_MatchGroup[i];
			BYTE byTimerID = (dwTimerID - IDI_GROUP_TIMER_START)/10;
			if (pIG->m_LoopTimer ==  byTimerID)
			{
				pIG->OnTimeMessage(dwTimerID - (pIG->m_LoopTimer * 10), dwBindParameter);
			}			
		}
		//报名状态也需要定时器
		if (IDI_CHECK_MATCH == dwTimerID)
			m_pCurMatchGroup->OnTimeMessage(dwTimerID - (m_pCurMatchGroup->m_LoopTimer * 10), dwBindParameter);
...
bool CImmediateMatch::OnEventDataBase(WORD wRequestID, IServerUserItem * pIServerUserItem, VOID * pData, WORD wDataSize,DWORD dwContextID)
{
	...
	case DBO_GR_MATCH_ANDROID_DEBUG:
		{
			//参数校验
			////ASSERT(wDataSize <= sizeof(DBO_GR_MatchAndroidDebugResult));
			if (wDataSize > sizeof(DBO_GR_MatchAndroidDebugResult)) return false;

			//提取数据
			DBO_GR_MatchAndroidDebugResult * pAndroidDebugResult = (DBO_GR_MatchAndroidDebugResult*)pData;
			...
			for (int i = 0; i < m_MatchGroup.GetCount(); i++)
			{
				if (m_MatchGroup[i]->m_lMatchNo == pAndroidDebugResult->lMatchNo)
				{
					pImmediateGroup = m_MatchGroup[i];

					if (pImmediateGroup != NULL)
					{
						pImmediateGroup->MatchAndroidDebug(pAndroidDebugResult->wPromotion);
					}
					break;
				}
			}
	...
	case DBO_GR_MATCH_ROUNDS_RANK_LIST:
		{
			...
			for (int i = 0; i < m_MatchGroup.GetCount(); i++)
			{
				if (m_MatchGroup[i]->m_lMatchNo == pMatchRankList->lMatchNo)
				{
					...
					if (pImmediateGroup != NULL)
					{						
						pImmediateGroup->RankingPromotion(pMatchRankList);
					}
					break;
				}
			}
			...
			for (int i=0;i<m_MatchGroup.GetCount();i++)
			{
				if(m_MatchGroup[i]->m_lMatchNo==pMatchRankList->lMatchNo)
				{
					...
					if(pImmediateGroup!=NULL)
					{
						...
						//参赛玩家退赛
						pos=pImmediateGroup->m_OnMatchUserMap.GetStartPosition();
						pITempUserItem=NULL;
						dwUserID=0;
...
bool CImmediateMatch::OnEventSocketMatch(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
	switch (wSubCmdID)
	{
	case SUB_GR_MATCH_FEE:	//比赛费用---扣除费用
		{
			//参数效验
...
//用户退出
bool CImmediateMatch::PromotionUserLogout(IServerUserItem *pIServerUserItem)
{
	//退出比赛
...
bool CImmediateMatch::OnEventUserJoinMatch(IServerUserItem * pIServerUserItem, BYTE cbReason,DWORD dwSocketID)
{
	...
	if (pIServerUserItem->IsAndroidUser()==true)
	{
		...
		if (m_pCurMatchGroup->CheckAndroidJoin() == FALSE)
		{
			return true;
		}
		////获取时间
	...
	{
		//机器数判断
		if (pIServerUserItem->IsAndroidUser() == true && (m_pCurMatchGroup->m_wCurrAndroidUserCount >= m_pImmediateMatch->wAndroidUserCount))
		{
			return true;
		}
...
//AI调试
bool CImmediateMatch::OnEventMatchAndroidDebug(CImmediateGroup *pMatch,WORD wPromotion,DWORD dwAndroidID1,DWORD dwAndroidID2,DWORD dwAndroidID3)
{
	//比赛结束
	if (pMatch != NULL)
	{
		DBR_GR_MatchAndroidDebug MatchAndroidDebug;
		ZeroMemory(&MatchAndroidDebug, sizeof(MatchAndroidDebug));

		//构造数据						
		MatchAndroidDebug.dwMatchID = m_pMatchOption->dwMatchID;
		MatchAndroidDebug.lMatchNo = pMatch->m_lMatchNo;
		MatchAndroidDebug.cbMatchType = MATCH_TYPE_IMMEDIATE;		
		MatchAndroidDebug.wServerID = m_pGameServiceOption->wServerID;
		MatchAndroidDebug.dwAndroidID1 = dwAndroidID1;
		MatchAndroidDebug.dwAndroidID2 = dwAndroidID2;
		MatchAndroidDebug.dwAndroidID3 = dwAndroidID3;
		MatchAndroidDebug.cbNo1Ratio1 = m_pImmediateMatch->cbNo1Ratio1;
		MatchAndroidDebug.cbNo1Ratio2 = m_pImmediateMatch->cbNo1Ratio2;
		MatchAndroidDebug.cbNo1Ratio3 = m_pImmediateMatch->cbNo1Ratio3;
		MatchAndroidDebug.wPromotion = wPromotion;
			
		//投递请求
		m_pIDataBaseEngine->PostDataBaseRequest(MatchAndroidDebug.dwMatchID, DBR_GR_MATCH_ANDROID_DEBUG, 0, &MatchAndroidDebug, sizeof(MatchAndroidDebug));
	}
...
//每轮结束
bool CImmediateMatch::OnEventMatchRoundsOver(CImmediateGroup *pMatch,int wPromotion)
{
	//比赛结束
	if (pMatch != NULL)
	{
		...
		MatchRoundsOver.lMatchNo = pMatch->m_lMatchNo;
		MatchRoundsOver.cbMatchType = MATCH_TYPE_IMMEDIATE;
		MatchRoundsOver.wPlayCount = m_pImmediateMatch->cbTotalPlayCount;
		MatchRoundsOver.wServerID = m_pGameServiceOption->wServerID;
		MatchRoundsOver.cbMatchRounds = pMatch->m_cbCurMatchRounds;
		MatchRoundsOver.wPromotion = wPromotion;

		//获取时间
		MatchRoundsOver.MatchStartTime = pMatch->m_MatchStartTime;
...
bool CImmediateMatch::OnEventMatchOver(CImmediateGroup *pMatch)
{
	...
	if(pMatch!=NULL)
	{
		...
		MatchOver.wPlayCount = m_pImmediateMatch->cbTotalPlayCount;
		MatchOver.wServerID=m_pGameServiceOption->wServerID;
		MatchOver.cbMatchRounds = pMatch->m_cbCurMatchRounds;
		...
		//测试代码			
		TCHAR szMessage[128];
		_sntprintf_s(szMessage, CountArray(szMessage), TEXT("即时赛结束:%d"), MatchOver.lMatchNo);
...
bool CImmediateMatch::SendMatchInfo(IServerUserItem * pIServerUserItem)
{
	...
	m_pIGameServiceFrame->SendMatchMessage(m_pCurMatchGroup->m_lMatchNo, m_pCurMatchGroup->m_OnMatchUserMap.GetCount());
...
void CImmediateMatch::InsertNullTable()
{
	...
	for (int i=0;i<m_pGameServiceOption->wTableCount;i++)
	{
		...
		if (CImmediateGroup::m_wChairCount != 0 && m_pImmediateMatch && nCurTableCount >= m_pImmediateMatch->wStartUserCount / CImmediateGroup::m_wChairCount)break;
		...
		for (int j=0;j<m_MatchGroup.GetCount();j++)
		{
			...
			for (int k = 0; k<wMatchGroupTableCount&& k < m_MatchTableArray.GetCount(); k++)
			{
				if (!m_MatchGroup[j] || !m_MatchTableArray[k])
				{
					continue;
				}
		...
		if( bIsInsert&&m_ppITableFrame[i]->GetNullChairCount()==m_pGameServiceAttrib->wChairCount)
		{
			...
			pTableFrameInfo->cbTotalRounds = m_pImmediateMatch->cbTotalRounds;			
			memcpy(&pTableFrameInfo->hMatchRounds[0], &m_pImmediateMatch->hMatchRounds[0], sizeof(tagMatchRounds)*MAX_MATCH_ROUNDS);
