...
//设置日志控制方式
bool CServiceUnits::SetLogControl(byte cbServerType, byte cbWriteLogTpye)
{
	...
	case LOGON_SERVER:
			{
						 m_AttemperEngineSink.CloseLogonServerLog(cbWriteLogTpye);
				break;
			}
	case GAME_SERVER:
			{
						m_AttemperEngineSink.CloseGamedServerLog(cbWriteLogTpye);
				break;
			}
	case CHAT_SERVER:
			{
						m_AttemperEngineSink.CloseChatServerLog(cbWriteLogTpye);
				break;
			}
	case GROUP_SERVER:
			{
						 m_AttemperEngineSink.CloseGroupServerLog(cbWriteLogTpye);
				break;
			}
		default:break;
	}
