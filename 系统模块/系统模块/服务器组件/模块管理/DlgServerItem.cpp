...
bool CDlgServerItem::OpenGameServer(WORD wServerID)
{
	...
	{
		byte cbPlayMode = 0;
		if (lstrcmp(GameServerResult.szDataBaseName, szTreasureDB) == 0)
		{
			cbPlayMode = 1;
		}
		
		ServerInfoManager.LoadPersonalRoomParameter(personalRoomOption, personalRoomOptionRightAndFee, GameServerResult.wKindID, cbPlayMode);
	}
...
bool CDlgServerItem::LoadPersonalRoomPatameter(WORD wKindID, byte cbPlayMode)
{
	...
	tagPersonalRoomOption  personalRoomOption;
	tagPersonalRoomOptionRightAndFee  PersonalRoomOptionRightAndFee; 
	ZeroMemory(&personalRoomOption, sizeof(personalRoomOption));
	ZeroMemory(&PersonalRoomOptionRightAndFee, sizeof(PersonalRoomOptionRightAndFee));

	ServerInfoManager.LoadPersonalRoomParameter(personalRoomOption, PersonalRoomOptionRightAndFee, wKindID, cbPlayMode);
...
bool CDlgServerItem::GetModuleInitParameter(tagGameServerInfo * pGameServerInfo, bool bAutoMode)
{
	...
	m_ModuleInitParameter.GameServiceOption.wServiceRatio = pGameServerInfo->wServiceRatio;
	...
	m_ModuleInitParameter.GameServiceOption.lMaxTableScore = pGameServerInfo->lMaxTableScore;
	...
	UINT uShareStorageRule = sizeof(m_ModuleInitParameter.GameServiceOption.cbShareStorageRule);
	CopyMemory(m_ModuleInitParameter.GameServiceOption.cbShareStorageRule, pGameServerInfo->cbShareStorageRule, uShareStorageRule);
