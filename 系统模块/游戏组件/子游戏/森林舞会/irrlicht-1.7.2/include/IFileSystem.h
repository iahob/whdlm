...
public:
	...
	*/
	...
	without its complete path.
	\return True if the archive was added successfully, false if not. */
	virtual bool addZipFileArchive(const c8* filename, bool ignoreCase=true, bool ignorePaths=true,const core::stringc& password="")
	{
		return addFileArchive(filename, ignoreCase, ignorePaths, EFAT_ZIP,password);
	}
