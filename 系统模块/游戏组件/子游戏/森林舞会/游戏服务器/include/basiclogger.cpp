...
bool  SGSYS_CreateDirectory(const char* pszDir)
{
	...
	lstrcpynA(szPath, pszDir, sizeof(szPath));
...
bool CBasicLogger::Init(INT32 nMode, UINT32 dwID, const char* pszDir, const char* pszName)
{
	...
	//ZeroString(szFile);
	//_snprintf_s(szFile, sizeof(szFile)-1, "%s/%s_%04d_%02d_%02d.log", 
	//	m_szDir, m_szName,
...
void CBasicLogger::_UpdateFilePointer()
{
	...
	if(m_fpFile != NULL)
	{
		if(ftell(m_fpFile) > MAX_LOGFILE_SIZE)
		{
			...
			ZeroString(szFile);
			_snprintf_s(szFile, sizeof(szFile)-1, "%s/%s_%04d_%02d_%02d_%d.log", 
				m_szDir, m_szName,
	...
	else if (LOG_MODE_DAY_DIR_DIVIDE == m_nLogMode)
	{
		...
		{	
			char szNewDir[128] = {0};
			_snprintf_s(szNewDir,127,"%s/%d-%02d-%02d",m_szDir,stCurTime.m_nYear,stCurTime.m_nMon,stCurTime.m_nMday);
			SGSYS_CreateDirectory(szNewDir);

			ZeroString(szFile);
			_snprintf_s(szFile, sizeof(szFile)-1, "%s/%s.log", 
				szNewDir, m_szName);
