...
public:
	...
	void GetRandLimitTimes(int64_t& lCurMinTimes, int64_t& lCurMaxTimes)
	{
		int nStartIndex = -1, nEndIndex = -1;
		int64_t nTotalWeight = 0;
		for (auto& it : m_weightConfig)
		{
			if (nStartIndex < 0 && lCurMinTimes <= it.lMinTimes)
				nStartIndex = static_cast<int>(it.lIndex);
			if (nEndIndex < 0 && (lCurMaxTimes <= it.lMaxTimes || it.lMaxTimes < 0))
				nEndIndex = static_cast<int>(it.lIndex);
		}
		for (auto& it : m_weightConfig)
		{
			if (it.lIndex >= nStartIndex && it.lIndex <= nEndIndex)
			{
				nTotalWeight += it.lWeight;
			}
		}
		if (nTotalWeight == 0)
		{
		...
		int nRandValue = rand() % nTotalWeight;
		int64_t nStart = 0;
		for (auto& it : m_weightConfig)
		{
			if (it.lIndex < nStartIndex || it.lIndex > nEndIndex)
			...
			if (nRandValue >= nStart && nRandValue < nStart + it.lWeight)
			{
				...
				lCurMaxTimes = it.lMaxTimes;
				if (lCurMaxTimes < 0) lCurMaxTimes = MAX_LONGLONG;
				else if (lCurMaxTimes == 0)
				{
					lCurMaxTimes = lCurMinTimes;
					lCurMinTimes = 0;
				}
				return;
			}
