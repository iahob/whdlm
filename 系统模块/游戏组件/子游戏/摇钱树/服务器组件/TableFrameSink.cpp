...
bool CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	_sntprintf_s(szConfigPath, MAX_PATH, TEXT("%s\\FishYQSConfig\\%s.fdx"), szDirectoryPath, m_pGameServiceOption->szServerName);
	FILE * pFile = NULL;
	...
	_snprintf_s(g_szLogoFileName, MAX_PATH, "%s\\FishYQSConfig\\%s_日志.log", szDirectoryPathA.m_psz, szServerNameA.m_psz);
	_snprintf_s(g_szStockFileName, MAX_PATH, "%s\\FishYQSConfig\\%s_库存.log", szDirectoryPathA.m_psz, szServerNameA.m_psz);
	...
	if( m_ArrayFishPathPositive.GetCount() == 0 )
	{
		...
		_sntprintf_s(szFishMermaidPath, MAX_PATH, TEXT("%s\\FishYQSConfig\\FishYQSPath.pat"), szDirectoryPath);
		...
		if ( bOpen == FALSE )
		{	
			AfxMessageBox(TEXT("文件 FishYQSConfig\\FishYQSPath.pat 读取失败！"));
			return false;
		}
								   ...
								   bool bFlockKill)
{
	...
	if ( cbFishType == FishType_Max )
	{
		...
		if( GetFishCount(FishType_YinGuiYQS) >= 1 ) nEYu = 0;
		if( GetFishCount(FishType_JinLong) >= 1 ) nJinChan = 0;
		if( GetFishCount(FishType_LvGuiYQS) >= 1 ) nJinSeNianYu = 0;
		if( GetFishCount(FishType_ShuangTouQiEn) >= 1 ) nMoFaYu = 0;
		if (GetFishCount(FishType_JinGuiYQS) >= 1) nJinYQS = 0;
		...
		// 大鱼限制
		if( GetFishCount(FishType_YinGuiYQS) + GetFishCount(FishType_JinLong) + GetFishCount(FishType_LvGuiYQS) + GetFishCount(FishType_ShuangTouQiEn)+GetFishCount(FishType_JinGuiYQS) >= (uint)m_nMaxBigFishCount )
		{
			nEYu = 0;
			nJinChan = 0;
			nJinSeNianYu = 0;
		...
		// 随机鱼
		cbFishType = RandomArea(FishType_Normal_Max, 2, 5, 5, 20, 20, 20, 20, 20, 10, 10, 10, 8, 8, 8, 8, nHeiDongYu, nJinSeChuiTouSha, nMoFaYu, nJinSeNianYu, nJinChan, nEYu,nJinYQS,nBingDongYu, nBaoZhaYu, 0 );
	}
	...
	case 0:
		{
			...
			for (int nFishIndex = FishType_MoGuiYu; nFishIndex <= FishType_JinGuiYQS; ++nFishIndex)
			{
