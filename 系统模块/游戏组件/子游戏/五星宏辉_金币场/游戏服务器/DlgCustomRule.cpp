...
bool CDlgCustomRule::SaveCustomRule(LPBYTE pcbCustomRule, WORD wCustomSize)
{
	...
	if( m_hWnd )
	{
		...
		TCHAR sz29Data[256] = { 0 };
		m_DlgCustomAndroid.GetDlgItemText(IDC_EDIT_ANDROID_21, sz29Data, sizeof(sz29Data));
		...
		TCHAR sz30Data[256] = { 0 };
		m_DlgCustomAndroid.GetDlgItemText(IDC_EDIT_ANDROID_22, sz30Data, sizeof(sz30Data));
	...
	if (m_DlgCustomAndroid.m_CustomAndroid.lRobotMinTotalJetton > m_DlgCustomAndroid.m_CustomAndroid.lRobotMaxTotalJetton) {
		AfxMessageBox(L"[最小总下注范围]不能大于[最大总下注范围]");
		return false;
	}
	if (m_DlgCustomAndroid.m_CustomAndroid.lRobotMinTotalJetton == m_DlgCustomAndroid.m_CustomAndroid.lRobotMaxTotalJetton) {
		AfxMessageBox(L"[最小总下注范围]不能等于[最大总下注范围]");
		return false;
	}
	...
	if (!CheckDataMinMax(m_DlgCustomAndroid.m_CustomAndroid.lRobotMinTotalJetton)) return false;
	if (!CheckDataMinMax(m_DlgCustomAndroid.m_CustomAndroid.lRobotMaxTotalJetton)) return false;
	//超级抢庄配置
...
VOID CDlgCustomAndroid::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_ANDROID_21, m_CustomAndroid.lRobotMinTotalJetton);//机器人最小总下注
	DDX_Text(pDX, IDC_EDIT_ANDROID_22, m_CustomAndroid.lRobotMaxTotalJetton);//机器人最大总下注
}
