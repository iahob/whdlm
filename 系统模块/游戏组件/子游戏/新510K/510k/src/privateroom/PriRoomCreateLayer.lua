...
function PriRoomCreateLayer:ctor( scene )
    ...
    local tabCount = PriRoom:getInstance().m_tabCellScoreList --{1, 2, 5, 10, 20}
...
function PriRoomCreateLayer:onLoginPriRoomFinish()
    ...
    if ((meUser.cbUserStatus == yl.US_FREE or meUser.cbUserStatus == yl.US_NULL or meUser.cbUserStatus == yl.US_PLAYING or meUser.cbUserStatus == yl.US_SIT)) then
        if PriRoom:getInstance().m_nLoginAction == PriRoom.L_ACTION.ACT_CREATEROOM then
            ...
            --LogAsset:getInstance():logData(self.m_nSelectScore)
...
function PriRoomCreateLayer:getInviteShareMsg( roomDetailInfo )
    ...
    local friendC = "510k房间ID:" .. roomDetailInfo.szRoomID .. " 局数:" .. roomDetailInfo.dwPlayTurnCount
    return {title = "【约战房间: " .. roomDetailInfo.szRoomID .. "】", content = shareTxt .. " 510k游戏精彩刺激, 一起来玩吧! ", friendContent = friendC}
end
...
function PriRoomCreateLayer:onCellSelectedEvent(tag, sender)
    ...
    local tabCount = PriRoom:getInstance().m_tabCellScoreList
