    ...
    elseif BTN_INVITE == tag then
        ...
        PriRoom:getInstance():getPlazaScene():popTargetShare(function(target, bMyFriend)
            ...
            elseif nil ~= target then
                GlobalUserItem.bAutoConnect = false
                MultiPlatform:getInstance():shareToTarget(target, sharecall, "【约战房间: " .. self.m_atlasRoomID:getString() .. " 】", shareTxt .. " 510k游戏精彩刺激, 一起来玩吧! ", url, "")
            end
        end)
...
function PriGameLayer:onPriGameEnd( cmd_table )
    self._gameLayer.m_priTotalEnd = true
    ...
    for i = 1, 4 do
        if 1 == self._gameLayer._gameView.m_tabPriUserLeave[i] then
            self._gameLayer._gameView:removeHead(i)
        end   
    end
end
