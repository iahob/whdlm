...
function GameViewLayer:paramInit()
    ...
    self.m_tabPriUserLeave = {0,0,0,0} --私人房总结算消息来之前 记录玩家信息 不显示退出
...
function GameViewLayer:loadResource()
	...
	--[[
	local cards = {0x09,0x18,0x03,0x21,0x2D,0x07,0x0D,0x02,0x0A,0x05,0x2A,0x23,0x25}
	local result = GameLogic:Search510K(cards,#cards,0,0)
	cards = GameLogic:SortCardList(cards, #cards, 0,nil)
	self.m_tabNodeCards[1]:updateCardsNode(cards, true, false)		
	--]]
	--dump(result,"Search510K")
	--print("--- result[3] count "..#result[3])
    self.m_tabAlertSp = {}
    --3个警报灯
    for i = 2, 4 do
		...
		--local cards = {0x09,0x18,0x03,0x21,0x2D,0x07,0x0D,0x02,0x0A,0x05,0x2A,0x23,0x25}
		--self:updateCardsNodeLayout(i,#cards,true)
    end
    ...
    for i = 1, 4 do
        ...
        elseif nil == self.m_tabUserItem[viewId] then
        ...
        local heart_2 = head_bg:getChildByName("heart_2")
        heart_2:setVisible(false)
        heart_2:setLocalZOrder(TAG.Tag_Head + 5)
        --
		...
		local tip_declare = csbNode:getChildByName(string.format("tip_declare_%d", viewId))
        tip_declare:setVisible(false)
		
		local tip_no_declare = csbNode:getChildByName(string.format("tip_no_declare_%d", viewId))
        tip_no_declare:setVisible(false)
		
		local tip_ask = csbNode:getChildByName(string.format("tip_ask_%d", viewId))
        tip_ask:setVisible(false)
		
		local tip_no_ask = csbNode:getChildByName(string.format("tip_no_ask_%d", viewId))
        tip_no_ask:setVisible(false)
        ...
        self.m_tabVoiceBox[viewId] = img_voice_box
		...
		--私人房隐藏金币图标
		local gold_img = head_bg:getChildByName("gold_img")
	...
	--庄家选择按钮
	self.m_text_bankChoose = csbNode:getChildByName("text_bankChoose")
	self.m_text_bankChoose:setVisible(false)
	self:getParentNode().m_tabCurrentCards = {}
...
function GameViewLayer:reSetGame()
    ...
    self.m_cardNum1:setVisible(false)
	self:hideAllAddTimesTip()
	self:hideAllAskTip()
	self:hideAllDeclareTip()
...
function GameViewLayer:reSetUserInfo()
	...
	if PriRoom and GlobalUserItem.bPrivateRoom then
		self.m_atlasScore:setString(string.format("%d",self.m_historyScore[cmd.MY_VIEWID].lCollectScore))
	end
end
...
function GameViewLayer:updateClock( clockId, cbTime)
    ...
    if cbTime <= 0 then
        ...
        elseif cmd.TAG_COUNTDOWN_ADD_TIMES == clockId then --加倍阶段时间到直接退出
            -- 私人房无自动托管
            if not GlobalUserItem.bPrivateRoom and self.m_nCountDownView == cmd.MY_VIEWID then
                --self:onGameTrusteeship(true)
                self._scene:onExitTable()
...
function GameViewLayer:OnUpdateUser(viewId, userItem, bLeave)
    print(" update user " .. viewId,tostring(userItem == nil),bLeave)
    local head_bg = self.m_csbNode:getChildByName(string.format("head_bg_%d", viewId))
    local head = head_bg:getChildByTag(TAG.Tag_Head)
    if nil == userItem then
        if bLeave then
            if PriRoom and GlobalUserItem.bPrivateRoom and not self._scene.m_priTotalEnd then
                self.m_tabPriUserLeave[viewId] = 1
            else
    ...
    print("--- OnUpdateUser  userItem.cbUserStatus ---",userItem.cbUserStatus,userItem.szNickName)
    ...
    --dump(self.m_historyScore,"self.m_historyScore")
    ...
    if nil == self.m_tabUserItem[viewId] then
		...
		if PriRoom and GlobalUserItem.bPrivateRoom and 1 == PriRoom:getInstance().m_tabPriData.cbIsGoldOrGameScore then
			local str = string.format("%d",self.m_historyScore[viewId].lCollectScore)
			str = string.gsub(str, "-", "/");
	...
	if PriRoom and GlobalUserItem.bPrivateRoom and nil ~= PriRoom:getInstance().m_tabPriData.dwTableOwnerUserID then
...
function GameViewLayer:onGameStart()
	...
	local nodeCard = self.m_tabNodeCards[cmd.MY_VIEWID]
    ...
    if true == dataBuffer.bAsk then
        ...
        for i = 1, cmd.PLAYER_COUNT do
            ...
            if(i - 1) == self.m_wChairId then -- (i - 1 == wMyFriend) or 
                if (i - 1) == self._scene.m_wBankerUser and self._scene.m_cbFriendFlag == 2 then
...
function GameViewLayer:onGameAddTimes(dataBuffer)
	...
	print("wAddTimesUser "..wAddTimesUser.." addViewId "..addViewId)
        ...
        if curViewId == cmd.MY_VIEWID then
            ...
            -- 出牌控制
                ...
                --self.m_spInfoTip:setPosition(yl.WIDTH * 0.5, 160)
                    ...
                    local sel = self.m_tabNodeCards[cmd.MY_VIEWID]:getSelectCards()
                    local selCount = #sel
                    dump(sel, " onGetOutCard getSelectCards") 
                    dump(lastOutCards, " onGetOutCard lastOutCards") 
                    if selCount > 0 then
...
function GameViewLayer:onGetGameConclude( rs )
	...
	local cbCardCount = rs.cbCardCount[1]
	local cbHandCardData = rs.cbHandCardData[1]
	local startCardsIndex = 0
    for i = 1 ,cmd.PLAYER_COUNT do
        local viewId = self._scene:SwitchViewChairID(i - 1)
		...
		--显示手上牌
		--显示玩家牌
		local cardCount = cbCardCount[i]
		if viewId ~= cmd.MY_VIEWID then
			local cards = {}
			for index = 1,cardCount do
				table.insert(cards,index,cbHandCardData[startCardsIndex + index]) 
			end
			--dump(cards,"updateCardsNodeLayout cards")
			self:updateCardsNodeLayout(viewId, cardCount, true,cards)
            self.m_tabNodeCards[viewId].m_cardsHolder:setVisible(false)
		end
		
		startCardsIndex = startCardsIndex  + cardCount
    end
    -- 取消托管
    if self.m_trusteeshipBg:isVisible() then
        self.m_trusteeshipBg:setVisible(false)
...
--私人房显示输赢分
function GameViewLayer:updateCollectScore()
	print("************************************ GameViewLayer:updateCollectScore()")
	for i = 1, cmd.PLAYER_COUNT do
        local viewId = self._scene:SwitchViewChairID(i - 1)
		local head_bg = self.m_csbNode:getChildByName(string.format("head_bg_%d", viewId))
		local player_score = head_bg:getChildByName("player_score")
...
function GameViewLayer:presentAskFriendBtns(cbSelectedCardData)
    self.m_bt_ask:setVisible(true)
	if nil ~= cbSelectedCardData and cbSelectedCardData ~= 0 then
		local bFind = self:findSelectedCard(cbSelectedCardData)
		if bFind then
			self.m_bt_ask:loadTextures("game_res/bt_agree_1.png", "game_res/bt_agree_2.png", "game_res/bt_agree_1.png")
			self.m_bt_no_ask:loadTextures("game_res/bt_disagree_1.png", "game_res/bt_disagree_2.png", "game_res/bt_disagree_1.png")
		else
			self.m_bt_ask:loadTextures("game_res/bt_ask_1.png", "game_res/bt_ask_2.png", "game_res/bt_ask_1.png")
			self.m_bt_no_ask:loadTextures("game_res/bt_noAsk_1.png", "game_res/bt_noAsk_2.png", "game_res/bt_noAsk_1.png")
		end
	end
...
function GameViewLayer:findSelectedCard(cbSelectedCardData)
    local bFind = false
    local handData = self.m_tabNodeCards[1]:getHandCards()
	for i = 1, #handData do
		if cbSelectedCardData == handData[i] then
			bFind = true
			break
		end
	end
...
function GameViewLayer:hideAllAskTip()
    for i = 1, cmd.PLAYER_COUNT do
        local tip_ask = self.m_csbNode:getChildByName(string.format("tip_ask_%d", i))
        tip_ask:setVisible(false)

        local tip_no_ask = self.m_csbNode:getChildByName(string.format("tip_no_ask_%d", i))
        tip_no_ask:setVisible(false)
    end
end

function GameViewLayer:hideAllDeclareTip()
    for i = 1, cmd.PLAYER_COUNT do
        local tip_declare = self.m_csbNode:getChildByName(string.format("tip_declare_%d", i))
        tip_declare:setVisible(false)

        local tip_no_declare = self.m_csbNode:getChildByName(string.format("tip_no_declare_%d", i))
        tip_no_declare:setVisible(false)
    end
end
...
function GameViewLayer:presentBankChoose(cbSelectedCardData)
	if cbSelectedCardData == 0 then
		return
	end
	local value = GameLogic:GetCardLogicValue(cbSelectedCardData)
	local color = GameLogic:GetCardColor(cbSelectedCardData)
	color = bit:_rshift(color,4) + 1
	local valueStr = GameLogic._ValueName[value]
	local colorStr = GameLogic._ColorName[color]
	--print(string.format("cbSelectedCardData %d color %d",cbSelectedCardData ,color ))
	local str = "庄家选择:"..colorStr..valueStr
	--print(str)
	self.m_text_bankChoose:setVisible(true)
	self.m_text_bankChoose:setString(str)
end

function GameViewLayer:openFindFriendLayer()
    local tChooseLayer = ChooseLayer:create(self)
    tChooseLayer:setPosition(cc.p(680,400))
...
function GameViewLayer:updateCardsNodeLayout( viewid, cardCount,visible,cardData)
    ...
    --for index = 2, cmd.PLAYER_COUNT do 
		...
		local dx = 0
		if viewid == cmd.TOP_VIEWID and nil ~=cardData then
			dx = - 350
		end
		local startY = 0
		local interval_x = tabCardPositionChange[viewid].x
		local interval_y = tabCardPositionChange[viewid].y
		if 0 ~= interval_x and nil ~= cardData then
			interval_x = interval_x + CARD_WIDTH_SMALL / 5 * 3 - 13
		end
		if 0 ~= interval_y and nil ~= cardData then
			interval_y = interval_y + CARD_HEIGHT_SMALL / 3
		end
		--两边从上往下
		interval_y = - interval_y -- 为负
		if cmd.TOP_VIEWID ~= viewid and nil ~= cardData then
			--startY = 400
			startY = - 30
            ...
            if card ~= nil then
                if i > cardCount then
					...
					if nil ~= cardData then
						local _value = cardData[i]
						local cardValue = yl.POKER_VALUE[_value]  --math.mod(cbCardData, 16) --bit:_and(cbCardData, 0x0F)
						local cardColor = yl.CARD_COLOR[_value]
						card:setTextureRect(cc.rect((cardValue - 1) * CARD_WIDTH_SMALL, cardColor * CARD_HEIGHT_SMALL, CARD_WIDTH_SMALL, CARD_HEIGHT_SMALL))
					else
						card:setTextureRect(cc.rect(0, 4 * CARD_HEIGHT_SMALL, CARD_WIDTH_SMALL, CARD_HEIGHT_SMALL))
					end
                    card:setVisible(true)
                    card:setPosition( interval_x * (i - 1) + dx, startY - interval_y * (i - 1))
                ...
                if cardCount / 2 == math.floor(cardCount / 2) then --是否为偶数
                    ...
                    card:setPosition(interval_x * (i - 1) + dx, startY - interval_y * (i - 1  - halfNum) + dy)
                else
                    card:setPosition(interval_x * (i - 1) + dx, startY - interval_y * (i - 1  - halfNum))
                end
            end
