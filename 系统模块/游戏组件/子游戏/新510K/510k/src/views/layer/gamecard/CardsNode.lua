	...
	if 0 ~= handCount and haveCardData then
		...
		for k,v in pairs(cards) do
            ...
            if nil ~= removeIdx then
                ...
                table.remove(self.m_mapSelectedCards, v)
                table.remove(self.m_mapDragSelectCards, v)
            end
        end
		...
		bOutOk = true
		--dump(self.m_cardsData, "outCard after self.m_cardsData")
		print("outCard after self.m_cardsCount "..self.m_cardsCount)
		self:reSortCards()
...
function CardsNode:reSortCards()
	...
	if cmd.MY_VIEWID == self.m_nViewId then
		...
		--dump(result,"reSortCards result")
		if #result[3] > 0 then
			cards = GameLogic:SortCardList(cards, count, 0,result)
			--dump(cards,"SortCardList cards")
			--self:updateCardsNode( cards, cmd.MY_VIEWID == self.m_nViewId, false)
		end
