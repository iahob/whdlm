...
{
	//效验参数
	ASSERT((wChairID<GAME_PLAYER)&&(cbBufferCount>=MAX_CARD_COUNT));
	if ((wChairID>=GAME_PLAYER)||(cbBufferCount<MAX_CARD_COUNT)) return 0;
...
VOID CHistoryCard::OnEventSaveData(WORD wChairID)
{
	...
	if (m_cbEventFlag[wChairID][INDEX_CURRENT]!=AF_NULL)
	{
		...
		CopyMemory(m_cbCardData[wChairID][INDEX_HISTORY],m_cbCardData[wChairID][INDEX_CURRENT],sizeof(BYTE)*MAX_CARD_COUNT);

		//清理数据
		m_cbCardCount[wChairID][INDEX_CURRENT]=0;
		m_cbEventFlag[wChairID][INDEX_CURRENT]=AF_NULL;
		ZeroMemory(m_cbCardData[wChairID][INDEX_CURRENT],sizeof(BYTE)*MAX_CARD_COUNT);
	}
