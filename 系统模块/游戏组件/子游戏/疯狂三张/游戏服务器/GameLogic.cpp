...
//对比扑克
BYTE CGameLogic::CompareCard(BYTE cbFirstData[], BYTE cbNextData[], BYTE cbCardCount,bool bColor/*=false*/)
{
	//设置变量
	...
	case CT_JIN_HUA:		//金花
		{
			...
			if (bColor)
			{
				return (GetCardColor(FirstData[0]) > GetCardColor(NextData[0])) ? 1 : 0;
			}
	...
	case CT_SHUN_JIN:		//顺金 432>A32
		{		
			...
			BYTE cbNextColor=GetCardColor(NextData[0]);
			BYTE cbFirstColor=GetCardColor(FirstData[0]);

			//特殊A32
			if(cbNextValue==14 && GetCardLogicValue(NextData[cbCardCount-1])==2)
			{
				cbNextValue=3;
				cbNextColor=GetCardColor(NextData[1]);
			}
			if(cbFirstValue==14 && GetCardLogicValue(FirstData[cbCardCount-1])==2)
			{
				cbFirstValue=3;
				cbFirstColor=GetCardColor(FirstData[1]);
			}
			...
			if(bColor)
			{
				return (cbFirstColor>cbNextColor)?1:0;
			}
			return DRAW;
		}
	case CT_DOUBLE:			//对子
		{
			...
			if(bColor)//比对子的花色
			{
				BYTE cbFirstMaxColor=0;
				for(int i=0;i<3;i++)
				{
					if(GetCardLogicValue(FirstData[i])==bFirstDouble && GetCardColor(FirstData[i])>cbFirstMaxColor)
						cbFirstMaxColor=GetCardColor(FirstData[i]);
				}

				BYTE cbNextMaxColor=0;
				for(int i=0;i<3;i++)
				{
					if(GetCardLogicValue(NextData[i])==bNextDouble && GetCardColor(NextData[i])>cbNextMaxColor)
						cbNextMaxColor=GetCardColor(NextData[i]);
				}
				return (cbFirstMaxColor>cbNextMaxColor)?1:0;
			}
