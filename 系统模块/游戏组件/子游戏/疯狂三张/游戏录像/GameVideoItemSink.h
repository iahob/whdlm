...
public:		
	...
	//增加录像数据
	virtual bool __cdecl    AddVideoData(WORD wMsgKind,void *pPack,int nSize,bool bFirst=true);
	//获取回放码
	virtual CHAR* __cdecl	GetVideoNumber();
protected:
	void					ResetVideoItem();
	bool					RectifyBuffer(size_t iSize);	
	VOID					BuildVideoNumber(WORD wServerID,WORD wTableID);
