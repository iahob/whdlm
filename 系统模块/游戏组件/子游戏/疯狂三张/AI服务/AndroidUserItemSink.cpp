			...
			else
			{
				...
				if (m_bMingZhu == false)
				{
					...
					else//比牌概率
					{
						...
						{
							lCurrentScore = m_lUserMaxScore - m_lTableScore[wMeChairID];//梭哈
							bShowHand = true;
						}
...
bool CAndroidUserItemSink::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
	...
	{
		//设置定时器
		UINT nElapse = TIME_DISPATCH_CARD + (rand() % TIME_USER_ADD_SCORE);
		m_pIAndroidUserItem->SetGameTimer(IDI_USER_ADD_SCORE, nElapse);
	}
...
bool CAndroidUserItemSink::OnSubCompareCard(const void * pBuffer, WORD wDataSize)
{
	...
	m_bShowHand = pCompareCard->bShowHand;
	//玩家人数
	BYTE cbUserCount = 0;
	for (WORD i=0; i<GAME_PLAYER; i++)
	{
...
bool CAndroidUserItemSink::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	//开始时间
	if (pGameEnd->wEndState == GER_COMPARECARD)//比牌结束
	{
		UINT nElapse = 10 + (rand() % TIME_DELAY_TIME);
		m_pIAndroidUserItem->SetGameTimer(IDI_DELAY_TIME, nElapse);
	}
	else
	{
		UINT nElapse = 5 + (rand() % TIME_START_GAME);
		m_pIAndroidUserItem->SetGameTimer(IDI_START_GAME, nElapse);
	}
