...
CTableFrameSink::CTableFrameSink()
{
	...
	m_hDebugInst = NULL;
    m_pServerDebug = NULL;
	m_hDebugInst = LoadLibrary(TEXT("BaccaratNewServerDebug.dll"));
    if(m_hDebugInst)
    {
        typedef void *(*CREATE)();
        CREATE ServerControl = (CREATE)GetProcAddress(m_hDebugInst, "CreateServerDebug");
        if(ServerControl)
        {
            m_pServerDebug = static_cast<IServerControl *>(ServerControl());
        }
    }
...
bool CTableFrameSink::OnEventGameConclude(WORD wChairID, IServerUserItem *pIServerUserItem, BYTE cbReason)
{
    ...
    case GER_NORMAL:		//常规结束
    {
		...
		//保存上局总注
		CopyMemory(m_lLastAllJettonPlace, m_lPlayBet, sizeof(m_lPlayBet));
		//清空本局最后一注
		ZeroMemory(m_lLastAllJettonArea, sizeof(m_lLastAllJettonArea));
...
bool CTableFrameSink::OnEventSendGameScene(WORD wChiarID, IServerUserItem *pIServerUserItem, BYTE cbGameStatus, bool bSendSecret)
{
    ...
    case GAME_SCENE_FREE:			//空闲状态
    {
		...
		CopyMemory(&StatusFree.lChipArray, &m_lChipArray, sizeof(m_lChipArray));
    ...
    case GAME_SCENE_END:		//结束状态
    {
		...
		CopyMemory(StatusPlay.lChipArray, m_lChipArray, sizeof(m_lChipArray));
		//CopyMemory(StatusPlay.lLastPlayBet, m_lLastAllJettonPlace[wChiarID], sizeof(StatusPlay.lLastPlayBet));
        //占位玩家成绩
        for(WORD i = 0; i < MAX_OCCUPY_SEAT_COUNT; i++)
        {
            if(m_wOccupySeatChairIDArray[i] == INVALID_CHAIR)
...
bool  CTableFrameSink::OnGameMessage(WORD wSubCmdID, VOID *pData, WORD wDataSize, IServerUserItem *pIServerUserItem)
{
	...
	case SUB_C_CONTINUE_JETTON:		//用户加注
	{
		//用户效验
		if (pIServerUserItem->GetUserStatus() != US_PLAYING) return true;

		//消息处理
		return OnUserRePlayBet(pIServerUserItem->GetChairID());
	}
...
bool CTableFrameSink::OnFrameMessage(WORD wSubCmdID, VOID *pData, WORD wDataSize, IServerUserItem *pIServerUserItem)
{
    ...
    if(wSubCmdID >= SUB_GF_FRAME_MESSAG_GAME_MIN && wSubCmdID <= SUB_GF_FRAME_MESSAG_GAME_MAX)
    {
        ...
        case SUB_C_AMDIN_COMMAND:
        {
            ...
            return m_pServerDebug->ServerControl(wSubCmdID, pData, wDataSize, pIServerUserItem, m_pITableFrame, m_pGameServiceOption);
        }
    ...
    // 聊天消息
    if(wSubCmdID == SUB_GF_USER_CHAT && CUserRight::IsGameDebugUser(pIServerUserItem->GetUserRight()))
    {
        //变量定义
...
bool CTableFrameSink::OnUserPlayBet(WORD wChairID, BYTE cbBetArea, LONGLONG lBetScore)
{
    ...
    if(GetMaxPlayerScore(cbBetArea, wChairID) >= lBetScore)
    {
		...
		m_lLastAllJettonArea[wChairID][cbBetArea] += lBetScore;
...
bool CTableFrameSink::OnUserRePlayBet(WORD wChairID) 
{
	//玩家积分
	IServerUserItem * pIServerUserItem = m_pITableFrame->GetTableUserItem(wChairID);
	...
	for (int nAreaIndex = 0; nAreaIndex < AREA_MAX; ++nAreaIndex)
	{
		lLastAllJettonArea += m_lLastAllJettonArea[pIServerUserItem->GetChairID()][nAreaIndex];
	}

	//统计上局总下注
	for (int nAreaIndex = 0; nAreaIndex < AREA_MAX; ++nAreaIndex)
	{
		lLastAllJettonPlace += m_lLastAllJettonPlace[pIServerUserItem->GetChairID()][nAreaIndex];
	}

	//统计本局已下注
	for (int nAreaIndex = 0; nAreaIndex < AREA_MAX; ++nAreaIndex)
	{
		lUserJettonScore += m_lPlayBet[pIServerUserItem->GetChairID()][nAreaIndex];
	}
	...
	{
		//本局有最后下一注
		if (m_lLastAllJettonArea[pIServerUserItem->GetChairID()][nAreaIndex] > 0)
		{
			bIsLast = false;
			break;
		}
	...
	if (!bCheckPlaceJettonIsOk)
	{
		...
		SendContinuePlaceBetFail(pIServerUserItem->GetChairID());
		return true;
	}
	...
	if (bIsLast) //无最后一注
	{
		for (int nAreaIndex = 0; nAreaIndex < AREA_MAX; nAreaIndex++)
		{
			if (m_lLastAllJettonPlace[pIServerUserItem->GetChairID()][nAreaIndex] + m_lPlayBet[pIServerUserItem->GetChairID()][nAreaIndex] > m_lAreaLimitScore)
			{
				bCheckPlaceJettonIsOk = false;
				break;
			}
	...
	else  //有最后一注
	{
		for (int nAreaIndex = 0; nAreaIndex < AREA_MAX; nAreaIndex++)
		{
			if (m_lLastAllJettonArea[pIServerUserItem->GetChairID()][nAreaIndex] + m_lPlayBet[pIServerUserItem->GetChairID()][nAreaIndex] > m_lAreaLimitScore)
			{
				bCheckPlaceJettonIsOk = false;
				break;
			}
	...
	//保存数据
	LONGLONG  TemplAllJettonScore[AREA_MAX];           //全体总注
	LONGLONG  TemplUserJettonScore[GAME_PLAYER][AREA_MAX];   //个人总注
	LONGLONG  TemplLastAllJettonPlace[GAME_PLAYER][AREA_MAX]; //上局总下注
	LONGLONG  TempLastAllJettonArea[GAME_PLAYER][AREA_MAX];   //本局最后下注

	CopyMemory(TemplAllJettonScore, m_lAllBet, sizeof(m_lAllBet));
	CopyMemory(TemplUserJettonScore, m_lPlayBet, sizeof(m_lPlayBet));
	CopyMemory(TemplLastAllJettonPlace, m_lLastAllJettonPlace, sizeof(m_lLastAllJettonPlace));
	CopyMemory(TempLastAllJettonArea, m_lLastAllJettonArea, sizeof(m_lLastAllJettonArea));
	...
	if (bIsLast == true)
	{
		if (bCheckPlaceJettonIsOk == true)
		{
			...
			for (int nAreaIndex = 0; nAreaIndex < AREA_MAX; nAreaIndex++)
			{
				...
				//全体总注
				TemplAllJettonScore[nAreaIndex] += TemplLastAllJettonPlace[pIServerUserItem->GetChairID()][nAreaIndex];
				//个人总注
				TemplUserJettonScore[pIServerUserItem->GetChairID()][nAreaIndex] += TemplLastAllJettonPlace[pIServerUserItem->GetChairID()][nAreaIndex];
	...
	else
	{
		...
		if (bCheckPlaceJettonIsOk == true)
		{
			...
			for (int nAreaIndex = 0; nAreaIndex < AREA_MAX; nAreaIndex++)
			{
				...
				//本局最后一次操作 不变
				//全体总注
				TemplAllJettonScore[nAreaIndex] += TempLastAllJettonArea[pIServerUserItem->GetChairID()][nAreaIndex];
				//个人总注
				TemplUserJettonScore[pIServerUserItem->GetChairID()][nAreaIndex] += TempLastAllJettonArea[pIServerUserItem->GetChairID()][nAreaIndex];
	...
	{
		//变量定义
		CMD_S_ContinuePlaceBet PlaceLastJetton;
		ZeroMemory(&PlaceLastJetton, sizeof(PlaceLastJetton));

		//构造变量
		PlaceLastJetton.wChairID = pIServerUserItem->GetChairID();

		//设置变量
		CopyMemory(m_lAllBet, TemplAllJettonScore, sizeof(TemplAllJettonScore));
		CopyMemory(m_lPlayBet, TemplUserJettonScore, sizeof(TemplUserJettonScore));
		...
		for (int nAreaIndex = 0; nAreaIndex < AREA_MAX; nAreaIndex++)
		{
			PlaceLastJetton.lBetScore[nAreaIndex] = m_lPlayBet[pIServerUserItem->GetChairID()][nAreaIndex];
		}
		//发送消息
		m_pITableFrame->SendTableData(INVALID_CHAIR, SUB_S_CONTINUE_JETTON, &PlaceLastJetton, sizeof(PlaceLastJetton));
		m_pITableFrame->SendLookonData(INVALID_CHAIR, SUB_S_CONTINUE_JETTON, &PlaceLastJetton, sizeof(PlaceLastJetton));
	}
	else SendContinuePlaceBetFail(pIServerUserItem->GetChairID());

	return true;
}

void CTableFrameSink::SendContinuePlaceBetFail(WORD wChairID)
{
	CMD_S_ContinuePlaceBetFail PlaceBetFail;
	PlaceBetFail.wChairID = wChairID;
	m_pITableFrame->SendTableData(wChairID, SUB_S_CONTINUE_JETTON_FAI, &PlaceBetFail, sizeof(PlaceBetFail));
}
//发送消息
...
{
    m_bControl = false;
    if(m_pServerDebug && m_pServerDebug->NeedControl() && m_pServerDebug->ControlResult(m_cbTableCardArray[0], m_cbCardCount))
    {
        m_bControl = true;
    }
...
void CTableFrameSink::ReadConfigInformation()
{
	...
	memcpy(m_lChipArray, pCustomConfig->CustomGeneral.lChipArray, sizeof(m_lChipArray));
    return;
}
