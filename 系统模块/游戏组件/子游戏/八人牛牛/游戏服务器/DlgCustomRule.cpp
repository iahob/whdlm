...
bool CDlgCustomRule::FillDebugToData()
{
	...
	{
		AfxMessageBox(TEXT("开始有效时间5-15秒，请重新设置！"), MB_ICONSTOP);
		return false;
	}

	if (!(m_CustomRule.nTimeCallBanker >= 5 && m_CustomRule.nTimeCallBanker <= 15))
	{
		AfxMessageBox(TEXT("叫庄有效时间5-15秒，请重新设置！"), MB_ICONSTOP);
		return false;
	}

	if (!(m_CustomRule.nTimeAddScore >= 5 && m_CustomRule.nTimeAddScore <= 15))
	{
		AfxMessageBox(TEXT("下注有效时间5-15秒，请重新设置！"), MB_ICONSTOP);
		return false;
	}

	if (!(m_CustomRule.nTimeOpenCard >= 5 && m_CustomRule.nTimeOpenCard <= 15))
	{
		AfxMessageBox(TEXT("摊牌有效时间5-15秒，请重新设置！"), MB_ICONSTOP);
		return false;
	}
