...
//混乱扑克
VOID CGameLogic::RandCardList(BYTE cbCardBuffer[][ITEM_X_COUNT], double dControl)
{
	dControl /= 105;//0.47 
			...
			if(rand() % 2 == 0)
			{
				for (int j = 0; j < ITEM_X_COUNT; j++)
				{
					if (cbCardData[i][j] != 0xFF)
					{
						...
						{
							cbCardBuffer[m_ptXian[i][j].x][m_ptXian[i][j].y] = CT_SHUIHUZHUAN;
							cbShuiHuZhuan--;
						}
					}					
				}
			...
			else
			{
				for (int j = 0; j < ITEM_X_COUNT; j++)
				{
					if (cbCardData[i][j] != 0xFF)
					{
						...
						{
							cbCardBuffer[m_ptXian[i][ITEM_X_COUNT - j - 1].x][m_ptXian[i][ITEM_X_COUNT - j - 1].y] = CT_SHUIHUZHUAN;
							cbShuiHuZhuan--;
						}
					}					
				}
