...
{
	CString strName;
	strName.Format(TEXT("%s\\水浒传[%s].log"), GetFileDialogPath(), strRoomName);
...
static bool NcaTextOut(UINT uLine, IServerUserItem * pISendUserItem = NULL, CString str = TEXT(""), bool bRet = false)
{
	...
	CreateDirectory(GetFileDialogPath() + TEXT("\\WaterMarginLog"), NULL);

	CTime time = CTime::GetCurrentTime() ;

	CString strName;
	strName.Format(TEXT("%s\\WaterMarginLog\\WaterMargin%04d%02d%02d.log"), 
		GetFileDialogPath(),
		time.GetYear(),
		time.GetMonth(),
		time.GetDay()
