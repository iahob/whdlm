...
BOOL CGameFrameWindow::PreTranslateMessage(MSG * pMsg)
{
	...
	if (pMsg->message == WM_KEYDOWN && bKeyUp)
	{
		bKeyUp = false;
		//变量定义
		...
		if(pGameClientEngine != NULL)
		{
			...
			if (szClassName != CString(TEXT("Edit")) )
			{
				pGameClientEngine->PostMessage(IDM_GAME_OPERATE,pMsg->wParam,0);
				return TRUE;
			}
		}
	}
	else if (pMsg->message == WM_KEYUP || pMsg->message == WM_ACTIVATE)
	{
		bKeyUp = true;
	}
	...
	case IDC_CLOSE:				// 关闭按钮
		{
			...
			IGameFrameEngine* pIGameFrameEngine=(IGameFrameEngine *)pGlobalUnits->QueryGlobalModule(MODULE_GAME_FRAME_ENGINE,IID_IGameFrameEngine,VER_IGameFrameEngine);
			CGameClientEngine* pGameClientEngine = (CGameClientEngine*)pIGameFrameEngine;
			pGameClientEngine->PostMessage(IDM_GAME_CLOSE, 1, 0);
			return TRUE;
		}
	}
...
VOID CGameFrameWindow::RectifyControl(INT nWidth, INT nHeight)
{
	...
	{
		::SetWindowPos(pIGameFrameView->GetGameViewHwnd(), NULL, 0, 0, nWidth, nHeight, SWP_NOACTIVATE|SWP_NOCOPYBITS|SWP_NOZORDER);
	}
	...
	{
		CRect rcArce;
		CSize SizeRestrict(nWidth,nHeight);
...
INT CGameFrameWindow::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// 读取位置
	CSize SizeRestrict(Screen_Width,Screen_Height);
	// 正常位置
	CRect rcNormalSize;			
...
void CGameFrameWindow::OnActivate(UINT nState, CWnd* pWndOther, BOOL bMinimized)
{
	__super::OnActivate(nState, pWndOther, bMinimized);

// 	if(nState && IsWindowVisible())
// 	{
// 		// 窗口位置
// 		CRect rcArce;
// 		SystemParametersInfo(SPI_GETWORKAREA,0,&rcArce,SPIF_SENDCHANGE);
