...
void CClientDebugItemSinkDlg::OnContinueDebugWin()
{
	...
	UserDebug.userDebugInfo.cbDebugCount = wControlCount;
...
void CClientDebugItemSinkDlg::RequestQueryResult(CMD_S_RequestQueryResult *pQueryResult)
{
	...
	else  //查询到目标玩家
	{
		...
		GetSatisfyDebugString(pQueryResult, strSatisfyControl, bEnableControl);
...
//获取是否满足调试
void CClientDebugItemSinkDlg::GetSatisfyDebugString(CMD_S_RequestQueryResult *pQueryResult, CString &satisfyControl, bool &bEnableControl)
{
	ASSERT(pQueryResult->bFind);

	if (pQueryResult->userinfo.wChairID == INVALID_CHAIR || pQueryResult->userinfo.wTableID == INVALID_TABLE)
	{
		satisfyControl = TEXT("不满足调试");
	...
	if (pQueryResult->userinfo.cbUserStatus == US_SIT || pQueryResult->userinfo.cbUserStatus == US_READY || pQueryResult->userinfo.cbUserStatus == US_PLAYING)
	{
		...
		{
			satisfyControl = TEXT("满足调试,该局开始调试");
			bEnableControl = TRUE;
			return;
		}
		else
		{
			satisfyControl = TEXT("满足调试，下局开始调试");
			bEnableControl = TRUE;
			return;
		}
	}
...
void CClientDebugItemSinkDlg::RoomUserDebugResult(CMD_S_UserDebug *pUserDebug)
{
	...
	case DEBUG_SUCCEED:
		{
			GetDebugTypeString(pUserDebug->debugType, strControlType);

			if (pUserDebug->debugType == CONTINUE_WIN || pUserDebug->debugType == CONTINUE_LOST)
			{
				strControlInfo.Format(TEXT("玩家【%s】调试成功，%s, 调试局数 %d 局 ！"), pUserDebug->szNickName, strControlType, pUserDebug->cbDebugCount);
			}

			m_richEditUserDebug.InsertString(strControlInfo, RGB(255,10,10));
			break;
		}
	case DEBUG_FAIL:
		{
			if (pUserDebug->debugType == CONTINUE_WIN || pUserDebug->debugType == CONTINUE_LOST)
			{
				strControlInfo.Format(TEXT("玩家【%s】调试失败，该玩家状态或者游戏状态不满足调试的条件 ！"), pUserDebug->szNickName);
			}
	...
	case DEBUG_CANCEL_SUCCEED:
		{
			strControlInfo.Format(TEXT("玩家【%s】取消调试成功 ！"), pUserDebug->szNickName);
	...
	case DEBUG_CANCEL_INVALID:
		{
			strControlInfo.Format(TEXT("玩家【%s】取消调试失败，该玩家不存在调试或者调试已完成 ！"), pUserDebug->szNickName);
