...
BYTE CGameLogic::GetSpecialType(BYTE bHandCardData[], BYTE bCardCount)
{
	...
	do 
	{
		...
		for(BYTE j=0;j<4;j++)
		{
			if(cbSameCardCount[j]%5!=0 && cbSameCardCount[j]%5!=3)
			{
				bCondition=false;
				break;
			}
		...
		for(BYTE j=0;j<4;j++)
		{
			if(cbSameCardCount[j]==0) continue;

			if(cbSameCardCount[j]==3 || cbSameCardCount[j]==5)
			{
				if(IsLinkCard(cbSameCardData[j],cbSameCardCount[j])==false)
				{
					bCondition=false;
					break;
				}
			}
			else if(cbSameCardCount[j]==8 || cbSameCardCount[j]==10)
			{
				if(SetLinkCard(cbSameCardData[j],cbSameCardCount[j],NULL)==false)
				{
					bCondition=false;
					break;
				}
		...
		if(bCondition) return CT_EX_SANTONGHUASHUN;
	} while (0);
	//三分天下
	...
	*/
	//六对半
	if (6 == GetDoubleCount(bCardData, bCardCount, NULL))
		return CT_EX_LIUDUIBAN;
	
	//三顺子
	if(SetLinkCard(bCardData, bCardCount,NULL))
	...
	do 
	{
		...
		if(bCondition)	return CT_EX_SANTONGHUA;
	}while(0);
...
BYTE CGameLogic::GetSpecialCardData(BYTE bHandCardData[HAND_CARD_COUNT], BYTE bHandCardCount, BYTE bSpecCardData[3][5])
{
	...
	{
		//同花变量
		BYTE cbSameColorCount[4] = {0};
		BYTE cbSameColorData[4][13] = {0};
		...
		for (BYTE i = 0; i < bCardCount; i++)
		{
			...
			//原牌数目
			BYTE cbCount = cbSameColorCount[cbCardColor];

			//追加扑克
			cbSameColorData[cbCardColor][cbCount] = bCardData[i];
			cbSameColorCount[cbCardColor]++;
		}
		...
		for(BYTE j=0;j<4;j++)
		{
			if(cbSameColorCount[j]%5!=0 && cbSameColorCount[j]%5!=3)
			{
				bCondition=false;
				break;
			}
		...
		BYTE bLinkCardData[3][5];
		ZeroMemory(bLinkCardData,sizeof(bLinkCardData));
		//判断顺子
		for(BYTE j=0;j<4;j++)
		{
			if(cbSameColorCount[j]==0) continue;

			if(cbSameColorCount[j]==3)
			{
				if(IsLinkCard(cbSameColorData[j],cbSameColorCount[j])==false)
				{
					bCondition=false;
					break;
				}
				...
				{
					CopyMemory(&bLinkCardData[0],&cbSameColorData[j],sizeof(BYTE)*cbSameColorCount[j]);
				}
			}
			else if(cbSameColorCount[j]==5)
			{
				if(IsLinkCard(cbSameColorData[j],cbSameColorCount[j])==false)
				{
					bCondition=false;
				...
				{
					//降序排序 先放后墩
					if(bLinkCardData[2][0]==0)
					{
						CopyMemory(&bLinkCardData[2],&cbSameColorData[j],sizeof(BYTE)*cbSameColorCount[j]);
					}
					else
					{
						CopyMemory(&bLinkCardData[1],&cbSameColorData[j],sizeof(BYTE)*cbSameColorCount[j]);
					}
				}
			}
			else if(cbSameColorCount[j]==8 || cbSameColorCount[j]==10)
			{
				if(SetLinkCard(cbSameColorData[j],cbSameColorCount[j],bLinkCardData)==false)
				{
					bCondition=false;
					break;
				}
		...
		if(bCondition)
		{
			CopyMemory(bSpecCardData,bLinkCardData,sizeof(bLinkCardData));
			//防止乌龙
	...
	//三分天下 (AAAA KKKK QQQQ J)
	if(AnalyseData.bFourCount==3 && AnalyseData.bOneCount==1)
	{
		//填充后墩 (AAAA J)
		CopyMemory(&bSpecCardData[2],&bCardData[AnalyseData.bFourFirst[0]],sizeof(BYTE)*4);
		CopyMemory(&bSpecCardData[2][4],&bCardData[AnalyseData.bOneFirst[0]],sizeof(BYTE)*1);
		
		//填充中墩(KKKK Q)
		CopyMemory(&bSpecCardData[1],&bCardData[AnalyseData.bFourFirst[1]],sizeof(BYTE)*4);
		CopyMemory(&bSpecCardData[1][4],&bCardData[AnalyseData.bFourFirst[2]+3],sizeof(BYTE)*1);

		//填充前墩(QQQ)
		CopyMemory(&bSpecCardData[0],&bCardData[AnalyseData.bFourFirst[2]],sizeof(BYTE)*3);

		return 1;
	}
	//四套三条(AAA KKK QQQ JJJ 10)
	if(AnalyseData.bThreeCount==4 && AnalyseData.bOneCount==1)
	{
		//填充后墩 (AAA JJ)
		CopyMemory(&bSpecCardData[2],&bCardData[AnalyseData.bThreeFirst[0]],sizeof(BYTE)*3);
		CopyMemory(&bSpecCardData[2][3],&bCardData[AnalyseData.bThreeFirst[3]],sizeof(BYTE)*2);

		//填充中墩(KKK J 10)
		CopyMemory(&bSpecCardData[1],&bCardData[AnalyseData.bThreeFirst[1]],sizeof(BYTE)*4);
		CopyMemory(&bSpecCardData[1][3],&bCardData[AnalyseData.bThreeFirst[3]+2],sizeof(BYTE)*1);
		CopyMemory(&bSpecCardData[1][4],&bCardData[AnalyseData.bOneFirst[0]],sizeof(BYTE)*1);

		//填充前墩(QQQ)
		CopyMemory(&bSpecCardData[0],&bCardData[AnalyseData.bThreeFirst[2]],sizeof(BYTE)*3);
		return 1;
	}
	*/

	if (GetDoubleCount(bCardData, bCardCount, bSpecCardData) == 6) return 1;

	//三顺子
	if(SetLinkCard(bCardData,bCardCount,bSpecCardData))
	{
		//防止乌龙
		if (enCRGreater != CompareCard(bSpecCardData[1], bSpecCardData[2], 5, 5, true))
		{
	...
	do 
	{
		...
		if(bCondition)
		{
			for(BYTE j=0;j<4;j++)
			{
				...
				if(cbSameColorCount[j]==3)
				{
					CopyMemory(&bSpecCardData[0],cbSameColorData[j],sizeof(BYTE)*3);
				}
				else if(cbSameColorCount[j]==5)
				{
					//先放尾墩
					if(bSpecCardData[2][0]==0)
						CopyMemory(&bSpecCardData[2],cbSameColorData[j],sizeof(BYTE)*5);
					else
						CopyMemory(&bSpecCardData[1],cbSameColorData[j],sizeof(BYTE)*5);
				}
				else if(cbSameColorCount[j]==8)
				{
					CopyMemory(&bSpecCardData[1],cbSameColorData[j],sizeof(BYTE)*5);
					CopyMemory(&bSpecCardData[0],&(cbSameColorData[j][5]),sizeof(BYTE)*3);
				}
				else if(cbSameColorCount[j]==10)
				{
					CopyMemory(&bSpecCardData[2],cbSameColorData[j],sizeof(BYTE)*5);
					CopyMemory(&bSpecCardData[1],&(cbSameColorData[j][5]),sizeof(BYTE)*5);
...
BYTE CGameLogic::AutoPutCard(BYTE bCardData[], BYTE bPutCardData[], bool bIsMobileUser, BYTE bPutMobileCard[4][HAND_CARD_COUNT], BYTE cbSpecialType, BYTE cbSpecialData[3][5])
{
	...
	for (bSubscript[0] = 0; bSubscript[0] < 13; bSubscript[0] ++)
	{
		for (bSubscript[1] = bSubscript[0] + 1; bSubscript[1] < 13; bSubscript[1] ++)
		{
			for (bSubscript[2] = bSubscript[1] + 1; bSubscript[2] < 13; bSubscript[2] ++)
			{
				...
				{
					if (!blBiaoJi[bSubscript[3]])
					{
						for (bSubscript[4] = bSubscript[3] + 1; bSubscript[4] < 13; bSubscript[4] ++)
						{
							if (!blBiaoJi[bSubscript[4]])
							{
								for (bSubscript[5] = bSubscript[4] + 1; bSubscript[5] < 13; bSubscript[5] ++)
								{
									if (!blBiaoJi[bSubscript[5]])
									{
										for (bSubscript[6] = bSubscript[5] + 1; bSubscript[6] < 13; bSubscript[6] ++)
										{
											if (!blBiaoJi[bSubscript[6]])
											{
												for (bSubscript[7] = bSubscript[6] + 1; bSubscript[7] < 13; bSubscript[7] ++)
												{
													if (!blBiaoJi[bSubscript[7]])
													{
														do
														{
															//从大到小将牌保存到bCard[1]数组中
															for (int i = 0; i < 5; i++)
																bCard[1][4 - i] = bCardList[bSubscript[3 + i]];

															iFWeights = GetCardType(bCard[2], 3, bWCard);
															iSWeights = GetCardType(bCard[1], 5, bWCard);

															if (CompareCard(bCard[2], bCard[1], 3, 5, false) != enCRGreater)
																break;
															...
															iTWeights = GetCardType(bCard[0], 5, bWCard);

															if (CompareCard(bCard[0], bCard[1], 5, 5, true) == enCRGreater)
																break;
															bool bSet = false;
															for (int i = 0; i < 6; i++)
															{
															...
															{
																//检查是否与特殊牌型相同
																if (cbSpecialType > CT_EX_INVALID)
																{
																	if (IsSameCard(cbSpecialData[0], bCard[2], 3) && (IsSameCard(cbSpecialData[1], bCard[1], 5) || IsSameCard(cbSpecialData[1], bCard[0], 5)))
																		break;
																}
																bool bFirstThree = false;
																BYTE cbFirstThreeIndex = 0;
																//不存在就保存
																int i = 0;
																for (i = 0; i < 6; i++)
																{
																	if (iTempWeights[i][2] >= CT_THREE)
																	{
																		bFirstThree = true;
																		cbFirstThreeIndex = i;
																	}
																...
																if (i == 6 && iFWeights >= CT_THREE)//队列已满
																{
																	if (bFirstThree == false || iFWeights > iTempWeights[cbFirstThreeIndex][2])
																	{
																		BYTE cbTempindex = bFirstThree ? cbFirstThreeIndex : 5;
																		iTempWeights[cbTempindex][2] = iFWeights;
																		iTempWeights[cbTempindex][1] = iSWeights;
																		iTempWeights[cbTempindex][0] = iTWeights;

																		for (int j = 0; j < 3; j++)
																		{
																			CopyMemory(bTempCard[cbTempindex][j], bCard[j], sizeof(BYTE) * 5);
																		}
																	}
																}
															}
	...
	for (int i = 0; i < 6; i++)
	{
		for (int j = i + 1; j < 6; j++)
		{
			if ((iTempWeights[j][0] > iTempWeights[i][0] && iTempWeights[i][2]<CT_THREE) || (iTempWeights[j][2] >= CT_THREE && iTempWeights[j][2]>iTempWeights[i][2]))
			{
				for (int k = 0; k < 3; k++)
				{
					int iTemp;
	...
	for (int i = 0; i < 6; i++)
	{
		if (iTempWeights[i][0] == 0 && iTempWeights[i][1] == 0 && iTempWeights[i][2] == 0) continue;
		nPutCount++;
	}
	//牌组填充
	CopyMemory(bPutCardData, bTempCard[0][2], sizeof(BYTE) * 3);
	CopyMemory(&bPutCardData[3], bTempCard[0][1], sizeof(BYTE) * 5);
	CopyMemory(&bPutCardData[8], bTempCard[0][0], sizeof(BYTE) * 5);
	...
	//防止乌龙
	if (enCRGreater != CompareCard(&bPutCardData[3], &bPutCardData[8], 5, 5, true))
	{
		BYTE cbCardTemp[5] = { 0 };
		CopyMemory(cbCardTemp, &bPutCardData[3], sizeof(cbCardTemp));
		CopyMemory(&bPutCardData[3], &bPutCardData[8], sizeof(BYTE) * 5);
		CopyMemory(&bPutCardData[8], cbCardTemp, sizeof(BYTE) * 5);
	}

	if (bIsMobileUser && bPutMobileCard != NULL)
	{
		ZeroMemory(bPutMobileCard, sizeof(bPutMobileCard));
		BYTE cbMobileCount = min(nPutCount, 3);
		for (BYTE i = 0; i < cbMobileCount; i++)
		{
			//牌组填充
			CopyMemory(bPutMobileCard[i], bTempCard[i][2], sizeof(BYTE) * 3);
			CopyMemory(&bPutMobileCard[i][3], bTempCard[i][1], sizeof(BYTE) * 5);
			CopyMemory(&bPutMobileCard[i][8], bTempCard[i][0], sizeof(BYTE) * 5);

			//防止乌龙
			if (enCRGreater != CompareCard(&bPutMobileCard[i][3], &bPutMobileCard[i][8], 5, 5, true))
			{
				BYTE cbCardTemp[5] = { 0 };
				CopyMemory(cbCardTemp, &bPutMobileCard[i][3], sizeof(cbCardTemp));
				CopyMemory(&bPutMobileCard[i][3], &bPutMobileCard[i][8], sizeof(BYTE) * 5);
				CopyMemory(&bPutMobileCard[i][8], cbCardTemp, sizeof(BYTE) * 5);
			}
		}
	}

	return min(nPutCount, 3);
}
			...
			case CT_SINGLE:				//单牌类型
				{
					...
					//牌面相同
					return GetCardColor(bNextList[0])>GetCardColor(bFirstList[0])?enCRGreater:enCRLess ;
				}
			case CT_ONE_DOUBLE:			//对带一张
				{
					if(GetCardLogicValue(bNextList[NextAnalyseData.bTwoFirst[0]])==GetCardLogicValue(bFirstList[FirstAnalyseData.bTwoFirst[0]]))
					{
						...
						for(BYTE i=0 ; i<NextAnalyseData.bOneCount ; ++i)
						{
						...
						//牌面相同
						return GetCardColor(bNextList[NextAnalyseData.bOneFirst[0]])>GetCardColor(bFirstList[FirstAnalyseData.bOneFirst[0]])?enCRGreater:enCRLess ;
					}
					else 
					{
						if (GetCardLogicValue(bNextList[NextAnalyseData.bTwoFirst[0]]) > GetCardLogicValue(bFirstList[FirstAnalyseData.bTwoFirst[0]]))
...
bool CGameLogic::IsLinkCard(const BYTE cbCardData[], BYTE cbCardCount)
{
	...
	{
		BYTE cbFirstCard=GetCardLogicValue(cbCardBuffer[1]);

		for (WORD i=2; i<cbCardCount; i++)
		{
			BYTE cbNextCard=GetCardLogicValue(cbCardBuffer[i]);
			if (cbFirstCard != cbNextCard+i-1)
			{
				return false;
			}
		}

		//除开A是顺子 如果最后一张是2 总体就是顺子
...
//获取对数
BYTE CGameLogic::GetDoubleCount(BYTE bHandCardData[], BYTE bCardCount, BYTE bSpecCardData[3][5])
{
	BYTE cbCardBuffer[13] = { 0 };
	CopyMemory(cbCardBuffer, bHandCardData, sizeof(BYTE)*bCardCount);
	BYTE cbBufferCount = bCardCount;
	//降序排列
	SortCardList(cbCardBuffer, cbBufferCount, enDescend);

	//定义变量
	BYTE bDoubleCardList[13][2];
	BYTE bDoubleIndex = 0;
	ZeroMemory(bDoubleCardList, sizeof(bDoubleCardList));
	BYTE bLogicValue = GetCardLogicValue(cbCardBuffer[0]);
	BYTE bFristCard = cbCardBuffer[0];

	//对子排列
	for (BYTE i = 1; i < cbBufferCount; i++)
	{
		BYTE bCardValueTemp = GetCardLogicValue(cbCardBuffer[i]);
		//对子
		if (bCardValueTemp == bLogicValue)
		{
			bDoubleCardList[bDoubleIndex][0] = bFristCard;
			bDoubleCardList[bDoubleIndex][1] = cbCardBuffer[i];

			bDoubleIndex++;

			if (i + 1 < cbBufferCount)
			{

				bLogicValue = GetCardLogicValue(cbCardBuffer[i + 1]);
				bFristCard = cbCardBuffer[i + 1];
				i += 1;
			}
		}
		else
		{
			...
			bLogicValue = bCardValueTemp;
			bFristCard = cbCardBuffer[i];
		}
	}

	//统计对子个数
	BYTE bDoubleCont = 0;
	BYTE bOutCardList[6][2];
	BYTE bTmpSigleCard = 0;
	for (BYTE j = 0; j < bDoubleIndex; j++)
	{
		if (bDoubleCardList[j][1] == 0)
		{

			if (bTmpSigleCard == 0) bTmpSigleCard = bDoubleCardList[j][0];

			continue;
		}

		CopyMemory(bOutCardList[bDoubleCont], bDoubleCardList[j], sizeof(BYTE) * 2);
		bDoubleCont++;
	}

	if (bTmpSigleCard == 0)
		bTmpSigleCard = bFristCard;
	//六对半 （e.g. AA KK QQ JJ 99 77 2）后墩 (AA KK 7)中墩 (QQ JJ 7)前墩 (99 2)
	if (bDoubleCont >= 6 && bSpecCardData != NULL)
	{
		ZeroMemory(bSpecCardData, sizeof(bSpecCardData));

		//后墩
		CopyMemory(bSpecCardData[2], bOutCardList, sizeof(BYTE) * 4);
		bSpecCardData[2][4] = bOutCardList[5][0];
		//中墩
		CopyMemory(bSpecCardData[1], &bOutCardList[2], sizeof(BYTE) * 4);
		bSpecCardData[1][4] = bOutCardList[5][1];
		//前墩
		CopyMemory(bSpecCardData[0], &bOutCardList[4], sizeof(BYTE) * 2);
		bSpecCardData[0][2] = bTmpSigleCard;
	}
		...
		if (cbTakeCardResultCount > 0)
		{
			for (BYTE i=0; i<cbSameCardResultCount; i++)
			{
				...
				for (BYTE j=0; j<cbTakeCardResultCount; j++)
				{
					...
					if (GetCardLogicValue(SameCardResult.cbResultCard[i][0]) == GetCardLogicValue(TakeCardResult.cbResultCard[cbTakeCardResultCount-1-j][0]))
						continue;
					...
					CopyMemory(&pSearchCardResult->cbResultCard[cbResultCount][cbSameCount], TakeCardResult.cbResultCard[cbTakeCardResultCount-1-j], sizeof(BYTE)*cbTakeCardCount);
					cbResultCount++;
				}
			}
		}
...
BYTE CGameLogic::SearchLineCardType( const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbLineCount, tagSearchCardResult *pSearchCardResult )
{
	...
	//交换顺序（从大到小排列）
	for(int i=0;i<cbResultCount/2;i++)
	{
		BYTE cbTmpCard[13];
		ZeroMemory(cbTmpCard,sizeof(cbTmpCard));

		CopyMemory(cbTmpCard,pSearchCardResult->cbResultCard[i],sizeof(BYTE)*13);
		CopyMemory(pSearchCardResult->cbResultCard[i],pSearchCardResult->cbResultCard[cbResultCount-i-1],sizeof(BYTE)*13);
		CopyMemory(pSearchCardResult->cbResultCard[cbResultCount-i-1],cbTmpCard,sizeof(BYTE)*13);
	}
	return cbResultCount;
}
...
BYTE CGameLogic::SearchSameColorLineType(const BYTE cbHandCardData[], BYTE cbHandCardCount, BYTE cbLineCount, tagSearchCardResult *pSearchCardResult)
{
	...
	for (BYTE i = 0; i < cbHandCardCount; i++)
	{
		...
		//追加扑克
		cbSameColorData[cbCardColor][cbCount] = cbHandCardData[i];
		cbSameColorCount[cbCardColor]++;
	}

	tagSearchCardResult tagTempResult;
	ZeroMemory(&tagTempResult,sizeof(tagSearchCardResult));
	//同花中搜索顺子
	for (int i = 0; i < 4; i++)
	{
		if(cbSameColorCount[i]<cbLineCount) continue;

		BYTE cbLineResultCount = SearchLineCardType(cbSameColorData[i],cbSameColorCount[i] ,cbLineCount, &tagTempResult);
		if (cbLineResultCount > 0)
		{
			pSearchCardResult->cbCardCount[cbResultCount] = cbLineCount;
			CopyMemory(pSearchCardResult->cbResultCard[cbResultCount], tagTempResult.cbResultCard[0], sizeof(BYTE)*cbLineCount);
			cbResultCount++;
		}
	}
...
bool CGameLogic::IsSameCard(BYTE cbACard[], BYTE cbBCard[], BYTE cbCardCount)
{
	if (cbCardCount <= 0)
		return false;
	...
	BYTE cbTempA[5] = { 0 };
	BYTE cbTempB[5] = { 0 };
	CopyMemory(cbTempA, cbACard, cbCardCount);
	CopyMemory(cbTempB, cbBCard, cbCardCount);
	SortCardList(cbTempA, cbCardCount, enDescend);
	SortCardList(cbTempB, cbCardCount, enDescend);
...
int CGameLogic::CardTypeToWeights(BYTE bType, BYTE cbIndex)
{
	...
	case CT_FIVE_STRAIGHT_FLUSH_BACK_A:									//同花顺牌
	{
		if (2 == cbIndex)
		{
			return (13 + 10);
...
//设置顺子
bool CGameLogic::SetLinkCard(const BYTE cbCardData[], BYTE cbCardCount,BYTE bSpecCardData[3][5])
{
	//校验牌数目
	ASSERT(cbCardCount%5==0 || cbCardCount%5==3);
	if(cbCardCount%5!=0 && cbCardCount%5!=3 || cbCardCount==0) return false;

	//复制数据
	BYTE cbLineCardData[3][5];
	ZeroMemory(cbLineCardData,sizeof(cbLineCardData));
	...
	{
		for(BYTE i=0;i<cbLineCardResult5;i++)
		{
			BYTE cbTmpCardData[13];
			CopyMemory(cbTmpCardData,cbCardData,sizeof(BYTE)*cbCardCount);
			BYTE cbTmpCount=cbCardCount;
			RemoveCard(LineCardResult.cbResultCard[i], LineCardResult.cbCardCount[i],cbTmpCardData,cbTmpCount);
			cbTmpCount -= LineCardResult.cbCardCount[i];

			if(cbTmpCount==8)
			{
				if(SetLinkCard(cbTmpCardData,cbTmpCount,cbLineCardData)==false)
				{
					continue;
				}
				else
				{
					CopyMemory(&cbLineCardData[2],LineCardResult.cbResultCard[i],sizeof(BYTE)*5);
					if(bSpecCardData!=NULL)
					{
						CopyMemory(bSpecCardData,cbLineCardData,sizeof(cbLineCardData));
					}
					return true;
				}
			}

			if(IsLinkCard(cbTmpCardData,cbTmpCount))
			{
				//八张牌 放前中墩
				if(cbTmpCount==3)
				{
					CopyMemory(&cbLineCardData[0],cbTmpCardData,sizeof(BYTE)*3);
					CopyMemory(&cbLineCardData[1],LineCardResult.cbResultCard[i], sizeof(BYTE)*LineCardResult.cbCardCount[i]);
				}
				//十张 放中后墩
				else
				{
					CopyMemory(&cbLineCardData[1],cbTmpCardData,sizeof(BYTE)*5);
					CopyMemory(&cbLineCardData[2],LineCardResult.cbResultCard[i], sizeof(BYTE)*LineCardResult.cbCardCount[i]);
				}
				if(bSpecCardData!=NULL)
				{
					CopyMemory(bSpecCardData,cbLineCardData,sizeof(cbLineCardData));
...
{
	if(bSpecialType<=CT_EX_INVALID) return 0;
	switch(bSpecialType)
	{
	...
	case CT_EX_YITIAOLONG:			return 13;
	case CT_EX_ZHIZUNQINGLONG:		return 13;
	}
	return 0;
