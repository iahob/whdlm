...
BYTE CGameLogic::AutoPutCard(BYTE bCardData[], BYTE bPutCardData[], bool bIsMobileUser,BYTE bPutMobileCard[4][HAND_CARD_COUNT])
{
	...
	for(bSubscript[2] = bSubscript[1] + 1; bSubscript[2] < 13; bSubscript[2] ++)
	{
		...
		do{
			...
			for(int i = 0 ; i < 5 ; i ++) bCard[1][4 - i] = bCardList[bSubscript[3 + i]];

			iFWeights = CardTypeToWeights(GetCardType(bCard[2],3,bWCard), 1);
			iSWeights = CardTypeToWeights(GetCardType(bCard[1],5,bWCard), 2);

			//比较第一注和第二注牌型权值的大小
			if(iFWeights > iSWeights) break;
			//将已经使用的牌进行标记
			for(int i = 0 ; i < 5 ; i ++) blBiaoJi[bSubscript[3 + i]] = true;

			//将剩下元素放入bCard[0]数组当中
			int iCount = 0;
			for(int i = 0; i < 13; i ++)
			{
				if(!blBiaoJi[i])bCard[0][iCount ++ ] = bCardList[i];
			}	
	
			iTWeights = CardTypeToWeights(GetCardType(bCard[0],5,bWCard), 3);
			if(iSWeights > iTWeights)	break;
	...
	if(bIsMobileUser && bPutMobileCard!=NULL)
	{
		...
		for(BYTE i=0;i<cbMobileCount;i++)
		{
			...
			//牌组优化
			OptimizationCombo(&bPutMobileCard[i][3],&bPutMobileCard[i][8],5,5);
			OptimizationCombo(bPutMobileCard[i],&bPutMobileCard[i][8],3,5);
			OptimizationCombo(bPutMobileCard[i],&bPutMobileCard[i][3],3,5);
	...
	if(true==bComperWithOther)
	{
		...
		if(bNextType==bFirstType) 
		{
			...
			case CT_FIVE_STRAIGHT_FLUSH_FIRST_A://A在前同花顺
				{
					return GetCardColor(bNextList[1])>GetCardColor(bFirstList[1])?enCRGreater:enCRLess;
				}
