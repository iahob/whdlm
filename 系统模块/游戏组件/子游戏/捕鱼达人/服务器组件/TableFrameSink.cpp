...
bool CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	_sntprintf_s(szConfigPath, MAX_PATH, TEXT("%s\\FishDRConfig\\%s.fdx"), szDirectoryPath, m_pGameServiceOption->szServerName);
	FILE * pFile = NULL;
	...
	_snprintf_s(g_szLogoFileName, MAX_PATH, "%s\\FishDRConfig\\%s_日志.log", szDirectoryPathA.m_psz, szServerNameA.m_psz);
	_snprintf_s(g_szStockFileName, MAX_PATH, "%s\\FishDRConfig\\%s_库存.log", szDirectoryPathA.m_psz, szServerNameA.m_psz);
	...
	if( m_ArrayFishPathPositive.GetCount() == 0 )
	{
		...
		_sntprintf_s(szFishMermaidPath, MAX_PATH, TEXT("%s\\FishDRConfig\\FishDRPath.pat"), szDirectoryPath);
		...
		if ( bOpen == FALSE )
		{	
			AfxMessageBox(TEXT("文件 FishDRConfig\\FishDRPath.pat 读取失败！"));
			return false;
		}
...
bool CTableFrameSink::OnEventGameConclude(WORD wChairID, IServerUserItem * pIServerUserItem, byte cbReason)
{
	...
	if ( cbReason == GER_DISMISS )
	{
		...
		// 全局爆炸还原
		if ( GetFishCount(FishType_ZhaDanYu) > 0 )
		{
			...
			while ( nFishIndex < m_ArrayFishInfo.GetCount() )
			{
				...
				if( FishMove(PointFish, TFishInfo) == FishMoveType_Delete )
				{
					...
					else
					{
						...
						// 盒子爆炸
						if( TFishInfo.nFishType == FishType_BaoChuan && TFishInfo.wHitChair != INVALID_CHAIR )
						{
							// 活动重载
						...
						// 全局爆炸还原
						if ( TFishInfo.nFishType == FishType_ZhaDanYu )
						{
							// 飞镖消耗
							m_dDartStock += m_dTableWaitDartStock;
							m_dWaitDartStock -= m_dTableWaitDartStock;
			...
			if ( !m_bBackExchange )
			{
				...
				else if ( !m_bBackExchange && ( GetTickCount() > m_nFishTeamTime || m_ArrayFishInfo.GetCount() == 0 ) )
				{
					...
					if (m_dDartStock >= (double)((double)nMultipleMax * (double)m_nExplosionStartCount)
						&& GetFishCount(FishType_ZhaDanYu) == 0 
					  )
					{
						//税收条件要大于全局爆炸预扣条件
						...
						if (m_dDartStock>dCreatBZContion)
						{
							...
							// 创建飞镖
							CreateFish( 1, FishType_ZhaDanYu );
...
void CTableFrameSink::DeleteFish(int nFishIndex, int nMultipleIndex, IServerUserItem *pIServerUserItem)
{
	...
	if (nFishIndex >= 0 && nFishIndex < m_ArrayFishInfo.GetCount())
	{
		...
		if (nMultipleIndex != Multiple_Max && !bAndroidUser)
		{
			...
			// 盒子爆炸
			if (TFishInfo.nFishType == FishType_BaoChuan)
			{
				nFishScore = 0;
			}
	...
	for ( int nIndex = 0; nIndex < FishCatch_Max; ++nIndex )
	{
		...
		if ( pCatchFish->nFishKey[nIndex] != 0 )
		{
			...
			if ( bRealKill )
			{
				...
				// 判断鱼信息
				if( nFishState == FishState_Killer || nFishType == FishType_ZhaDanYu)
				{
					// 大倍发送信息
...
bool CTableFrameSink::OnSubFire(const void * pBuffer, word wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	if ( !m_bPlaySupply[wChairID] && m_nSupplyValue[wChairID] != 0 && m_nEnergyValue[wChairID] >= m_nSupplyValue[wChairID] && !m_bBackExchange )
	{
		...
		// 创建补给箱
		CreateFish( 1, FishType_BaoChuan, nPathIndex, 0, 0, 0.f, false, false, KillerType_No, wChairID, CShortPoint(0, 0), 0.f, false );
								   ...
								   bool bFlockKill)
{
	...
	if ( cbFishType == FishType_Max )
	{
		...
		int nHeiDongYu = 4;
		if( GetFishCount(FishType_BaoZhaYu) >= 1 ) nBaoZhaYu = 0;
		if( GetFishCount(FishType_BingDongYu) >= 1 ) nBingDongYu = 0;
		if( GetFishCount(FishType_EYu) >= 1 ) nEYu = 0;
		if( GetFishCount(FishType_JinChan) >= 1 ) nJinChan = 0;
		if( GetFishCount(FishType_JinSeNianYu) >= 1 ) nJinSeNianYu = 0;
		if( GetFishCount(FishType_MoFaYu) >= 1 ) nMoFaYu = 0;
		if( GetFishCount(FishType_JinSeChuiTouSha) + GetFishCount(FishType_HeiDongYu) >= 2 )
		{
			nJinSeChuiTouSha = 0;
			nHeiDongYu = 0;
		}
		...
		// 大鱼限制
		if( GetFishCount(FishType_EYu) + GetFishCount(FishType_JinChan) + GetFishCount(FishType_JinSeNianYu) + GetFishCount(FishType_MoFaYu) >= (uint)m_nMaxBigFishCount )
		{
			nEYu = 0;
			nJinChan = 0;
			nJinSeNianYu = 0;
	...
	//  宝船保存当前倍数
	if ( cbFishType == FishType_BaoChuan && wHitChair != INVALID_CHAIR )
	{
...
// 		}
		...
		// 停留鱼鱼
		bool bAquatic = false;	
		if( bCanAquatic && nFishScore < 20 && RandomArea(2, 1, 30) == 0 && cbFishType != FishType_BaoChuan )
		{
			bAquatic = true;
		}
...
uint CTableFrameSink::GroupOfFish( uint nBeginTime /* = 0 */ )
{
	...
	case 0:
		{
			...
			for (int nFishIndex = FishType_MoGuiYu; nFishIndex <= FishType_EYu; ++nFishIndex)
			{
			...
			{
				CreateFish(1, FishType_ShuiMu, 16, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(DEFAULE_WIDTH * nIndex / nFishCount, -40), 0.f, false);
				CreateFish(1, FishType_ShuiMu, 17, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(DEFAULE_WIDTH * nIndex / nFishCount, 40), 0.f, false);
			}

			nFishCount = 8;//16;
			for( int nIndex = 1; nIndex < nFishCount; ++nIndex )
			{
				CreateFish( 1, FishType_ShenXianYu, 16, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(DEFAULE_WIDTH * nIndex / nFishCount, 70), 0.f, false );
				CreateFish( 1, FishType_ShenXianYu, 17, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(DEFAULE_WIDTH * nIndex / nFishCount, -70), 0.f, false );
			}
		}
		return 52000;
	case 1:
		{
			// 创建大
			CreateFish( 1, FishType_MoFaYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(640, 400), 0.f, false );
			CreateFish( 1, FishType_HaiGui, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(440, 400), 0.f, false );
			CreateFish( 1, FishType_HaiGui, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(840, 400), GL_PI, false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(451, 125), 0.f, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(489, 125), 0.f, false );	
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(527, 125), 0.f, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(565, 125), 0.f, false );	
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(603, 125), 0.f, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(641, 125), 0.f, false );	
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(679, 125), 0.f, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(717, 125), 0.f, false );	
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(755, 125), 0.f, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(793, 125), 0.f, false );	
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(831, 125), 0.f, false );	
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(451, 672), GL_PI, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(489, 672), GL_PI, false );	
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(527, 672), GL_PI, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(565, 672), GL_PI, false );	
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(603, 672), GL_PI, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(641, 672), GL_PI, false );	
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(679, 672), GL_PI, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(717, 672), GL_PI, false );	
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(755, 672), GL_PI, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(793, 672), GL_PI, false );	
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(831, 672), GL_PI, false );	
			//CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(869, 672), GL_PI, false );	

			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(365, 129), AngleToRadian(-10), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(276, 162), AngleToRadian(-30), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(204, 223), AngleToRadian(-50), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(156, 305), AngleToRadian(-70), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(139, 399), AngleToRadian(-90), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(156, 492), AngleToRadian(-110), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(203, 575), AngleToRadian(-130), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(276, 636), AngleToRadian(-150), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(365, 668), AngleToRadian(-170), false );

			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(916, 129), AngleToRadian(10), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(1005, 162), AngleToRadian(30), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(1078, 223), AngleToRadian(50), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(1125, 305), AngleToRadian(70), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(1141, 399), AngleToRadian(90), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(1125, 492), AngleToRadian(110), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(1078, 575), AngleToRadian(130), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(1005, 636), AngleToRadian(150), false );
			...
			CreateFish( 1, FishType_FangYu, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, CShortPoint(916, 668), AngleToRadian(170), false );
	...
	case 2:
		{
			// 创建小鱼
			byte cbFishSmallIndex[3] = { FishType_XiaoHuangYu, FishType_WuZei, FishType_HeTun };
			...
			// 创建中鱼
			byte cbFishBigIndex = ( rand() % 6 ) + FishType_HeiDongYu;
	...
	case 3:
		{
			int nFishIndex = (rand() % 4) + FishType_MoFaYu;
			// 创建BOSS
			...
			{
				// 创建5圈鱼
				CreateFish( 1, FishType_FangYu, nPathIndex7, 0, 0, GL_PI * 2.f / nCountMax * nCount, false, false, KillerType_No, INVALID_CHAIR, PointOffSet7, 0.f, false );
				CreateFish(1, FishType_ShuiMu, nPathIndex6, 3000, 0, GL_PI * 2.f / nCountMax * nCount, false, false, KillerType_No, INVALID_CHAIR, PointOffSet6, 0.f, false);
				CreateFish( 1, FishType_ShenXianYu, nPathIndex7, 6000, 0, GL_PI * 2.f / nCountMax * nCount, false, false, KillerType_No, INVALID_CHAIR, PointOffSet7, 0.f, false );
				CreateFish(1, FishType_ShuiMu, nPathIndex6, 9000, 0, GL_PI * 2.f / nCountMax * nCount, false, false, KillerType_No, INVALID_CHAIR, PointOffSet6, 0.f, false);
				CreateFish( 1, FishType_ShenXianYu, nPathIndex7, 12000, 0, GL_PI * 2.f / nCountMax * nCount, false, false, KillerType_No, INVALID_CHAIR, PointOffSet7, 0.f, false );
			}
	...
	case 4:
		{
			// 创建BOSS
			byte cbFishBigIndex = ( rand() % 4 ) + FishType_MoFaYu;
			...
			for( int nIndex = 0; nIndex < nCountMax; ++nIndex )
			{
				...
				CreateFish( 1, FishType_WuZei, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, Rotate( CShortPoint(318, 400), fAngle, CShortPoint(318, 400 + 140) ), fAngle + GL_PI, false );
				CreateFish( 1, FishType_WuZei, 15, 0, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, Rotate( CShortPoint(957, 400), fAngle, CShortPoint(957, 400 + 140) ), fAngle + GL_PI, false );
			}
			...
			{
				float fAngle = GL_PI * 2.f / (float)nCountMax * (float)nIndex;
				CreateFish(1, FishType_ShuiMu, 15, 500, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, Rotate(CShortPoint(318, 400), fAngle, CShortPoint(318, 400 + 225)), fAngle + GL_PI, false);
				CreateFish(1, FishType_ShuiMu, 15, 500, 0, 0.f, false, false, KillerType_No, INVALID_CHAIR, Rotate(CShortPoint(957, 400), fAngle, CShortPoint(957, 400 + 225)), fAngle + GL_PI, false);
			}
...
bool CTableFrameSink::SendAwardTip( word wChairID, byte nFishType, int nFishMultiple, longlong lFishScore, EnumScoreType nScoreType )
{
	...
	if( m_bAwardChatBox && nFishMultiple >= 100 && lFishScore >= 0 )
	{
		...
		else if ( nFishType == FishType_BaoChuan )
		{
			StrAwardTip += _T("运气爆表！！！");
		}
		else
...
longlong CTableFrameSink::PlayCatchFish(word wChairID, int nBulletSplit, double dBulletInvest, uint nFishCatchKey, int nMultipleIndex, EnumPlayCatchType nPlayCatchType, int &nRealKillCount)
{
	...
	for( int nFishIndex = 0; nFishIndex < m_ArrayFishInfo.GetCount(); ++nFishIndex )
	{
		...
		// 特殊鱼过滤
		if ( TFishInfo.nFishState != FishState_Killer && TFishInfo.nFishType != FishType_BaoZhaYu && TFishInfo.nFishType != FishType_ZhaDanYu )
		{
			// 返回
			break;
		}
		...
		for( int nFishPeripheryIndex = 0; nFishPeripheryIndex < m_ArrayFishInfo.GetCount(); ++nFishPeripheryIndex )
		{
			...
			else if ( TFishInfo.nFishType == FishType_BaoZhaYu )
			{
				// 短距离
				bRelatedFish = DistanceOfPoint(PointFish, PointPeriphery) < 300.0;
			}
			else if( TFishInfo.nFishType == FishType_ZhaDanYu) //爆炸鱼
			{
				if(!IsAndroidUser(wChairID))
				{					
					CDoublePoint PointPeriphery;
...
// 			}
			...
			// 记录停留鱼
			if ( TFishInfo.nFishType == FishType_BingDongYu && !bStayFish )
			{
				// 记录信息
			...
			// 记录全屏爆炸
			if( TFishInfo.nFishType == FishType_ZhaDanYu && !bFishFullScreen )
			{
				// 记录信息
			...
			// 记录爆炸鱼
			if ( TFishInfo.nFishType == FishType_BaoZhaYu && !bFishExplosion )
			{
				// 记录信息
	...
	if( bFishExplosion )
	{
		...
		while( nDeathIndex < m_ArrayFishInfo.GetCount() )
		{
			...
			// 周边鱼距离
			CDoublePoint PointPeriphery;
			if( FishMove( PointPeriphery, TPeripheryInfo ) == FishMoveType_Normal && DistanceOfPoint(PointExplosion, PointPeriphery) < 300.0 && TPeripheryInfo.nFishType != FishType_ZhaDanYu )
			{
				// 捕获中鱼
		...
		case 6:
			{
				...
				// 大倍发送信息
				SendAwardTip(wChairID, FishType_BaoChuan, m_nGiftScore[ nBoxActivity - 2 ], lCatchScore, EST_Cold);
			}
			break;
		case 7:	// 空宝船
			{
...
void CTableFrameSink::InitExplosionAdmin()
{
	...
	while ( Pos )
	{
		...
		{
			ASSERT(GetFishCount(FishType_ZhaDanYu)==0);
