...
function GameLayer:ctor( frameEngine,scene )        
	...
	self.m_wAddTimesCount = 0--点击加倍的人数 
	
	self.m_bXuanZhanUserAddTimes = false --宣战用户点击了加倍
	
	self.m_bXuanZhan = false
	...
	self.m_cbSelectedCardData = 0

    self.m_priTotalEnd = false --总结算

    self.m_playerCardCount = {}

    for i = 1, cmd.PLAYER_COUNT do
        self.m_playerCardCount[i] = cmd.NORMAL_COUNT 
    end
end
...
function GameLayer:SetGameClock(chair,id,time)
	if chair == cmd.INVALID_CHAIR then
		return
	end
...
function GameLayer:onUserVoiceEnded( useritem, filepath )
    local viewid = self:SwitchViewChairID(useritem.wChairID)
    local voiceImg = self._gameView.m_tabVoiceBox[viewid]
    if nil ~= voiceImg then
        self._gameView.m_tabVoiceBox[viewid]:setVisible(false)
    else
        for i = 1, cmd.PLAYER_COUNT do
            self._gameView.m_tabVoiceBox[i]:setVisible(false)
        end
    end
end
...
function GameLayer:onSubGameStart(dataBuffer)
	...
	local cards = cmd_table.cbCardData[1]
	local result = GameLogic:Search510K(cards,#cards,0,0)
	cards = GameLogic:SortCardList(cards, cmd.NORMAL_COUNT, 0,result)
...
function GameLayer:onSubDeclareWar( dataBuffer )
    ...
    self:KillGameClock()
    self.m_wCurrentUser = cmd_table.wCurrentUser
	local viewId = self:SwitchViewChairID(cmd_table.wXuanZhanUser)
	self.m_bXuanZhan = cmd_table.bXuanZhan
    if false == cmd_table.bXuanZhan then
		...
		local tip_no_declare = self._gameView.m_csbNode:getChildByName(string.format("tip_no_declare_%d", viewId))
        tip_no_declare:setVisible(true)
    else
        self.m_wXuanZhanUser = cmd_table.wXuanZhanUser
        self.m_bMingDu = true
        self.m_cbGameStatus = cmd.GAME_ADD_TIMES
        self:KillGameClock()
        self:SetGameClock(cmd_table.wCurrentUser, cmd.TAG_COUNTDOWN_ADD_TIMES, cmd.COUNTDOWN_ADDTIME)
        if self._gameView.m_trusteeshipBg:isVisible() then

        end
		if cmd_table.wXuanZhanUser ~= self:GetMeChairID() then
			self._gameView:presentAddTimesBtns()
		end
        
		self._gameView:hideAllAskTip()
        self:SetFriendFlag()
        self._gameView:onGameDeclareWar(cmd_table.wXuanZhanUser,cmd_table.bXuanZhan)
		local tip_declare = self._gameView.m_csbNode:getChildByName(string.format("tip_declare_%d", viewId))
        tip_declare:setVisible(true)
    end
...
function GameLayer:onSubFindFriend( dataBuffer )
	...
	self.m_cbSelectedCardData = cmd_table.cbSelectedCardData
	self._gameView:hideAllDeclareTip()
	self._gameView:presentBankChoose(cmd_table.cbSelectedCardData)
    --LogAsset:getInstance():logData(jsonStr,true)
    ...
    if cmd_table.wCurrentUser == self:GetMeChairID() then
        self._gameView:presentAskFriendBtns(cmd_table.cbSelectedCardData)
    else
...
function GameLayer:onSubAskFriend( dataBuffer )
	...
	print("-------onSubAskFriend------len "..dataBuffer:getlen())
    self.m_wCurrentUser = cmd_table.wCurrentUser
    self.m_cbFriendFlag = cmd_table.cbFriendFlag
	local viewId = self:SwitchViewChairID(cmd_table.wAskUser)
    ...
    if true == cmd_table.bAsk then --问了显示同盟
        ...
        self.m_cbGameStatus = cmd.GAME_ADD_TIMES
        self:KillGameClock()
		if self.m_wCurrentUser ~= cmd.INVALID_CHAIR and cmd_table.wAskUser ~= self:GetMeChairID() then
			self:SetGameClock(self.m_wCurrentUser, cmd.TAG_COUNTDOWN_ADD_TIMES, cmd.COUNTDOWN_ADDTIME)
		end
        if self.m_wCurrentUser == self:GetMeChairID() and not cmd_table.bStart then
            if self._gameView.m_trusteeshipBg:isVisible() then
		...
		local tip_ask = self._gameView.m_csbNode:getChildByName(string.format("tip_ask_%d", viewId))
		--判断是否显示答应
		local wFriend = cmd_table.wFriend[1]
		local wBankFriend = wFriend[self.m_wBankerUser + 1]
		if cmd_table.wAskUser == self:GetMeChairID() then
			local bFind = self._gameView:findSelectedCard(self.m_cbSelectedCardData)
			if bFind then
				tip_ask:loadTexture("game_res/tip_agree.png")
			else
				tip_ask:loadTexture("game_res/tip_ask.png")
			end
		else
			if wBankFriend == cmd_table.wAskUser then
				tip_ask:loadTexture("game_res/tip_agree.png")
			else
				tip_ask:loadTexture("game_res/tip_ask.png")
			end
        ...
        tip_ask:setVisible(true)
    else
        self.m_wNoAskFriendCount = self.m_wNoAskFriendCount + 1
        if self.m_wNoAskFriendCount >= 4 then
            self._gameView:onGameAskFriend(cmd_table)
        end
		local tip_no_ask = self._gameView.m_csbNode:getChildByName(string.format("tip_no_ask_%d", viewId))
        tip_no_ask:setVisible(true)
        if self.m_wCurrentUser == self.m_wBankerUser then
            ...
            if self.m_wCurrentUser == self:GetMeChairID() then
                ...
                self._gameView:presentAskFriendBtns(self.m_cbSelectedCardData)
                self.m_bWaitAskFriend = false
            else
                self.m_bWaitAskFriend = false
            end
	...
	if cmd_table.bStart then
		if not GlobalUserItem.bPrivateRoom then
            self._gameView:setTrusteeBtnEnable(true)
            self._gameView:changeBtnState(self._gameView.m_bt_trusteeship,true)
            if self.m_wCurrentUser == self:GetMeChairID() then
                print("------------- ask friend start ----------")
                local handCards = self._gameView.m_tabNodeCards[1]:getHandCards()
                self:updatePromptList({}, handCards, cmd.MY_VIEWID, cmd.MY_VIEWID)
                self._gameView:onGetOutCard(1, 1, {})
                self._gameView:onChangePassBtnState(false)
...
function GameLayer:onSubAddTimes( dataBuffer )
	...
	if cmd_table.wAddTimesUser ~= cmd.INVALID_CHAIRID  then
		self._gameView:onGameAddTimes(cmd_table)
		self.m_wAddTimesCount = self.m_wAddTimesCount + 1
		if self.m_wXuanZhanUser == cmd_table.wAddTimesUser and self.m_bXuanZhan then
			self.m_bXuanZhanUserAddTimes = true
		end
		--[[
		if self.m_wAddTimesCount >= 3 and not self.m_bXuanZhanUserAddTimes then
			self:KillGameClock()
			self:SetGameClock(self.m_wXuanZhanUser, cmd.TAG_COUNTDOWN_ADD_TIMES, cmd.COUNTDOWN_ADDTIME)
		end
		--]]
		if cmd_table.wAddTimesUser == self:GetMeChairID() then
			self:KillGameClock()
		end
...
function GameLayer:onSubOutCard(dataBuffer)
    ...
    local currentCount = self.m_playerCardCount[outView] - outCount
    self.m_playerCardCount[outView] = currentCount
    ...
    if outView ~= cmd.MY_VIEWID then
        ...
        if currentCount <= 2 and currentCount > 0 then
...
function GameLayer:onSubPassCard(dataBuffer)
    ...
    if self:IsValidViewID(curView) and self:IsValidViewID(passView) then
        ...
        if curView == cmd.MY_VIEWID then -- and 1 ~= cmd_table.cbTurnOver 
            self._gameView:onGetOutCard(curView, self.m_nLastOutViewId, self.m_tabLastCards, false)
        else
            self._gameView:onGetOutCard(curView, self.m_nLastOutViewId, {}, false)
        end
...
function GameLayer:onSubGameConclude(dataBuffer)
	...
	self.m_wAddTimesCount = 0
	self.m_bXuanZhan = false
	self.m_bXuanZhanUserAddTimes = false
...
function GameLayer:onEventGameSceneFree( dataBuffer )
    ...
    -- 刷新局数
    if PriRoom and GlobalUserItem.bPrivateRoom then
        if 1 == PriRoom:getInstance().m_tabPriData.cbIsGoldOrGameScore then
            self._gameView:updateCollectScore()
        end
        local curcount = PriRoom:getInstance().m_tabPriData.dwPlayCount
	...
	--[[
    local cards =  {0x0A,0x23,0x1b,0x35,0x15,0x26,0x16,0x3b,0x17,0x27,0x37,0x25,0x2A,0x2D}
    --local cards =  {0x76,0x2D,0x2C,0x1C,0x0C,0x06,0x04,0x33,0x32,0x56}
    self._gameView.m_tabNodeCards[1]:updateCardsNode(cards, true, false)
    local handCards = self._gameView.m_tabNodeCards[1]:getHandCards()
    self:updatePromptList({}, handCards, 2, cmd.MY_VIEWID)
    self._gameView:onGetOutCard(1, 1, {})
    self._gameView:onChangePassBtnState(false)
    self._gameView:presentOutCardsBtns()
    --]]
...
function GameLayer:onEventGameScenePlay( dataBuffer )
    ...
    self.m_lCellScore = cmd_table.lCellScore
    self.m_cbFriendFlag = cmd_table.cbFriendFlag
	if self.m_gameScenePlay.cbGameStatus > cmd.GAME_FIND_FRIEND  then
        self._gameView:presentBankChoose(cmd_table.cbSelectedCardData)
    end
    ...
    for i = 1, 4 do
        ...
        if cmd.MY_VIEWID == viewId then
			...
			local result = GameLogic:Search510K(cards,#cards,0,0)
			cards = GameLogic:SortCardList(cards, count, 0,result)
    ...
    elseif self.m_gameScenePlay.cbGameStatus == cmd.GAME_FIND_FRIEND then
        if  self.m_wCurrentUser == myChairId then
            self._gameView:openFindFriendLayer()
        end
    ...
    self._gameView.m_lGameScore = cmd_table.cbGameScore[1]
     ...
     for i = 1 ,cmd.PLAYER_COUNT do
        ...
        text_curScore:setString(string.format("当前得分:%d", self._gameView.m_lGameScore[i]))

        if viewId ~= cmd.MY_VIEWID then
            self.m_playerCardCount[viewId] = cmd_table.cbHandCardCount[1][i]
...
function GameLayer:SwitchViewChairID(chair)
    ...
    if nChairID ==  cmd.INVALID_CHAIRID then
        return nChairID
    end
