...
protected:
	...
	//上局信息
public:
	LONGLONG						m_lLastAllJettonPlace[AREA_MAX +1][GAME_PLAYER]; //上局总下注
	LONGLONG						m_lLastAllJettonArea[AREA_MAX +1][GAME_PLAYER];  //本局最后一次操作
...
protected:
	...
	//调试操作
protected:
	BYTE							m_cbDebugArea;						//调试区域
	BYTE							m_cbDebugTimes;						//调试次数
	bool							m_bDebug;								//是否调试
	LONGLONG						m_lDebugStorage;						//调试后的损耗写入
...
protected:
	...
	INT								m_nMultiple[AREA_MAX];					//区域倍数
	INT								m_nAnimalPercent[AREA_MAX];				//开中比例
	INT								m_nAnimalTimes[AREA_MAX];				//出现次数
...
protected:
	...
	LONGLONG						m_lRobotAreaScore[AREA_MAX+1];		//区域下注
