...
// 构造函数
CDialogDexter::CDialogDexter(UINT nIDTemplate, int nShowMax, CDialogControl *pParentWnd) : CDialog(nIDTemplate, pParentWnd)
{
...
// 控制信息
bool CDialogDexter::SendControlMessage(uint nMessageID, void *pData, uint nSize)
{
    m_pParentWnd->SendControlMessage(nMessageID, pData, nSize);
    return true;
}
...
BEGIN_MESSAGE_MAP(CDialogControl, CDialog)
    ...
    ON_BN_CLICKED(IDC_BUTTON_DEFAULT, &CDialogControl::OnBnClickedButtonDefault)
    ON_BN_CLICKED(IDC_BUTTON_REFRESH_RULE, &CDialogControl::OnBnClickedButtonRefreshRule)
    ON_BN_CLICKED(IDC_BUTTON_MODIFY_RULE, &CDialogControl::OnBnClickedButtonModifyRule)
END_MESSAGE_MAP()
...
bool CDialogControl::Create(CWnd *pParentWnd, IClientControlCallback *pIClientControlCallback)
{
    ...
    SendControlMessage(SUB_C_CONTROL_REFRESH_RULE, &bAutoRefresh, sizeof(bAutoRefresh));
...
// 消息函数
bool CDialogControl::OnControlMessage(WORD nMessageID, WORD wTableID, void *pData, WORD nSize)
{
    // 无效接口
    ...
    switch(nMessageID)
    {
    case  SUB_S_CONTROL_REFRESH_RULE:
    {
        //校验大小
        ...
        if(pCustomRule->cbUpdateType == CONTROL_REFRESH_RULE)
        {
            MessageBox(TEXT("游戏配置刷新成功！"), TEXT("温馨提示"));
        }
        else if(pCustomRule->cbUpdateType == CONTROL_MODIFY_RULE)
        {
    ...
    for(int nIndex = 0; nIndex < MAX_CUSTOM; ++nIndex)
    {
        if(m_DialogCustom[nIndex]->OnControlMessage(nMessageID, wTableID, pData, nSize))
        {
            return true;
        }
    }
...
// 控制信息
bool CDialogControl::SendControlMessage(uint nMessageID, void *pData, uint nSize)
{
    if(m_pIClientControlCallback != NULL)
    {
        // 发送消息
        m_pIClientControlCallback->OnControlInfo(nMessageID, 0, pData, nSize);
    }
    return true;
}

// 变换选项
void CDialogControl::OnTcnSelchangeTabOptisons(NMHDR *pNMHDR, LRESULT *pResult)
{
    // 获取分页
    CTabCtrl *pTabCtrl = (CTabCtrl *)GetDlgItem(IDC_TAB_CUSTOM);
...
void CDialogControl::OnBnClickedButtonDefault()
{
...
void CDialogControl::OnBnClickedButtonRefreshRule()
{
    // 刷新配置
    ...
    //获取礼物
    SendControlMessage(SUB_S_REFRESH_GIFT);
}


void CDialogControl::OnBnClickedButtonModifyRule()
{
    // 判断窗口
    for(int nIndex = 0; nIndex < MAX_CUSTOM; ++nIndex)
    {
    ...
    // 修改配置
    SendControlMessage(SUB_C_CONTROL_MODIFY_RULE, &g_CustomRule, sizeof(g_CustomRule));
...
// 构造函数
CDialogSpecial::CDialogSpecial(int nShowMax/* = 0*/, CDialogControl *pParent /*=NULL*/) : CDialogDexter(IDD_CUSTOM_SPECIAL, nShowMax, pParent)
{
    // 设置变量
}
...
// 消息函数
bool CDialogSpecial::OnControlMessage(WORD nMessageID, WORD wTableID, void *pData, WORD nSize)
{
    // 判断消息
    switch(nMessageID)
    {
    ...
    case SUB_S_QUERY_CONTROL_FAIL:			//修改失败
    {
        ASSERT(sizeof(CMD_S_QueryControlFail) == nSize);
        if(sizeof(CMD_S_QueryControlFail) != nSize)
        {
            return false;
        }

        // 定义信息
        CMD_S_QueryControlFail *pQueryControlFail = (CMD_S_QueryControlFail *)pData;

        // 实物信息
        TCHAR szOperateName[4][64] = { TEXT("修改礼物"), TEXT("修改金币"), TEXT("查询修改"), TEXT("修改碎片") };
        CString strValue;
        strValue.Format(TEXT("玩家GAMEID：%d，%s失败！"), pQueryControlFail->dwGameID, szOperateName[pQueryControlFail->cbQueryType]);
        MessageBox(strValue, TEXT("温馨提示"));

        return true;
    }
    case SUB_S_QUERY_CONTROL_SUCCESS:		//修改成功
    {
        ASSERT(sizeof(CMD_S_QueryControlSuccess) == nSize);
        if(sizeof(CMD_S_QueryControlSuccess) != nSize)
        {
            return false;
        }

        // 定义信息
        CMD_S_QueryControlSuccess *pQueryControlSuccess = (CMD_S_QueryControlSuccess *)pData;

        CString strValue;
        strValue.Format(TEXT("玩家GAMEID：%d\n待中奖金币：%I64d\n待中碎片：%I64d\n待中奖礼物1：%s\n待中奖礼物2：%s\n待中奖礼物3：%s"), pQueryControlSuccess->dwGameID, pQueryControlSuccess->lGlodTotalScore, pQueryControlSuccess->lGiftTotalScore, g_szKindRewardName[pQueryControlSuccess->cbGiftType[0]], g_szKindRewardName[pQueryControlSuccess->cbGiftType[1]], g_szKindRewardName[pQueryControlSuccess->cbGiftType[2]]);
        TCHAR szOperateName[4][64] = { TEXT("修改礼物成功"), TEXT("修改金币成功"), TEXT("查询修改成功"), TEXT("修改碎片成功") };
        MessageBox(strValue, szOperateName[pQueryControlSuccess->cbQueryType]);
...
{
    // 发送消息
    SendControlMessage(SUB_C_REFRESH_STOCK);
}
...
void CDialogSpecial::OnBnClickedBtnModifyStock()
{
    ...
    // 发送消息
    SendControlMessage(SUB_C_MODIFY_STOCK, &ModifyStock, sizeof(ModifyStock));
}
...
{
    CString strValue;
    CMD_C_ControlGlod ControlGlod;
    GetDlgItemText(IDC_EDIT_GAME_ID, strValue);
    ControlGlod.dwGameID = _ttoi(strValue);
    if(ControlGlod.dwGameID <= 0)
    {
    ...
    ControlGlod.lGoldScore = _ttoi64(strValue);
    if(ControlGlod.lGoldScore < 0)
    {
    ...
    // 发送消息
    SendControlMessage(SUB_C_CONTROL_GLOD, &ControlGlod, sizeof(ControlGlod));
}
...
{
    CString strValue;
    CMD_C_ControlGift ControlGift;
    GetDlgItemText(IDC_EDIT_GAME_ID, strValue);
    ControlGift.dwGameID = _ttoi(strValue);
    if(ControlGift.dwGameID <= 0)
    {
        MessageBox(TEXT("玩家ID错误，请重新填写！"), TEXT("温馨提示"));
        return;
    }
    ...
    ControlGift.cbGiftType = GetKindRewardIndex(strText);
    if(ControlGift.cbGiftType <= 0 && ControlGift.cbGiftType >= KindReward_Max)
    {
        MessageBox(TEXT("请选择正确的礼物，请重新填写！"), TEXT("温馨提示"));
        return;
    }
    ...
    // 发送消息
    SendControlMessage(SUB_C_CONTROL_GIFT, &ControlGift, sizeof(ControlGift));
}
...
{
    CString strValue;
    CMD_C_QueryControl QueryControl;
    GetDlgItemText(IDC_EDIT_GAME_ID, strValue);
    QueryControl.dwGameID = _ttoi(strValue);
    if(QueryControl.dwGameID <= 0)
    {
        MessageBox(TEXT("玩家ID错误，请重新填写！"), TEXT("温馨提示"));
        return;
    }
    ...
    // 发送消息
    SendControlMessage(SUB_C_QUERY_CONTROL, &QueryControl, sizeof(QueryControl));
}
...
void CDialogSpecial::OnBnClickedBtnShard()
{
    ...
    // 发送消息
    SendControlMessage(SUB_C_CONTROL_SHARD, &ControlGlod, sizeof(ControlGlod));
}
...
void CDialogSpecial::OnBnClickedBtnGiftOpenClose()
{
    ...
    if(pButton != NULL)
    {
        ...
        // 发送消息
        SendControlMessage(SUB_C_CONTROL_GIFT_FEATURES, &nGiftFeatures, sizeof(nGiftFeatures));
    }
}
...
// 构造函数
CDialogGeneral::CDialogGeneral(int nShowMax/* = 0*/, CDialogControl *pParent /*=NULL*/) : CDialogDexter(IDD_CUSTOM_SPECIAL, nShowMax, pParent)
{
}
...
// 消息函数
bool CDialogGeneral::OnControlMessage(WORD nMessageID, WORD wTableID, void *pData, WORD nSize)
{
    // 判断消息
    switch(nMessageID)
    {
...
// 构造函数
CDialogGift::CDialogGift(int nShowMax/* = 0*/, CDialogControl *pParent /*=NULL*/) : CDialogDexter(IDD_CUSTOM_GIFT, nShowMax, pParent)
{
}
...
// 消息函数
bool CDialogGift::OnControlMessage(WORD nMessageID, WORD wTableID, void *pData, WORD nSize)
{
    return false;
}
...
// 构造函数
CDialogDelivery::CDialogDelivery(int nShowMax/* = 0*/, CDialogControl *pParent /*=NULL*/) : CDialogDexter(IDD_CUSTOM_GIFT, nShowMax, pParent)
{
}
...
// 消息函数
bool CDialogDelivery::OnControlMessage(WORD nMessageID, WORD wTableID, void *pData, WORD nSize)
{
    return false;
}
