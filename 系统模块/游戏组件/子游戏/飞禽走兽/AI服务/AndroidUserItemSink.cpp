		...
		case IDI_CHECK_BANKER:		//检查上庄
			{
				...
				if (m_wCurrentBanker == INVALID_CHAIR)
				{
					...
					//MyDebug(_T("AI上庄(End) %d [%d %d] [%d %d]"), m_pIAndroidUserItem->GetChairID(), m_nWaitBanker, 
		...
		default:
			{
				if (nTimerID >= IDI_PLACE_JETTON && nTimerID <= IDI_PLACE_JETTON+MAX_CHIP_TIME)
				{
					...
					/*ASSERT( MyDebug(_T("AI下注 %d 下注次数 [%d/%d] 下注 [%d %d] 范围 [%d %d] 限制 [%I64d %I64d %I64d]"), wMyID, nTimerID-IDI_PLACE_JETTON, m_nChipTime, 
...
bool  CAndroidUserItemSink::OnEventGameMessage(WORD wSubCmdID, VOID * pBuffer, WORD wDataSize)
{
	...
	case SUB_S_CLEAN_JETTON:			//清楚下注 (忽略)
		{
			return true;
		}
	...
	case GAME_STATUS_FREE:			//空闲状态
		{
			...
			//MyDebug(_T("AI上庄(Free) %d [%d %d] [%d %d]"), m_pIAndroidUserItem->GetChairID(), m_nWaitBanker, 
			//	m_nRobotWaitBanker, m_stlApplyBanker, m_nRobotApplyBanker);
...
{	
	...
	//ASSERT( MyDebug(_T("AI %d 下注次数 %d 范围 [%d %d] 总人数 %d 限制 [%I64d %I64d] 上庄 [%d %d]"), wMyID, m_nChipTime, m_nChipLimit[0], m_nChipLimit[1], 
...
void CAndroidUserItemSink::ReadConfigInformation(tagCustomAndroid *pCustomAndroid)
{
	...
	//MyDebug(_T("AI 读取配置 [%I64d %I64d %d %d] %d [%d %d %d] 下注 %d 下降 %d"), m_lRobotJettonLimit[0], m_lRobotJettonLimit[1],
