...
void CMusic::AppendMusic(  const tchar * pszFilePath  )
{
	...
	_sntprintf(szFile, CountArray(szFile), TEXT("%s\\Line\\Sound\\%s"), szExePath, pszFilePath);
...
bool CSound::PlaySound( const tchar * pszFileName )
{
	...
	// 生成路径
	tchar szFile[MAX_PATH] = {0};
	_sntprintf(szFile, CountArray(szFile), TEXT("%s\\Line\\Sound\\%s"), szExePath, pszFileName);
