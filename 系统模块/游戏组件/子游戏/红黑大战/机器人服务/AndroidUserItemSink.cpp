	...
	default:
		{
			if (nTimerID >= IDI_PLACE_JETTON && nTimerID <= IDI_PLACE_JETTON+MAX_CHIP_TIME)
			{
				...
				for (int i = 0; i <AREA_COUNT; i++) nACTotal += m_RobotInfo.nAreaChance[i];
				...
				PlaceJetton.wUserChairID = m_pIAndroidUserItem->GetChairID();
				//构造变量
	...
	case GAME_SCENE_FREE:			//空闲状态
		{
			...
			CopyMemory(m_lAreaLimitScore, pStatusFree->lAreaLimitScore, sizeof(m_lAreaLimitScore));
	...
	case GAME_SCENE_GAME_END:	//结束状态
		{
			...
			CopyMemory(m_lAreaLimitScore, pStatusPlay->lAreaLimitScore, sizeof(m_lAreaLimitScore));
...
{	
	...
	for (int i = 0; i < AREA_COUNT; i++)
	{
		m_lAreaChip[i] = i != ID_POKER_TYPE_AR ? m_lAreaLimitScore[0] : m_lAreaLimitScore[1];
	}

	//系统当庄
	if (pGameStart->wBankerUser == INVALID_CHAIR)
	{
		m_lMaxChipBanker = 1000000000;
	}
	else
		m_lBankerScore = __min( m_lMaxChipUser, m_lMaxChipBanker );
...
bool CAndroidUserItemSink::OnSubPlaceJetton(const VOID * pBuffer, WORD wDataSize)
{
	...
	//红黑区域是1:1  牌型区域最高倍数是10倍
	BYTE cbMultiple[] = { 1,1,10 };
	//庄家可用金币 下注先冻结它的赔付金币
	m_lMaxChipBanker -= (pPlaceJetton->lJettonScore * cbMultiple[pPlaceJetton->cbJettonArea-1]);
	//是自己下注的话
