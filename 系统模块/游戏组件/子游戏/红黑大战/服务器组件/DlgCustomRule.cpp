...
bool CDlgCustomRule::SaveCustomRule(LPBYTE pcbCustomRule, WORD wCustomSize)
{
	...
	if(!CheckDataMinMax(m_DlgCustomGeneral.m_CustomGeneral.lAreaLimitScore[0])) return false;
	if (!CheckDataMinMax(m_DlgCustomGeneral.m_CustomGeneral.lAreaLimitScore[1])) return false;
...
VOID CDlgCustomGeneral::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX,IDC_EDIT_GENERAL_9,m_CustomGeneral.lAreaLimitScore[0]);
	...
	DDX_Text(pDX,IDC_EDIT_GENERAL_20, m_CustomGeneral.lAreaLimitScore[1]);
