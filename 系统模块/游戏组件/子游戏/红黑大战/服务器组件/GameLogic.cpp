...
BYTE CGameLogic::GetCardType(BYTE cbCardData[], BYTE cbCardCount)
{
	...
	if (cbCardCount==MAX_COUNT)
	{
		...
		if (bDouble == true)
		{
			if (bPanther) return CT_BAO_ZI;
			return GetCardLogicValue(cbCardData[1]) > 8 ? CT_DOUBLE_EX : CT_DOUBLE;
		}
...
//对比扑克
BYTE CGameLogic::CompareCard(BYTE cbFirstData[], BYTE cbNextData[], BYTE cbCardCount, BYTE cbPokerType[])
{
	//设置变量
	...
	cbPokerType[1] = cbNextType;
	cbPokerType[0] = cbFirstType;
...
BYTE CGameLogic::GetTimes(BYTE cbPokerType)
{
	switch (cbPokerType)
	{
	case CT_BAO_ZI:			//豹子
