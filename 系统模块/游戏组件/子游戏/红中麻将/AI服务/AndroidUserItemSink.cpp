...
bool CAndroidUserItemSink::OnEventSceneMessage(BYTE cbGameStatus, bool bLookonOther, void * pData, WORD wDataSize)
{
	...
	case GAME_SCENE_FREE:	//空闲状态
	{
		...
		int nRandTime = 2 + (rand() % __max(1, m_cbTimeStartGame / 2));
#ifdef _DEBUG
		nRandTime = 2;
#endif // _DEBUG
	...
	case GAME_SCENE_PLAY:	//游戏状态
	{
		...
		//设置变量
		m_cbMagicIndex = pStatusPlay->cbMagicIndex;
		...
		CopyMemory(m_bTing, pStatusPlay->bTing, sizeof(m_bTing));
		...
		if (((pStatusPlay->cbActionMask != WIK_NULL) || m_wCurrentUser == wMeChairID))
		{
			...
			//设置时间
			int nRandTime = 2 + (rand() % __max(1, m_cbTimeOperateCard / 2));
#ifdef _DEBUG
			nRandTime = 2;
#endif // _DEBUG
...
bool CAndroidUserItemSink::OnSubGameStart(VOID * pData, WORD wDataSize)
{
	...
	m_cbMagicIndex = pGameStart->cbMagicIndex;
	...
	{
		int nRandTime = 2 + (rand() % __max(1, m_cbTimeOutCard / 2));
#ifdef _DEBUG
		nRandTime = 2;
#endif // _DEBUG
		m_pIAndroidUserItem->SetGameTimer(IDI_OPERATE_CARD, TIME_DISPATCH_CARD + nRandTime);
	}
...
bool CAndroidUserItemSink::OnSubQsListen(const void * pBuffer, WORD wDataSize)
{
...
bool CAndroidUserItemSink::OnSubGameConclude(VOID * pData, WORD wDataSize)
{
	...
	for (int i = 0; i < 4; i++)
	{
		if (pGameConclude->cbMaCount[i] > cbMaCount)
		{
			cbMaCount = pGameConclude->cbMaCount[i];
		}
	}
	m_pIAndroidUserItem->SetGameTimer(IDI_START_GAME, nRandTime/* + cbMaCount * 1 + 6*/);
...
bool CAndroidUserItemSink::SearchOutCard(tagOutCardResult &OutCardResult)
{
	...
	if (m_cbActionMask & WIK_GANG)
	{
		if (m_wCurrentUser == wMeChairId)
		{
			...
			m_GameLogic.AnalyseGangCard(m_cbCardIndex, m_WeaveItemArray[wMeChairId], m_cbWeaveItemCount[wMeChairId], GangCardResult);
