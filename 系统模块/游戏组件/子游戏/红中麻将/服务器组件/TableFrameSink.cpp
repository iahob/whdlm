...
{
	CString cc;
	cc.Format(_T("游戏开始servertype=%d,outtime=%d,operatetime=%d"),m_pGameServiceOption->wServerType, m_pGameCustomRule->cbTimeOutCard, m_pGameCustomRule->cbTimeOperateCard);
	WriteDebugInfo(cc);
	//设置状态
	...
	CopyMemory(GameStart.cbPlayStatus, m_bPlayStatus, sizeof(m_bPlayStatus));
	...
	case GER_NORMAL:		//常规结束
		{
			...
			for (WORD i = 0; i < m_cbPlayerCount; i++)
			{
				...
				//收税
				if (GameConclude.lGameScore[i]>0 && ((m_pGameServiceOption->wServerType&GAME_GENRE_PERSONAL)==0 || m_pITableFrame->GetDataBaseMode()==1))
				{
	...
	case GAME_SCENE_PLAY:	//游戏状态
		{
			...
			if (StatusPlay.cbOutCardCount > 0)
			{
				for (int i = 0; i < MAX_COUNT; i++)
				{
					if (StatusPlay.cbHuCardCount[i] > 0)
					{
						for (int j = 0; j < StatusPlay.cbHuCardCount[i]; j++)
						{
							StatusPlay.cbHuCardRemainingCount[i][j] = GetRemainingCount(wChairID, StatusPlay.cbHuCardData[i][j]);
						}
					}
					else
						break;
...
bool CTableFrameSink::OnUserTrustee(WORD wChairID, bool bTrustee)
{
	...
	if(bTrustee)
	{
		if(wChairID == m_wCurrentUser)
		{
			...
			return OnUserOutCard(wChairID,m_GameLogic.SwitchToCardData(cardindex),true);
		}
		else if(m_wCurrentUser == INVALID_CHAIR && m_cbUserAction[wChairID] != WIK_NULL)
		{
			BYTE operatecard[3]={0};
			return OnUserOperateCard(wChairID,WIK_NULL,operatecard);
		}
	}
	else
	{
...
bool CTableFrameSink::SendOperateNotify()
{
	...
	m_pITableFrame->SetGameTimer(IDI_OPERATE_CARD, (m_pGameCustomRule->cbTimeOperateCard + 2) * 1000, 1, 0);
	m_tActionStartTime = time(0);
...
bool CTableFrameSink::DispatchCardData(WORD wSendCardUser, bool bTail)
{
	...
	{
		tagGangCardResult GangCardResult;
		m_cbUserAction[wCurrentUser] |= m_GameLogic.AnalyseGangCardEx(m_cbCardIndex[wCurrentUser],m_WeaveItemArray[wCurrentUser], m_cbWeaveItemCount[wCurrentUser],m_cbProvideCard ,GangCardResult);
	}
	...
	if (m_cbProvideCard == 0x35 && (m_cbUserAction[wCurrentUser] & WIK_CHI_HU))//只剩两个红中自动胡牌
	{
		if (m_GameLogic.GetCardCount(m_cbCardIndex[wCurrentUser]) == m_cbCardIndex[wCurrentUser][31])
		{
			BYTE cbOperateCard[3] = { m_cbProvideCard };
			return OnUserOperateCard(wCurrentUser, WIK_CHI_HU, cbOperateCard);
		}
	}
		...
		if (EstimatKind == EstimatKind_OutCard )
		{
			...
			if(!m_bEnjoinChiPeng[i])
			{
				...
				if(bPeng)
				{
					...
					if (m_cbUserAction[i] & WIK_PENG)//碰牌后只剩红中则不能碰
					{
						BYTE cbCardCount = m_GameLogic.GetCardCount(m_cbCardIndex[i]);
						if (cbCardCount == (2 + m_cbCardIndex[i][31]))
						{
