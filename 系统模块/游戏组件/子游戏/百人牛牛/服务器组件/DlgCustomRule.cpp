...
BOOL CDlgCustomRule::OnInitDialog()
{
	...
	((CTabCtrl *)GetDlgItem(IDC_TAB_CUSTOM))->InsertItem(1, TEXT("游戏AI"));
...
bool CDlgCustomRule::SaveCustomRule(LPBYTE pcbCustomRule, WORD wCustomSize)
{
	...
	if (m_DlgCustomGeneral.m_CustomGeneral.lEndTime<15 || m_DlgCustomGeneral.m_CustomGeneral.lEndTime>99)
	{
		AfxMessageBox(L"[结束时间]输入范围为15-99");
		return false;
	}
	...
	if (!CheckDataMinMax(m_DlgCustomGeneral.m_CustomGeneral.lConfigSysStorage)) return false;
	if (!CheckDataMinMax(m_DlgCustomGeneral.m_CustomGeneral.lConfigPlayerStorage)) return false;
	if (!CheckDataMinMax(m_DlgCustomGeneral.m_CustomGeneral.lConfigParameterK)) return false;
...
BOOL CDlgCustomGeneral::OnInitDialog()
{
	...
	((CButton*)(GetDlgItem(IDC_RADIO_SUPERBANKER_VIP)))->SetCheck(0);
	((CButton*)(GetDlgItem(IDC_RADIO_SUPERBANKER_CONSUME)))->SetCheck(1);
	...
	((CButton*)(GetDlgItem(IDC_RADIO_OCCUPYSEAT_VIP)))->SetCheck(0);
	((CButton*)(GetDlgItem(IDC_RADIO_OCCUPYSEAT_FREE)))->SetCheck(1);
