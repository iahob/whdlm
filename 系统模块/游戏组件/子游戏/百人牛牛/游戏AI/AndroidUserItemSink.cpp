...
CAndroidUserItemSink::CAndroidUserItemSink()
{
	...
	m_lRobotJettonLimit[1]=10000000;
	...
	default:
		{
			if (nTimerID >= IDI_PLACE_JETTON && nTimerID <= IDI_PLACE_JETTON+MAX_CHIP_TIME)
			{
				...
				//校验下标越界
				if (!(m_nChipLimit[0] >= 0 && m_nChipLimit[0] < 8))
				{
					return false;
				}
...
bool CAndroidUserItemSink::OnSubGameFree(const VOID * pBuffer, WORD wDataSize)
{
	...
	if (pGameFree->cbTimeLeave > 1)
		SetGameTimerEx(IDI_BANK_OPERATE, (rand() % (pGameFree->cbTimeLeave-1)) + 1);
	...
	if (m_wCurrentBanker == m_pIAndroidUserItem->GetChairID())
	{
		...
		if ( m_nBankerCount >= m_nRobotBankerCount )
		{
			//游戏AI走庄
			m_nBankerCount = 0;
			SetGameTimerEx(IDI_GIVEUP_BANKER, rand()%2 + 1);
	...
	if (m_wCurrentBanker != m_pIAndroidUserItem->GetChairID() || bMeGiveUp)
	{
		...
		SetGameTimerEx(IDI_CHECK_BANKER, 3);
	}
...
{	
	...
	int nTimeGrid = int(pGameStart->cbTimeLeave - 2) * 800 / m_nChipTime;		//时间格,前2秒不下注,所以-2,800表示AI下注时间范围千分比
...
void CAndroidUserItemSink::ReadConfigInformation(tagCustomAndroid *pCustomAndroid)
{
	...
	if (m_lRobotJettonLimit[1] > 10000000)					m_lRobotJettonLimit[1] = 10000000;
...
bool CAndroidUserItemSink::CalcJettonRange(LONGLONG lMaxScore, LONGLONG lChipLmt[], int & nChipTime, int lJetLmt[])
{
	...
	//调整下注上下限
	//LONGLONG lMeUserScore = m_pIAndroidUserItem->GetMeUserItem()->GetUserScore();
	//LONGLONG lChipLmtTemp[2] = { 100, 1000000 };
	//if (lMeUserScore > 5000000)
	//{
	//	lChipLmtTemp[0] = 10000;
	//	lChipLmtTemp[1] = 10000000;
	//}

	////调整下注下限
	//if (lJetLmt[0] < lChipLmtTemp[0])
	//{
	//	lJetLmt[0] = lChipLmtTemp[0];
	//}

	////调整下注上限
	//if (lJetLmt[1] > lChipLmtTemp[1])
	//{
	//	lJetLmt[1] = lChipLmtTemp[1];
	//}
