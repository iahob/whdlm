...
BEGIN_MESSAGE_MAP(CDialogGeneral, CDialogDexter)
	...
	ON_COMMAND_RANGE(IDC_CHECK_EAST, IDC_CHECK_NORTH, OnButtonAreaCtrlArea)
	ON_COMMAND_RANGE(IDC_RADIO_SYS_LOST, IDC_RADIO_SYS_WIN, OnButtonAreaCtrlSysWinLost)
...
CDialogGeneral::CDialogGeneral(int nShowMax/* = 0*/, CClientConfigDlg *pParent /*=NULL*/) : CDialogDexter(IDD_CUSTOM_GENERAL, nShowMax, pParent)
{
	...
	ZeroMemory(&m_tagAdminReq, sizeof(m_tagAdminReq));
...
void CDialogGeneral::OnBnClickedBtnSetAreaCtrl()
{
	...
	tagAdminReq tempAdminReq;
	ZeroMemory(&tempAdminReq, sizeof(tempAdminReq));

	for (BYTE cbAreaIndex = 0; cbAreaIndex < AREA_MAX; cbAreaIndex++)
	{
		if (((CButton *)GetDlgItem(IDC_CHECK_EAST + cbAreaIndex))->GetCheck())
		{
			tempAdminReq.bWinArea[cbAreaIndex + 1] = true;
			tempAdminReq.cbDebugStyle = CS_BET_AREA;
		}
	}

	if (((CButton *)GetDlgItem(IDC_RADIO_SYS_LOST))->GetCheck() == TRUE)
	{
		ZeroMemory(tempAdminReq.bWinArea, sizeof(tempAdminReq.bWinArea));

		tempAdminReq.cbDebugStyle = CS_SYS_LOSE;
	}
	else if (((CButton *)GetDlgItem(IDC_RADIO_SYS_WIN))->GetCheck() == TRUE)
	{
		ZeroMemory(tempAdminReq.bWinArea, sizeof(tempAdminReq.bWinArea));

		tempAdminReq.cbDebugStyle = CS_SYS_WIN;
	}
	
	int nSelectTimes = (INT)((CComboBox *)GetDlgItem(IDC_COMBO_TIMES))->GetCurSel();
	tempAdminReq.cbExcuteTimes = g_cbExecuteTimesArray[nSelectTimes];

	if (tempAdminReq.cbDebugStyle != 0 && tempAdminReq.cbExcuteTimes >= 0 && tempAdminReq.cbExcuteTimes < MAX_EXECUTE_TIMES)
	{
		adminReq.cbReqType = RQ_SET_WIN_AREA;
		CopyMemory(adminReq.cbExtendData, &tempAdminReq, sizeof(tempAdminReq));
		CopyMemory(&m_tagAdminReq, &tempAdminReq, sizeof(tempAdminReq));
...
bool CDialogGeneral::ReqResult(const void *pBuffer)
{
	...
	case ACK_SET_WIN_AREA:
	{
		if (pResult->cbResult == CR_ACCEPT)
		{
			...
			if (pAdminReq->cbDebugStyle == CS_SYS_LOSE)
			{
				str.Format(TEXT("执行调试,胜利区域:单局系统输,剩余次数:%d"), pAdminReq->cbExcuteTimes);
			}
			else if (pAdminReq->cbDebugStyle == CS_SYS_WIN)
			{
				str.Format(TEXT("执行调试,胜利区域:单局系统赢,剩余次数:%d"), pAdminReq->cbExcuteTimes);
			}
			else if (pAdminReq->cbDebugStyle == CS_BET_AREA)
			{
				CString strAreaDesc;
				for (WORD wAreaIndex = 1; wAreaIndex < AREA_MAX + 1; wAreaIndex++)
				{
					if (pAdminReq->bWinArea[wAreaIndex])
					{
						strAreaDesc += GetAreaCtrlPosDesc(wAreaIndex);
						strAreaDesc += TEXT(".");
					}
				}
				
				str.Format(TEXT("执行调试,胜利区域:%s,剩余次数:%d"), strAreaDesc, pAdminReq->cbExcuteTimes);
			}
		}
		else
		{
	...
	case ACK_RESET_DEBUG:
	{
		if (pResult->cbResult == CR_ACCEPT)
		{
			...
			for (int nIndex = 0; nIndex < AREA_MAX; nIndex++)
			{
				((CButton *)GetDlgItem(IDC_CHECK_EAST + nIndex))->SetCheck(0);
			}

			((CButton *)GetDlgItem(IDC_RADIO_SYS_LOST))->SetCheck(0);
			((CButton *)GetDlgItem(IDC_RADIO_SYS_WIN))->SetCheck(0);

			if (m_tagAdminReq.cbExcuteTimes > 0)
			{
...
bool CDialogGeneral::OnDebugMessage(WORD nMessageID, WORD wTableID, void *pBuffer, WORD wDataSize)
{
	...
	case SUB_S_CUR_AREACTRL_INFO:
	{
		...
		if (pCurAreaCtrlInfo->cbDebugStyle == CS_SYS_LOSE)
		{
			SetDlgItemText(IDC_STATIC_AREACTRL_POS, TEXT("系统输"));
		}
		else if (pCurAreaCtrlInfo->cbDebugStyle == CS_SYS_WIN)
		{
			SetDlgItemText(IDC_STATIC_AREACTRL_POS, TEXT("系统赢"));
		}
		else if (pCurAreaCtrlInfo->cbDebugStyle == CS_BET_AREA)
		{
			CString strAreaDesc;
			for (WORD wAreaIndex = 1; wAreaIndex < AREA_MAX + 1; wAreaIndex++)
			{
				if (pCurAreaCtrlInfo->bWinArea[wAreaIndex])
				{
					strAreaDesc += GetAreaCtrlPosDesc(wAreaIndex);
					strAreaDesc += TEXT(".");
				}
			...
			SetDlgItemText(IDC_STATIC_AREACTRL_POS, strAreaDesc);
		}
	...
	case SUB_S_SEND_USER_BET_INFO:
	{
		...
		LONGLONG lUserJettonScore[GAME_PLAYER][AREA_MAX + 1];
...
BOOL CDialogGeneral::OnInitDialog()
{
	...
	pListUserBet->InsertColumn(nColumnCount++, TEXT("东"), LVCFMT_CENTER, 70);
	pListUserBet->InsertColumn(nColumnCount++, TEXT("南"), LVCFMT_CENTER, 70);
	pListUserBet->InsertColumn(nColumnCount++, TEXT("西"), LVCFMT_CENTER, 70);
	pListUserBet->InsertColumn(nColumnCount++, TEXT("北"), LVCFMT_CENTER, 70);
...
void CDialogGeneral::OnButtonAreaCtrlArea(UINT nID)
{
	((CButton *)GetDlgItem(IDC_RADIO_SYS_LOST))->SetCheck(FALSE);
	((CButton *)GetDlgItem(IDC_RADIO_SYS_WIN))->SetCheck(FALSE);
}

void CDialogGeneral::OnButtonAreaCtrlSysWinLost(UINT nID)
{
	for (BYTE cbAreaIndex = 0; cbAreaIndex < AREA_MAX; cbAreaIndex++)
	{
		if (((CButton *)GetDlgItem(IDC_CHECK_EAST + cbAreaIndex))->GetCheck())
		{
			((CButton *)GetDlgItem(IDC_CHECK_EAST + cbAreaIndex))->SetCheck(FALSE);
		}
	}
}
	...
	case SUB_S_REFRESH_AREACTRL:
	{
		...
		else
		{
			...
			if (nItemIndex == -1)
			{
				...
				if (pAreaCtrlResult->cbDebugStyle == CS_SYS_LOSE)
				{
					pListAreaCtrl->SetItemText(nItem, 1, TEXT("系统输"));
				}
				else if (pAreaCtrlResult->cbDebugStyle == CS_SYS_WIN)
				{
					pListAreaCtrl->SetItemText(nItem, 1, TEXT("系统赢"));
				}
				else if (pAreaCtrlResult->cbDebugStyle == CS_BET_AREA)
				{
					CString strAreaDesc;
					for (WORD wAreaIndex = 1; wAreaIndex < AREA_MAX + 1; wAreaIndex++)
					{
						if (pAreaCtrlResult->bWinArea[wAreaIndex])
						{
							strAreaDesc += GetAreaCtrlPosDesc(wAreaIndex);
							strAreaDesc += TEXT(".");
						}
					...
					pListAreaCtrl->SetItemText(nItem, 1, strAreaDesc);
					SetDlgItemText(IDC_STATIC_AREACTRL_POS, strAreaDesc);
				}
...
BOOL CDialogAdvanced::OnInitDialog()
{
	...
	pListAreaCtrl->InsertColumn(nColumnCount++, TEXT("调试位置"), LVCFMT_CENTER, 100);
