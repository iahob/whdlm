...
BOOL CClientControlItemSinkDlg::OnInitDialog()
{
	...
	m_listUserBet.InsertColumn(nColumnCount++, TEXT("进入金币"), LVCFMT_CENTER, 80);
...
void CClientControlItemSinkDlg::OnBnClickedButtonReset()
{
	...
	SendDebugMessage(SUB_C_ADMIN_COMMDN,&ControlApplication,sizeof(ControlApplication));
...
void CClientControlItemSinkDlg::OnRefreshKuCun()
{
	...
	SendDebugMessage(SUB_C_ADMIN_COMMDN,&adminReq,sizeof(adminReq));
}
	...
	for(INT i=0; i<m_UserBetArray.GetCount(); i++)
	{
		...
		for(BYTE j=0; j<AREA_COUNT; j++)
		{
		 ...
		 default:
			...
			strInfo.Format(TEXT("%I64d"), m_UserBetArray[i].lUserBet[j+1]);
			m_listUserBet.SetItemText(nItemIndex, nSubItem, strInfo);

			lAllBetScore[j] += m_UserBetArray[i].lUserBet[j+1];
		}
...
void CClientControlItemSinkDlg::UpdateUserList(bool bDelUser)
{
	...
	for (int i=0;i<m_UserBetArray.GetCount();i++)
	{
		...
		strInfo.Format(_T("%.2f元"), m_UserBetArray[i].lUserBet[AREA_FERRARI]/100.f);
		...
		strInfo.Format(_T("%.2f元"), m_UserBetArray[i].lUserBet[AREA_MASERATI] / 100.f);
		...
		{
			strInfo.Format(_T("%.2f元"), m_UserBetArray[i].lUserBet[j] / 100.f);
		...
		//总下注
		strInfo.Format(_T("%.2f元"), lUserTotalBet / 100.f);
		...
		//进入金币
		strInfo.Format(_T("%.2f元"), (m_UserBetArray[i].lUserInsure+m_UserBetArray[i].lUserStartScore) / 100.f);
		m_listUserBet.SetItemText(nItemIndex,13,strInfo);
		//总输赢
		strInfo.Format(_T("%.2f元"), (m_UserBetArray[i].lUserWinLost) / 100.f);
		m_listUserBet.SetItemText(nItemIndex,14,strInfo);
	...
	{
		strInfo.Format(_T("%.2f元"),lAttentionTotalBet[i] / 100.f);
...
void CClientControlItemSinkDlg::OnBnClickedButtonAdduser()
{
	...
	if(str.IsEmpty()==false)
	{
		...
		{
			CString strGameID;
			strGameID.Format(_T("%d"), m_UserBetArray[i].dwUserGameID);
			if(0==str.Compare(strGameID))
			{
	...
	else
	{
		while(pos) //如果选择多行
		{
			...
			if(nIdx >=0&&nIdx<m_ListAttention.GetItemCount())
			{
				...
				{
					CString strTemp;
					strTemp.Format(_T("%d"), m_UserBetArray[i].dwUserGameID);
...
bool CClientControlItemSinkDlg::OnDebugMessage(WORD nMessageID, WORD wTableID, void *pBuffer, WORD wDataSize)
{
	...
	case SUB_S_SEND_USER_BET_INFO:	//更新下注
	{
		...
		LONGLONG lUserJettonScore[AREA_ALL+1][GAME_PLAYER];

		for (WORD i = 0; i < GAME_PLAYER; i++)
		{
			for (WORD wAreaIndex = 0; wAreaIndex <= AREA_ALL; wAreaIndex++)
			{
				lUserJettonScore[wAreaIndex][i] = (LONGLONG)pSendUserBetInfo->lUserJettonScore[wAreaIndex][i];
			}
		}
		...
		for (WORD i = 0; i < GAME_PLAYER; i++)
		{
			...
			UserBet.bAndroid = m_UserInfo.bIsAndroid[nUserIndex];
			UserBet.lUserInsure = m_UserInfo.lInsureScore[nUserIndex];
			for (BYTE k = 0; k < AREA_ALL; k++)
			{
				UserBet.lUserBet[k] = lUserJettonScore[k+1][i];
			}

			// 判断是否被关注
			CString strUserGameID;
			bool bAttention = false;
			strUserGameID.Format(_T("%d"), UserBet.dwUserGameID);
			for (int i = 0; i < m_ListAttention.GetItemCount(); i++)
			{
				CString strGameID = m_ListAttention.GetItemText(i, 0);
				if (strUserGameID.Compare(strGameID) == 0)
				{
					bAttention = true;
					break;
				}
			}
			UserBet.bAttention = bAttention;
