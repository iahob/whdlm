...
bool  CTableFrameSink::Initialization(IUnknownEx * pIUnknownEx)
{
	...
	m_hInst = NULL;
	m_pServerDebug = NULL;
	m_hInst = LoadLibrary(TEXT("LuxuryCarServerDebug.dll"));
	if ( m_hInst )
	{
		typedef void * (*CREATE)(); 
		...
		if (ServerDebug)
		{
			m_pServerDebug = static_cast<IServerControl*>(ServerDebug());
		}
	}
...
bool  CTableFrameSink::OnEventGameConclude(WORD wChairID, IServerUserItem * pIServerUserItem, BYTE cbReason)
{
	...
	case GER_NORMAL:		//常规结束	
		{
			if ( m_pServerDebug != NULL && m_pServerDebug->NeedControl() )
			{
				tagControlInfo ControlInfo;
				m_pServerDebug->ReturnControlArea(ControlInfo,m_lAllJettonScore);
				m_cbTableCard = ControlInfo.cbControlArea;
				m_pServerDebug->CompleteControl();
				//系统输赢
	...
	case GAME_STATUS_FREE:			//空闲状态
		{
			...
			CopyMemory(StatusFree.lBetScore, m_lBetScore, sizeof(StatusFree.lBetScore));
			//控制信息
	...
	case GS_GAME_END:			//结束状态
		{
			...
			CopyMemory(StatusPlay.lBetScore, m_lBetScore, sizeof(StatusPlay.lBetScore));
			//全局下注
	...
	if (wSubCmdID >= SUB_GF_FRAME_MESSAG_GAME_MIN && wSubCmdID <= SUB_GF_FRAME_MESSAG_GAME_MAX)
	{
		switch (wSubCmdID - SUB_GF_FRAME_MESSAG_GAME_MIN)
		{
			case SUB_C_ADMIN_COMMDN:
			{
				...
				if (m_pServerDebug == NULL) return true;
				...
				if (AdminReq->cbControlAppType == C_CA_SET_STORAGE)
				{
					...
					m_pServerDebug->SetStorage(m_lStorageCurrent);
				...
				return m_pServerDebug->ServerControl(wSubCmdID, pDataBuffer, wDataSize, pIServerUserItem, m_pITableFrame);
			}
...
void CTableFrameSink::RobotBankerControl()
{
	...
	for (int i = 0; i < GAME_PLAYER; i++)
	{
		...
		if (pIServerUserItem != NULL && pIServerUserItem->IsAndroidUser())
		{
			if ( m_wCurrentBanker != wChairID && pIServerUserItem->GetUserScore() >= m_lApplyBankerCondition)
			{
				//检测上庄列表
...
void CTableFrameSink::ReadConfigInformation()
{
	...
	CopyMemory(m_lBetScore, pCustomConfig->CustomGeneral.BetScore, sizeof(m_lBetScore));
...
void CTableFrameSink::SendUserBetInfo()
{
	...
	for (WORD i = 0; i < GAME_PLAYER; i++)
	{
		...
		{
			SendUserBetInfo.lUserJettonScore[wAreaIndex][i] = (LONG)m_lUserJettonScore[wAreaIndex][i];
		}
	}
