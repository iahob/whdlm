...
{
	//获取类型
	BYTE cbTempNextCardData[MAX_COUNT];
	ZeroMemory(cbTempNextCardData, sizeof(cbTempNextCardData));
	CopyMemory(cbTempNextCardData, cbNextCard, sizeof(BYTE)* cbNextCount);
	SortCardList(cbTempNextCardData, cbNextCount, ST_ORDER);

	BYTE cbTempFirstCardData[MAX_COUNT];
	ZeroMemory(cbTempFirstCardData, sizeof(cbTempFirstCardData));
	CopyMemory(cbTempFirstCardData, cbFirstCard, sizeof(BYTE)* cbFirstCount);
	SortCardList(cbTempFirstCardData, cbFirstCount, ST_ORDER);

	BYTE cbNextType = GetCardType(cbTempNextCardData, cbNextCount);
	BYTE cbFirstType = GetCardType(cbTempFirstCardData, cbFirstCount);
	...
	case CT_BOMB_CARD:
		{
			//获取数值
			BYTE cbNextLogicValue = GetCardLogicValue(cbTempNextCardData[0]);
			BYTE cbFirstLogicValue = GetCardLogicValue(cbTempFirstCardData[0]);
...
bool CGameLogic::IsValidCard(BYTE cbCardData)
{
	...
	if (cbCardValue >= 1 && cbCardValue <= 0x0F && cbCardColor >= 0 && cbCardColor <= 3)
	{
		return true;
	}
