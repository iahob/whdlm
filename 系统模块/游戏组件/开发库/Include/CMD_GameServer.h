...
#define SUB_GR_USER_ROOM_DEBUG		13									//房间配置
#define SUB_GR_USER_CHANGE_CHAIR	14									//换位置
#define SUB_GR_USER_ROOM_AICONFIG_DEBUG	15								//AI属性配置
...
#define SUB_GR_AI_HANGING_NOTIFY	105									//AI挂机事件
#define SUB_GR_AI_GRADE_NOTIFY		106									//AI等级事件
...
//AI挂机状态
struct CMD_GR_AndroidHangingNotify
{
	DWORD							dwUserID;							//用户标识
	bool							bAndroidUserHanging;				//挂机状态
};

//AI等级
struct CMD_GR_AndroidGradeNotify
{
	DWORD							dwUserID;							//用户标识
	WORD							wAndroidGrade;						//机器等级(携带和离开等级)
...
struct CMD_GR_S_UserInsureInfo
{
	...
	DWORD							dwUserRight;						//用户权限
	SCORE							lBankOPButtonArray[LEN_BANKEROP_BT];//银行存取操作按钮数组//银行存取操作按钮数组(后两位为全存全取)
};
...
//道具信息
#define SUB_GP_LIST_GAME_PROPERTY_TYPE				301						//道具类型
#define SUB_GP_LIST_GAME_PROPERTY_RELAT				302						//道具关系
#define SUB_GP_LIST_GAME_PROPERTY					303						//道具列表
#define SUB_GP_LIST_GAME_PROPERTY_SUB				304						//子道具列表
...
#define SUB_GR_ROOMDEBUG			13									//调试信息
#define SUB_GR_ROOMAICONFIGDEBUG	14									//AI属性调试信息
...
//AI属性配置
struct CMD_GR_RoomAIConfigDebug
{
	WORD							wSubCmdID;							//子命令
	WORD							wTableID;							//目标桌子
};
...
#define SUB_GR_MATCH_PROMOTION		17									//晋级提醒
...
#define SUB_GF_COMMOND_START		4									//命令开始

//聊天命令
#define SUB_GF_USER_CHAT			10									//用户聊天
#define SUB_GF_USER_EXPRESSION		11									//用户表情
#define SUB_GF_USER_VOICE			12									//用户语音
#define SUB_GF_H5_USER_VOICE		13									//H5用户语音

//关闭房间时写分，目前只针对 捕鱼、连环夺宝
#define SUB_GF_WRITE_REMAIN_SCORE	50									//写剩余分
...
//通知司令官显示开始按钮
#define SUB_GF_GAME_USER_COMMOND			800							//显示开始按钮
//////////////////////////////////////////////////////////////////////////////////
//用户信息
struct CMD_CF_UserInfo
{
	DWORD							wUserID[MAX_ROOMCHAIR];
	DWORD							wGameID[MAX_ROOMCHAIR];
	LONGLONG						lUserScore[MAX_ROOMCHAIR];
	LONGLONG						lInsureScore[MAX_ROOMCHAIR];
	SCORE							lHistoryKindIDWin[MAX_ROOMCHAIR];					//该游戏历史总输赢
	TCHAR							szNickName[MAX_ROOMCHAIR][LEN_NICKNAME];
	
	//用户属性
	bool							bIsAndroid[MAX_ROOMCHAIR];							//机器标识
	//用户状态
	WORD							wTableID[MAX_ROOMCHAIR];							//桌子索引
	WORD							wChairID[MAX_ROOMCHAIR];							//椅子索引
	BYTE							cbUserStatus[MAX_ROOMCHAIR];						//用户状态
...
//用户语音
struct CMD_GF_H5_UserVoice
{
	DWORD							dwSendUserID;						//发送用户
	BYTE							byVoiceID[MAXT_H5_VOICE_LENGTH];	//语音ID
};
...
//玩家命令
struct CMD_GF_UserCommond
{
	DWORD							dwUserID;							//玩家标识
	WORD							wChairID;							//椅子标识
	DWORD							dwWaitting;							//等待时间
};
...
#define SUB_GR_PLAYING_USER_INFO		400							//参与过游戏的玩家信息	

#define SUB_GR_AI_CREATE_TABLE			401							//AI创建桌子//创建桌子
...
struct CMD_GR_PersonalTableTip
{
	...
	BOOL							bRoomPwd;							//房间密码
	byte							cbPayMode;							//支付模式 0 房主支付， 1 AA 支付
	SCORE							lEnterScoreLimite;					//进入限制
	SCORE							lLeaveScoreLimite;					//离开限制;
	DWORD							dwGroupID;							//联盟ID
};
...
{
	TCHAR szMessage[260];
	BYTE  cbMessageType; // 0 代表 坐下 1 代表断线重入 2 旁观坐下返回
};
...
struct CMD_GR_PlayingUserInfo
{
	DWORD			dwUserID[MAX_CHAIR];				//申请解散玩家处理方式 0 未处理 1 同意 2 不同意
};
