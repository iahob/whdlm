...
//平台定义
#include "Platform.h"
#include "GameAIParameter.h"

//消息定义
#include "CMD_Commom.h"
#include "CMD_Correspond.h"
#include "CMD_GameServer.h"
#include "CMD_LogServer.h"
...
#define ANDROID_ENTERGAME			0x02								//进入游戏
#define ANDROID_OCCUPYSEAT			0x10								//AI占位
#define ANDROID_BEBANKER			0x20								//AI庄家

//AICHATTYPE
#define ANDROID_INVALID_CT			0x0000								//无效类型
#define ANDROID_OTHER_OVERTIME_CT	0x0001								//其他玩家超时
#define ANDROID_ENTERGAME_CT		0x0002								//进入游戏
#define ANDROID_STANDUP_CT			0x0004								//离开游戏
#define ANDROID_FREQUENT_CHAT_CT	0x0008								//频繁聊天
#define ANDROID_OTHER_STANDUP_CT	0x0010								//其他玩家离开游戏
#define ANDROID_OVERTIME_CT			0x0020								//自己超时
#define ANDROID_SERIALWIN_CT		0x0040								//其他玩家连续胜利三局以上
...
#define GER_DISMISS_FREE			0x04								//游戏解散（空闲时间）
...
struct tagGameParameter
{
	...
	SCORE							lDayWinLostScoreLimit;				//每日限制
	DWORD							dwDayTimeLimit;						//每日限制
};
...
//等级配置
struct tagGameServerLevelInfo
{
	BYTE							cbServerLevelCount;							//等级数目
	tagServerLevelItem				ServerLevelItemInfo[MAX_GAMESERVER_LEVEL];	//等级信息
};

//聊天配置
struct tagGameChatInfo
{
	BYTE							cbGameChatCount;							//聊天数目
	tagGameChatItem					GameChatItemInfo[MAX_GAMECHAT];				//聊天信息
};
...
struct tagScoreInfo
{
	...
	SCORE							lScore;								//用户分数
	SCORE							lGrade;								//用户成绩
	SCORE							lRevenue;							//游戏服务比例
	SCORE							lChipSerialScore;					//游戏流水
};
...
struct tagUserInfoPlus
{
	...
	SCORE							lRestrictScore;						//限制积分
	DWORD							dwDayPalyTime;						//每日时间
	SCORE							lDayWinLostScore;					//每日成绩
	...
	//群组信息
	TCHAR							szGroupID[GROUP_ID_ARRAY_LEN];		//大联盟ID数组
};
...
{
	DWORD							dwUserID;							//机器标识
	SCORE							lInitialCarryScore;					//初始携带
	SCORE							lGameVariationScore;				//游戏变化
...
//离开信息
struct tagAndroidLeaveInfo
{
	DWORD							dwUserID;							//机器标识
	DWORD							dwLeaveTickCount;					//离开时间
};
...
interface IGameServiceCustomRule : public IUnknownEx
{
	...
	//默认等级配置
	virtual bool DefaultServerLevel(LPBYTE pcbCustomRule, WORD wCustonSize, tagGameServerLevelInfo & GameServerLevelInfo, WORD wCurSelServerLevel) = NULL;
};
...
public:
	...
	//每日时间
	virtual DWORD  GetDayPlayTime() = NULL;
	//每日成绩
	virtual SCORE  GetDayWinLostScore() = NULL;
	//游戏数据
public:
	//游戏数据
...
public:
	...
	//挂机状态
	virtual bool IsAndroidUserHanging() = NULL;
	//设置挂机状态
	virtual VOID SetAndroidUserHanging(bool bAndroidUserHanging) = NULL;
	//设置机器等级(携带和离开等级)
	virtual VOID SetAndroidGrade(WORD wAndroidGrade) = NULL;
	//获取机器等级(携带和离开等级)
	virtual WORD GetAndroidGrade() = NULL;
...
public:
	...
	//获取名次
	virtual BYTE GetUserMatchRank() = NULL;
	//设置名次
	virtual VOID SetUserMatchRank(BYTE rank) = NULL;
...
public:
	...
	//写入积分
	virtual bool WriteUserScore(SCORE lScore, SCORE lGrade, SCORE lRevenue, SCORE lChipSerialScore, SCORE lIngot, BYTE cbScoreType, DWORD dwPlayTimeCount, DWORD dwWinExperience, DWORD dwGroupID) = NULL;
	//领取奖励
...
public:
	//大联盟数据
	virtual TCHAR *	GetGroupIDInfo() = NULL;
};
...
interface IServerUserItemSink : public IUnknownEx
{
	...
	//AI挂机事件
	virtual bool OnEventAndroidUserHangingNotify(IServerUserItem * pIServerUserItem, bool bAndroidUserHanging) = NULL;
	//AI等级事件
	virtual bool OnEventAndroidUserGradeNotify(IServerUserItem * pIServerUserItem, WORD wAndroidGrade) = NULL;
};
...
public:
	...
	//发送数据
	virtual bool SendLookonData(WORD wChairID, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize) = NULL;
	//发送数据
	...
	//发送数据
	virtual bool SendRoomAIConfigData(IServerUserItem * pIServerUserItem, WORD wSubCmdID) = NULL;
	//发送数据
	virtual bool SendRoomAIConfigData(IServerUserItem * pIServerUserItem, WORD wSubCmdID, VOID * pData, WORD wDataSize) = NULL;
	//发送日志数据
	virtual bool SendGameLogData(tagLogUserInfo LogUserInfo) = NULL;
	//发送日志数据
	virtual bool SendGameLogData(const TCHAR * szLogInfo, ...) = NULL;
	//发送日志数据
	virtual bool SendGameLogData(CString cstrLogInfo) = NULL;
	//发送日志数据
	virtual bool SendGameLogData(IServerUserItem * pIServerUserItem, const TCHAR * pszFormat, ...) = NULL;
	//发送日志数据
	virtual bool SendGameLogData(IServerUserItem * pIServerUserItem, const TCHAR szDescName[LEN_SERVER], const TCHAR * pszFormat) = NULL;
...
public:
	...
	//配置筹码
	virtual VOID SetBetValueArray(SCORE lBetValueArray[MAX_AI_CHIP_COUNT]) = NULL;

	//设置大联盟百人游戏庄家USERID
	virtual VOID SetGroupMaxChairBankerUserID(WORD wBankerChairID, DWORD dwBankerUserID) = NULL;
...
public:
	...
	virtual void SetPlayingStatusLeave(WORD wChairID, byte cbCanLeave) = NULL;
	//获取大联盟ID
	virtual DWORD GetGroupID() = NULL;
	//获取大联盟服务比例信息
	virtual void GetGroupTaxInfo(BYTE &cbTaxMode, LONGLONG &lTaxCount) = NULL;
	//共享库存服务
public:
	//共享库存子项改变
	virtual void ChangeCurSubStorageItem(ENUM_SHARESTORAGE_STATUS emShareConfigStatus, tagCurSubStorageItem & CurSubStorageItem) = NULL;
	//获取当前共享库存子项
	virtual tagCurSubStorageItem* GetCurSubStorageItem() = NULL;
	//获取配置共享库存子项
	virtual tagConfigSubStorageItem* GetConfigSubStorageItem() = NULL;
	//是否记录过参与次数
	virtual bool IsRecordDrawCount(DWORD dwUserID) = NULL;
};
...
#ifdef _UNICODE
#define VER_ITableShareStorage INTERFACE_VERSION(1,1)
static const GUID IID_ITableShareStorage = { 0xfc06aa07, 0x673d, 0x4a71, 0xac, 0x9a, 0x99, 0x21, 0xb5, 0x3, 0xf2, 0x71 };
#else
#define VER_ITableShareStorage INTERFACE_VERSION(1,1)
static const GUID IID_ITableShareStorage = { 0x4a3e2ae0, 0x2246, 0x4a2a, 0x95, 0x6b, 0x15, 0xd6, 0x9a, 0xac, 0x94, 0x3b };
#endif

//用户动作
interface ITableShareStorage : public IUnknownEx
{
	//校验接口
public:
	//校验共享库存
	virtual bool EfficacyConfigSubStorage(tagShareStorageItem *ptagShareStorageItem, TCHAR szResultDescripe[512]) = NULL;
};
...
public:
	...
	//比赛人数
	virtual bool SendMatchMessage(LONGLONG lMatchNo, WORD wMatchCount) = NULL;
	//系统消息
	virtual bool SendSystemMessage(LPCTSTR lpszMessage) = NULL;
	//发送日志
	virtual void SendLogData(IServerUserItem * pIServerUserItem, const TCHAR * pszFormat, ...) = NULL;
	//发送日志
	virtual void SendLogData(const TCHAR * pszFormat, ...) = NULL;
	//发送日志数据
	virtual void SendLogData(tagLogUserInfo LogUserInfo) = NULL;
	//发送日志数据
	virtual void SendLogData(IServerUserItem * pIServerUserItem, const TCHAR szDescName[LEN_SERVER], const TCHAR * pszFormat) = NULL;
...
public:
	...
	//群发数据
	virtual bool SendDataBatchToDebugUser(IServerUserItem * pIServerUserItem, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize) = NULL;
...
public:
	...
	//解锁AI
	virtual VOID UnLockAndroidUser(DWORD dwUserID) = NULL;
	//解散私人桌子
	...
	//更新桌子
	virtual VOID UpdateTablePlayerCount(WORD wServerID, WORD wTableID, WORD wCount, DWORD dwUserID, WORD wChairID, bool bSit, bool bGameStart) = NULL;
	//更新玩家大联盟分
	virtual VOID UpdateGroupScore(DWORD dwGroupID, IServerUserItem *pServerUserItem, SCORE lScore,bool bUpdateGame=false) = NULL;
	//更新大联盟百人游戏庄家USERID
	virtual VOID UpdateGroupMaxChairBankerUserID(WORD wServerID, WORD wTableID, WORD wBankerChairID, DWORD dwBankerUserID) = NULL;
	//开始游戏写入参与信息
...
public:
	...
	//获取挂机
	virtual bool IsAiHanging() = NULL;
	//设置挂机
	virtual VOID SetAiHanging(bool bHanging) = NULL;

	//配置信息
public:
	//获取A I 配置 
	virtual tagRoomAIParameter* GetRoomAIParameter() = NULL;
	//获取上次聊天时间
	virtual DWORD GetLastChatMessageTickCount() = NULL;
	//获取登录房间时间
	virtual DWORD GetLogonFinishTickCount() = NULL;
	//获取随机携带等级索引
	virtual BYTE GetRandomCarryGradeIndex() = NULL;
	//获取持有金币/初始金币
	virtual SCORE GetScoreVariationTimes() = NULL;
	//设置初始携带
	virtual void SetUserItemInitialCarryScore(DWORD dwUserID, SCORE lInitialCarryScore) = NULL;
	//设置游戏变化
	virtual void SetUserItemGameVariationScore(DWORD dwUserID, SCORE lGameVariationScore) = NULL;
	//设置离开等级
	virtual void SetAndroidLeaveGrade(WORD wLeaveGrade) = NULL;
	//设置机器动作
	virtual void SetAndroidAction(WORD wAndroidAction, bool bEnable) = NULL;
	//设置AI聊天
	virtual void SetAndroidChatType(WORD wAndroidChatType) = NULL;
	//获取换桌概率
	virtual WORD GetSwitchTableRate() = NULL;
	//校验同桌
	virtual bool EfficacyAndroidFreeSameTable(WORD wTableID) = NULL;
...
public:
	...
	//获取同桌真人玩家
	virtual	WORD GetSitRealUserCount() = NULL;
...
public:
	...
	//机器等级
	virtual bool JudgeAndroidGrade(WORD wAndroidGrade) = NULL;
...
public:
	...
	//发送聊天
	virtual bool SendChatMessage(DWORD dwTargetUserID) = NULL;
	//发送表情
...
public:
	//携带等级索引
	virtual BYTE GetCarryGradeIndex() = NULL;
	//获取AI聊天
	virtual BYTE GetAndroidChatType() = NULL;
};
...
public:
	...
	//移除参数
	virtual bool RemoveAndroidParameter() = NULL;
	//添加参数
	virtual bool AddAndroidParameter(tagAndroidParameter AndroidParameter) = NULL;
	//插入机器
	virtual bool InsertAndroidInfo(tagAndroidAccountsInfo AndroidAccountsInfo[], WORD wAndroidCount, tagGameChatInfo * pGameChatInfo) = NULL;
	//绑定桌子
...
public:
	...
	//创建机器
	virtual IAndroidUserItem * CreateAndroidUserItem(tagAndroidAccountsInfo & AndroidAccountsInfo) = NULL;
	//枚举机器
	virtual IAndroidUserItem * EnumAndroidUserItem(WORD wEnumIndex) = NULL;
	//设置初始携带
	virtual void SetAndroidInitialCarryScore(DWORD dwUserID, DWORD dwContextID, SCORE lInitialCarryScore) = NULL;
	//设置游戏变化
	virtual void SetAndroidGameVariationScore(DWORD dwUserID, DWORD dwContextID, SCORE lGameVariationScore) = NULL;
	//获取用户
	virtual IServerUserItem * GetUserItem(WORD wTableID, WORD wChairID) = NULL;
...
public:
	...
	//挂机机器数目
	virtual WORD GetAndroidHangingCount() = NULL;
	//加载机器
	virtual bool GetAndroidLoadInfo(DWORD & dwLoadCount) = NULL;
	//用户状况
	virtual WORD GetAndroidUserInfo(tagAndroidUserInfo & AndroidUserInfo, DWORD dwServiceMode) = NULL;
	//获取相应等级数量
	virtual WORD GetAndroidGradeCount(WORD wAndroidGrade) = NULL;
	//获取房间配置
	virtual tagGameServiceOption* GetGameServiceOption() = NULL;
	//获取游戏属性
	virtual tagGameServiceAttrib* GetGameServiceAttrib() = NULL;
	...
	virtual tagGameMatchOption* GetGameMatchOption() = NULL;
	//获取聊天配置
	virtual tagGameChatInfo * GetGameChatInfo() = NULL;
	//获取机器配置
...
public:
	...
	//发送日志数据
	virtual void  SendLogData(TCHAR * pszLogInfo) = NULL;
	//发送日志数据
	virtual void SendLogData(tagLogUserInfo LogUserInfo) = NULL;
};
