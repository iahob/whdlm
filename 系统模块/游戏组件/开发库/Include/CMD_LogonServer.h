...
//客户端类型(H5/U3D/LUA)
#define CLIENT_TYPE_H5				0x01								//H5
#define CLIENT_TYPE_U3D				0x02								//U3D
#define CLIENT_TYPE_LUA				0x03								//LUA
...
#define SUB_GP_MODIFY_LOGON_PASS_H5		104								//修改密码H5专用
#define SUB_GP_MODIFY_LOGON_PASS_H5_Ex	105								//修改密码H5专用
...
//签到服务
#define SUB_GP_CHECKIN_QUERY			222								//查询签到
#define SUB_GP_CHECKIN_INFO				221								//签到信息
#define SUB_GP_CHECKIN_DONE				220								//执行签到
#define SUB_GP_CHECKIN_RESULT			223								//签到结果
#define SUB_GP_CHECKIN_GETCYCREWARD		224								//领取礼包
#define SUB_GP_CHECKIN_INFO_END			225								//签到信息
...
//查询用户身上的钱
#define SUB_GP_GET_ALL_MONEY_KIND		502								//操作失败
#define SUB_GP_GET_ALL_MONEY_KIND_RESULT		503						//获取所有钱结果

//获取玩家财富信息
#define SUB_GP_GET_USER_WEALTH			504								//获取玩家财富信息
#define SUB_GP_GET_USER_WEALTH_RESULT	505								//获取玩家财富信息结果
//////////////////////////////////////////////////////////////////////////////////
...
//修改密码
struct CMD_GP_ModifyLogonPassH5
{
	DWORD							dwUserID;							//用户 I D
	TCHAR							szDesPassword[LEN_PASSWORD];		//用户密码
	TCHAR							szMobilePhone[LEN_MOBILE_PHONE];	//手机号
	TCHAR							szCheckCode[LEN_CHECK_CODE];		//验证码
};

struct CMD_GP_ModifyLogonPassH5_Ex
{
	TCHAR							szAccounts[LEN_ACCOUNTS];		//登录帐号
	TCHAR							szDesPassword[LEN_PASSWORD];		//用户密码
	TCHAR							szMobilePhone[LEN_MOBILE_PHONE];	//手机号
...
//等级配置
struct CMD_GP_ConfigGameServerLevel
{
	BYTE							cbServerLevelCount;						//等级数目
	tagServerLevelItem				ServerLevelItem[MAX_GAMESERVER_LEVEL];	//等级描述
};

//所有种类的钱
struct CMD_GP_AllKindMoney
{
	DWORD							dwUserID;							//用户 I D
	TCHAR							szPassword[LEN_PASSWORD];	//用户密码
};
...
//所有种类的钱
struct CMD_GP_AllKindMoneyResult
{
	SCORE							lInsure;							//银行金币
	SCORE							lDiamond;							//钻石
};
...
//所有种类的钱
struct CMD_GP_UserWealth
{
	DWORD							dwUserID;							//用户 I D
	TCHAR							szDynamicPassWord[LEN_PASSWORD];	//用户密码
};

//所有种类的钱
struct CMD_GP_GetUserWealthResult
{
	SCORE							lScore;					//携带金币
	SCORE							lInsure;				//银行金币
	SCORE							lDiamond;				//钻石
	SCORE							lAwardTicket;			//返奖券
};
//////////////////////////////////////////////////////////////////////////////////
...
struct CMD_GP_UserInsureInfo
{
	...
	SCORE							lBankOPButtonArray[LEN_BANKEROP_BT];//银行存取操作按钮数组(后两位为全存全取，大厅银行为0)
};
...
{	
	...
	DWORD							nGotPackageID[16];					//已领礼包
	DWORD							nPackCount;							//礼包总数
};
...
//领取礼包
struct CMD_GP_CheckInGetCycReward
{
	DWORD							dwUserID;							//用户标识
	DWORD							nPackageID;							//礼包I D
	TCHAR							szPassword[LEN_PASSWORD];			//登录密码
};
...
struct CMD_GP_CheckInResult
{
	...
	DWORD							nPackageID;							//礼包I D
	BYTE							cbType;								//礼包类型
	LONGLONG						lScore;								//用户金币
	LONGLONG						lDiamond;							//用户钻石
	int								nAwardTicket;						//用户奖卷
...
struct CMD_GP_PropertyBuy
{
	...
	BYTE							cbUseKind;							//购买类型(0钻石1金币)
	DWORD							dwPropCount;						//道具数目	
	TCHAR							szPassword[LEN_PASSWORD];			//登录密码
	TCHAR							szMachineID[LEN_MACHINE_ID];		//机器序列
};
...
#define SUB_MB_DATA_DEFINE_CONFIG	104								//私人房间配置

//升级提示
#define SUB_MB_UPDATE_NOTIFY		200									//升级提示
#define SUB_GP_CONFIG_GAMESERVER_LEVEL 201								//等级配置
...
struct CMD_MB_LogonOtherPlatform
{
	...
	BYTE							cbClientType;						//客户端类型(H5/U3D/LUA)
};
...
#define SUB_MB_LIST_LUA_GAME		103									//LUA游戏列表	
#define SUB_MB_LIST_H5_GAME			104									//H5游戏列表
#define SUB_MB_LIST_U3D_GAME		105									//U3D游戏列表
#define SUB_MB_LIST_GAME_END		106									//游戏列表结束
...
#define SUB_MB_QUERY_VIDEO_INFO_EX					6							//视频查询扩展
//返回结果
...
{
	int						iQueryType;									//0:约战 1:普通 2：约战积分 3 约战金币
	DWORD					dwUserID;									//用户 I D
...
//查询视频
struct  CMD_MB_C_QueryVideoInfoEx
{
	int						iQueryType;									//0:约战 1:普通 2：约战积分 3 约战金币
	DWORD					dwUserID;									//用户 I D
	DWORD					dwPlayBack;									//查询视频 （0全部，其它是回访码）
...
{
	INT						iVideoType;									//1:普通 2:约战积分  3:约战金币
	DWORD					dwUserID;									//用户I D
	DWORD					dwPersonalRoomID;							//约战房间ID
	TCHAR					szNickName[LEN_NICKNAME];					//用户昵称
	TCHAR					szPersonalGUID[PERSONAL_ROOM_GUID];			//约战房唯一ID
...
#define SUB_MB_QUERY_GOLD_ROOM_LIST					228					//获取金币约战房间列表
#define SUB_MB_QUERY_GOLD_ROOM_LIST_RESULT			229					//获取金币约战房间列表结果
#define SUB_MB_QUERY_GOLD_ROOM_LIST_FINISH			230					//获取金币约战房间列表结束
#define SUB_MB_PERSONAL_GET_OFFLINE_INFO    		231					//获取断线信息
#define SUB_MB_PERSONAL_GET_OFFLINE_INFO_RESULT    	232					//获取断线信息结果
...
//获取金币约战房间列表
struct CMD_MB_GetGoldRoomList
{
	WORD		wKindID;		//游戏类型
	byte		cbPlayMode;	//游戏模式
	DWORD		dwGroupID;		//俱乐部id， 非俱乐部模式 填 0
};

//服务端数据大小定义， 目前只有约战信息
#define MDM_MS_DATA_DEFINE						300						//数据大小定义
#define SUB_MS_DATA_DEFINE_DETAILS				1						//数据大小定义详情

const struct CMD_MS_DataDefine
{
	int nPersonalRuleLen;	//房间规则长度							
	int nSpecialInfoLen;	//针对房间结束时结算时的一些特殊要求
	int nPersonalRoomChair;	//私人房间座子上椅子的最大数目

	CMD_MS_DataDefine()
	{
		nPersonalRuleLen = RULE_LEN;
		nSpecialInfoLen = SPECIAL_INFO_LEN;
		nPersonalRoomChair = PERSONAL_ROOM_CHAIR;
	}
};
...
//获取断线信息结果
struct CMD_GP_GetOffLineInfoResult
{
	DWORD						dwServerID;								//服务端标识
};
//////////////////////////////////////////////////////////////////////////////////
